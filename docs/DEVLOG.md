# 개발 이력 (DEVLOG)

프로젝트 진행 중 결정·삽질·배운 점을 시간순으로 기록한다. 나중에 포트폴리오 회고/README의 재료가 된다.

---

## 2026-07-10 · 프로젝트 킥오프

- **결정**: 주제를 "선착순 티켓 예매(Sunchak)"로 확정. 이유 — DB 설계·동시성/큐·인증을 도메인이 자연스럽게 요구하는 소재.
- **스택 확정**: Next.js(FE) + NestJS(BE) + PostgreSQL + Redis + BullMQ + SSE. 인프라는 Docker + 클라우드 VM + GitHub Actions.
- **범위**: 실 결제(PG)는 제외, 모의 결제 + 비동기 처리로 대체. 좌석 배치도는 수량 기반으로 단순화.
- **문서**: 기술 로드맵(`01_기술_로드맵.md`), 서비스 기획안(`02_서비스_기획안.md`) 작성 완료.
- **셋업**: `~/Desktop/sunchak` 폴더 구조 생성, git 이력 시작, 로컬 postgres+redis용 docker-compose 작성.
- **다음**: W1 — Prisma 스키마(ERD 구현) + NestJS 프로젝트 스캐폴딩 + JWT 인증.

## 2026-07-10 · 의사결정 기록(ADR) 체계 도입

- **관행 도입**: 모든 중요한 선택은 `docs/decisions/`에 ADR로 남긴다(대안·근거 포함). 학습 최우선 원칙에 따라 "왜 이걸 골랐나"를 항상 기록.
- **초기 기록**: 지금까지의 결정 8건 작성 — NestJS(0001), Prisma(0002), PostgreSQL(0003), Redis(0004), BullMQ(0005), SSE(0006), 모노레포(0007), Next.js(0008).
- **원칙**: 대안이 있으면 비교표로, 없으면 근거만. 대체된 결정은 지우지 않고 `Superseded`로 표시해 사고 흐름을 보존.

## 2026-07-10 · W1 시작 — Prisma 스키마로 ERD 구현

- **셋업**: `apps/api`를 독립 패키지로 생성(ADR 0007 "초기엔 도구 없이 폴더 분리" 방침). `package.json`(prisma 스크립트), `.env`/`.env.example`(DATABASE_URL은 docker-compose 계정과 일치).
- **스키마**: `apps/api/prisma/schema.prisma`에 PRD의 ERD 구현 — User/Event/Inventory/Reservation/Payment + enum 4종(Role, EventStatus, ReservationStatus, PaymentStatus).
- **핵심 설계 결정(→ ADR 0009)**:
  - PK는 `Int autoincrement` — 인덱스 지역성·`EXPLAIN` 학습 우선. 순차 ID 노출 방어는 향후 과제.
  - **재고를 `Inventory`로 분리** — W2에서 재고 행만 잠가 락 경합 범위를 좁히기 위함(이 스키마의 핵심).
  - 상태값은 Postgres enum(DB 레벨 무결성), 금액은 `Int`(원 단위, Float 금지).
  - 조회 패턴 기반 인덱스 + `version`(낙관적 락), `idempotencyKey`(멱등성) 컬럼을 W2/W3용으로 미리 심음.
- **삽질**: 샌드박스에서 `prisma validate` 실행 실패 — 엔진 바이너리 다운로드가 네트워크 정책상 403. 스키마는 수동 리뷰로 검증. **실제 검증/마이그레이션은 로컬에서** `docker compose up -d` 후 `npx prisma migrate dev` 로 수행할 것.
- **다음**: 로컬에서 첫 마이그레이션(`migrate dev`) → NestJS 스캐폴딩 → PrismaModule 연결 → JWT 인증(argon2) + 이벤트 CRUD.

### 배운 점 / 메모
- 스키마 설계는 "지금 동작"이 아니라 "뒤에서 실험할 거리(락·큐·멱등)를 미리 심는" 작업이라는 관점 — 재고 분리/version/멱등키가 전부 W2~W3 학습과 1:1로 대응.
- 돈은 절대 Float 금지. 보조단위 없는 KRW는 정수 원으로.

## 2026-07-10 · W1 — NestJS 뼈대 얹기 (apps/api)

- **상황**: 위 Prisma 스키마 단계가 다른 세션에서 완료돼 있었다. 이 세션에서 이어받아 **기존 스키마는 보존**하고 그 위에 NestJS 레이어만 추가.
- **추가한 것**: `package.json`에 Nest 의존성/스크립트(`build`/`start:dev` 등), `tsconfig.json`·`nest-cli.json`, `src/main.ts`(PORT 3001), `src/app.module.ts`(ConfigModule 전역 + PrismaModule), `src/app.controller.ts`(GET `/health`), `src/prisma/{prisma.module,prisma.service}.ts` — `@Global` PrismaService로 어디서든 주입.
- **검증**: 샌드박스 임시 폴더에서 `npm install` + `nest build` 통과(dist 생성 확인). `prisma generate` 엔진 바이너리 다운로드만 샌드박스 네트워크(403)로 실패 — 로컬 맥에선 정상.
- **회고**: 이전에 `git add -A`가 다른 세션 파일을 엉뚱한 커밋에 혼입시킴 → 이후 커밋은 **경로 명시**.
- **다음(로컬에서)**: `cd apps/api && npm install && npx prisma migrate dev` → `npm run start:dev` 후 `curl localhost:3001/health` 확인 → 인증(JWT, argon2) + 이벤트 CRUD.

## 2026-07-10 · 인프라 결정 — 두 기기 공유 위해 Neon(클라우드 DB) 채택

- **요구**: 회사/집 두 노트북에서 데이터까지 동일하게 공유. 코드·DB구조는 git으로 이미 공유되지만 데이터(행)는 로컬 DB에 갇힘.
- **결정(ADR 0010)**: 개발 DB는 **Neon 서버리스 Postgres**(무료, 유휴 시 0원)에 두고 두 기기가 같은 `DATABASE_URL`로 접속 → 데이터 자동 공유. **Redis는 로컬**(휘발성이라 공유 불필요). 비밀값은 커밋 금지·기기별 주입. 초기 데이터는 시드 스크립트로 재현.
- **비교**: 로컬 Docker만(데이터 공유 X), Supabase(7일 미사용 시 정지), Railway(상시 무료 아님) → Neon 채택.
- **주의**: W2 대량 부하테스트는 무료 한도에 걸리므로 그 구간만 로컬 Postgres로 분리 예정.
- **반영**: `apps/api/.env.example`에 Neon(dev)/로컬(W2) 두 형식 주석, CLAUDE.md §9·PROJECT_INSTRUCTIONS에 "기기 간 재현성" 원칙 추가.
- **다음**: Neon 프로젝트 생성 → 연결 문자열을 `apps/api/.env`에 주입 → `npx prisma migrate dev --name init`.

## 2026-07-14 · 비밀값 관리 — Infisical 채택 + Neon 프로젝트 생성 완료

- **Neon**: `sunchak` 프로젝트 생성 완료(싱가포르 리전, Postgres 18, production 브랜치). 유휴 시 scale-to-zero.
- **결정(ADR 0011)**: 비밀값은 **Infisical**(무료·오픈소스)에 저장, `infisical run -- <명령>`으로 런타임 주입. 평문 `.env`를 기기 간에 나르지 않음.
- **비교**: 로컬 .env 수동공유(유출 위험), Doppler(클로즈드), 1Password(유료) → Infisical 채택.
- **반영**: `.env.example` 상단에 "값은 Infisical, 이 파일은 키 목록" 명시. CLAUDE.md §9·PROJECT_INSTRUCTIONS 갱신.

## 2026-07-14 · 기기 간 세션 싱크 — `STATUS.md` 도입

- **문제**: Claude Code 데스크톱 앱이 기기(회사/집) 간 세션(대화)을 공유하지 않음이 확인됨. 코드는 git으로 공유되지만 "어디까지 했는지"의 맥락이 끊김.
- **결정**: 세션 공유는 포기하고, **`docs/STATUS.md`**(현재 상태·다음 할 일의 항상 최신인 스냅샷 1장)로 싱크. 계속 쌓지 않고 **덮어써서** 유지. 세션 시작 시 먼저 읽고, 끝/커밋 전 최신화.
- **역할 분리**: `STATUS.md`(현재 스냅샷) / `DEVLOG.md`(시간순 이력·삽질) / `decisions/`(결정 근거).
- **반영**: CLAUDE.md §9, PROJECT_INSTRUCTIONS "기기 간 재현성"·"새 세션에서 이어가기" 갱신.
- **다음**: Neon 연결 문자열 확보 → Infisical 가입/프로젝트 생성 → CLI 설치·로그인 → `infisical run -- npx prisma migrate dev --name init`로 첫 마이그레이션.

## 2026-07-14 · 패키지 매니저 pnpm 채택 (ADR 0012)

- **결정**: 모노레포·재현성 위해 **pnpm** + corepack 버전 고정. npm/yarn 대비 빠르고 디스크 절약, 엄격한 의존성.
- **전환**: `apps/api`에서 `corepack use pnpm@latest`로 버전 고정, `package-lock.json` 제거 → `pnpm-lock.yaml` 생성.
- **명령 변경**: `npm install`→`pnpm install`, `npx prisma ...`→`pnpm exec prisma ...`. 마이그레이션은 `infisical run --env=dev -- pnpm exec prisma migrate dev --name init`.
- **비고**: Infisical CLI는 brew의 Command Line Tools 구버전 이슈로 `npm i -g @infisical/cli`(npm 전역)로 설치. `infisical init`으로 apps/api 연결(.infisical.json — 비밀값 없음, 커밋 가능).
- **다음**: pnpm 전환 후 첫 마이그레이션 실행 → 생성된 migration.sql 리뷰.

## 2026-07-14 · W1 — 인증(회원가입/로그인/보호가드) + 이벤트 CRUD

- **첫 마이그레이션 성공**: `infisical run --env=dev -- pnpm exec prisma migrate dev --name init` → Neon에 테이블 5 + enum/인덱스/FK 생성. 서버 기동 + `/health` 200 확인.
- **인증 구현 (→ ADR 0013)**:
  - `POST /auth/signup` — DTO(class-validator) 검증 + argon2 해싱 + 유저 생성, 비번 해시 응답 제외. 전역 ValidationPipe.
  - `POST /auth/login` — `argon2.verify`(재해싱 비교) 후 JWT 발급. 실패는 401 동일 메시지.
  - 보호 가드 — passport-jwt Strategy + `JwtAuthGuard` + `@CurrentUser`. `GET /auth/me`.
- **이벤트 CRUD**: `GET /events`(공개 목록)·`GET /events/:id`(공개 상세, 404)·`POST /events`(관리자만). `RolesGuard`+`@Roles(Role.ADMIN)`, 두 가드 순서(JWT→Roles), Prisma 중첩 생성으로 Event+Inventory 동시 생성, `ParseIntPipe`.
- **삽질/메모**:
  - pnpm11은 build script 기본 차단 → `pnpm-workspace.yaml`의 `allowBuilds`로 prisma·argon2 허용(package.json `pnpm` 필드는 pnpm11에서 무시됨).
  - `@nestjs/jwt`의 `expiresIn`이 ms의 엄격한 타입 요구 → `config.get`(제네릭 없이)로 회피.
  - JWT payload는 암호화가 아니라 인코딩(누구나 디코딩) → 민감정보 금지. role 변경 시 **재로그인** 필요(토큰에 role이 스냅샷됨).
- **학습 규칙 강화**: "코드 자체(각 줄·문법)도 설명" 규칙을 CLAUDE.md/지침에 추가.
- **다음(W1 마무리 → W2)**: auth/events 단위 테스트(Jest) → 동시성 실험(순진한 구현 → 초과판매 재현 → 락 3종+Redis) + k6.

## 2026-07-14 · W1 마무리 — Jest 단위 테스트 셋업 + auth/events 테스트

- **테스트 대상 선정 원칙(학습)**: "분기(`if`/`throw`) 수 ≈ 테스트 수". 우리가 쓴 판단 로직만 테스트하고, DB(Prisma)·argon2·JWT 같은 남의 코드는 mock으로 잘라낸다.
- **작성한 테스트(8개, 전부 통과)**:
  - `auth.service.spec.ts` — login(없는 이메일 401 / 비번 틀림 401 / 성공 시 accessToken+payload), signup(중복 409 / 응답에 password 미포함).
  - `events.service.spec.ts` — findOne(없으면 404 / 있으면 반환), create(재고 `remainingQty === totalQty` 초기화 검증, `expect.objectContaining`으로 관심 필드만 좁게).
- **mock 두 방식 정리(학습)**: 코드가 의존성을 "어떻게 손에 넣느냐"가 방식을 결정. 생성자 주입 → NestJS DI에 `useValue`로 교체(인스턴스 교체), 직접 `import` → `jest.mock`으로 모듈 로더 가로채기(모듈 교체). Jest가 sandbox의 모듈 로더를 소유하기에 `jest.mock`이 가능(hoisting으로 import보다 먼저 등록).
- **삽질 3종**:
  1. `pnpm install`이 Node v22.13+ 요구인데 v22.12.0 → nvm으로 v22.23.1 설치, `nvm alias default`, 루트에 `.nvmrc`(22.23.1) 추가해 두 기기 고정.
  2. `pnpm test`가 테스트 전 의존성 검사에서 실패 — `pnpm-workspace.yaml`의 `unrs-resolver`가 placeholder 문자열이라 boolean 요구를 위반. `false`로 명시(네이티브 가속기, 테스트 불필요).
  3. `@nestjs/testing` 미설치로 `Test.createTestingModule` import 실패 → devDep 추가.
- **추가 설치**: `jest`·`ts-jest`·`@types/jest`·`@nestjs/testing`(devDep), package.json에 jest 설정(`rootDir: src`, `testRegex: *.spec.ts`, `transform: ts-jest`, `testEnvironment: node`) + `test`/`test:watch`/`test:cov` 스크립트.
- **다음(W2)**: 동시성 실험 — 순진한 예매 구현 → 초과판매(oversell) 재현 → 락 3종 + Redis 비교 + k6 부하테스트.

## 2026-07-16 · W2 시작 — 순진한 예매 구현 + 초과판매(oversell) 재현

- **로컬 실험 환경 분리(§9)**: W2 부하는 Neon 무료 한도를 깎으므로 로컬 Postgres로 분리. `infra/docker-compose.yml`의 PG(계정 `sunchak/sunchak`, :5432) 기동 → `apps/api/.env`의 `DATABASE_URL`만 로컬로 교체(기존 Neon URL은 주석 보존) → `prisma migrate deploy`로 스키마 반영. 서버는 infisical 없이 `pnpm start:dev`(로컬 .env 자동 로드).
- **순진한 예매 API**: `reservations` 모듈 신규 — `POST /events/:eventId/reservations`(JWT 필요). 서비스 로직은 일부러 방어 없이 ①재고 읽기 → ②`remainingQty >= quantity` 확인 → ③`remainingQty = 읽은값 - quantity`로 **절대값 덮어쓰기** → ④예매 기록(status 기본 HELD). 단일 요청 검증: 재고 5→4 정상 차감 확인.
- **초과판매 재현**: 재고 1개 이벤트에 동시 요청 30개 발사. race window를 결정적으로 벌리려 ②와 ③ 사이에 `await setTimeout(50ms)` 삽입(학습용 확대경, 실코드 아님).
  - **결과**: 30개 전부 HTTP 201, **예매 30건 / 재고 1 → 초과판매 29건.**
  - **핵심 관찰**: 재고가 `-29`가 아니라 **`0`**. ③을 절대값 덮어쓰기로 했기에 30번의 차감이 서로를 덮어써 사라짐(**lost update**) → 카운터가 "정상"처럼 0을 가리켜 오히려 버그가 안 보임. (원자연산 `{decrement:1}`이었다면 `-29`로 티는 났을 것 — 락 비교 때 재활용 예정.)
- **개념 정리(사용자 학습)**: 버그의 근본은 `await`(지연)이 아니라 **①읽기·③쓰기가 원자적이지 않음**(별개의 두 DB 왕복 = "틈"). `setTimeout`은 틈을 *만든* 게 아니라 *넓힌* 것 — 없어도 버그는 존재하며(DB 지연·폭주 트래픽이 틈을 채움) 다만 확률적(flaky)이라 더 위험. 필요조건=내 읽기~쓰기 틈(방), 방아쇠=**다른 요청의 읽기가 그 틈에 입장**. → 락 = 그 방의 문을 잠가 남의 읽기를 못 들어오게 하는 것.
- **다음**: 락 3종(비관/낙관/DB 원자연산) + Redis로 이 버그를 하나씩 막고 before/after 비교(§8). 첫 타자는 비관적 락(`SELECT … FOR UPDATE` + 트랜잭션).

## 2026-07-16 · W2 — 비관적 락(pessimistic lock)으로 초과판매 차단

- **구현**: `create()`를 `prisma.$transaction(async (tx) => …)` + `tx.$queryRaw\`… FOR UPDATE\``로 교체. 재고 행을 잠그고 읽어 커밋 전까지 다른 예매(쓰기)를 대기시킴. ①~④를 전부 `tx`로 묶어 락 안에서 처리. 트랜잭션 옵션 `{maxWait:20000,timeout:20000}`로 직렬 대기 여유 확보.
- **검증(동일 조건: 재고 1 · 동시 30)**: 순진한 버전 30건 성공/29 초과판매 → **비관적 락 1건 성공·29건 409·초과판매 0.** race window 확대용 `setTimeout(50ms)`를 남겨둔 채로도 방어 성공.
- **개념(사용자 학습)**:
  - `FOR UPDATE` = "곧 UPDATE할 행을 지금 잠근다"(배타적 행 락). 평범한 SELECT는 MVCC로 안 막힘.
  - `tx` = 화살표 콜백의 매개변수이자 "이 트랜잭션 전용 Prisma 클라이언트". `tx.*`로 호출해야 락 안에서 실행. `this.prisma.*`로 하면 트랜잭션 밖(락 무의미).
  - MVCC: "읽는 자는 쓰는 자를 막지 않고, 쓰는 자는 읽는 자를 막지 않는다." 서로 막는 건 쓰기 vs 쓰기.
  - **트레이드오프**: 비관적 락은 "같은 행"을 다투는 요청만 직렬화 → 정확하지만 핫 로우에선 처리량↓. (선착순이 정확히 그 상황.) → 낙관적 락/원자연산과 처리량 비교가 다음 과제.
- **다음**: 낙관적 락(`version` 조건부 UPDATE + 재시도) → DB 원자연산 → Redis + k6 부하 비교(§8).

## 2026-07-16 · W2 — 낙관적 락(optimistic lock)으로 초과판매 차단

- **구현**: `create()`를 재시도 루프(상한 5)로 교체. ① 락 없이 `findUnique`로 `version`+재고 읽기 → ② 매진 확인 → ③ `updateMany({ where:{id, version}, data:{ remainingQty: 읽은값-quantity, version:{increment:1} } })` → `count===0`(그 사이 version 변경=충돌)이면 `continue`로 재시도, `count===1`이면 ④ 예매 기록. `update` 대신 `updateMany`를 쓴 이유: 결과 `{count}`로 조건부 성공/실패를 알 수 있고 unique 아닌 where(version 가드) 허용.
- **검증(동일 조건: 재고 1·동시 30)**: **1건 성공·29건 409·초과판매 0** — 비관적 락과 같은 정확성을, 락을 전혀 잡지 않고 달성. 29건은 version 충돌로 1회 재시도 후 재고 0을 읽어 매진 처리(재시도 상한 미도달).
- **개념(사용자 학습)**:
  - version 가드 UPDATE = compare-and-swap. "내가 읽은 version 그대로일 때만" 적용되므로, 그 조건이 참이면 값이 안 바뀐 게 보장 → 절대값 덮어쓰기도 안전.
  - **비관 vs 낙관 = 비용의 위치**: 비관=대기 비용(핫 로우 직렬화), 낙관=재시도 비용(경합 심하면 thrashing). 경합 드물면 낙관 유리, 극심하면 비관 안정적. 선착순은 순간 경합 극심 → k6로 수치 비교 예정.
- **삽질**: 세션이 1시간을 넘겨 JWT(1h) 만료 → 이벤트 생성이 404로 실패. 재로그인으로 토큰 갱신 후 정상. (토큰에 만료가 박혀 있으니 장시간 테스트 땐 재발급 필요.)
- **다음**: DB 원자연산(`remainingQty >= quantity` 조건부 단일 UPDATE `{decrement}`) → Redis → k6 부하로 세 방식 처리량 before/after(§8).

## 2026-07-16 · W2 — DB 원자연산(atomic conditional update)으로 초과판매 차단

- **구현**: `create()`를 단일 문장으로 축소 — `updateMany({ where:{ eventId, remainingQty:{ gte:quantity } }, data:{ remainingQty:{ decrement:quantity } } })`. 앱에서 읽지 않으므로 read-check-write 틈 자체가 없음. `count===0`이면 매진(409). 트랜잭션·version·재시도·재현용 지연 전부 제거(틈이 없어 지연도 무의미). `NotFoundException` 고아 import 정리.
- **검증**: 재고 1·동시 30 → **1건**만 성공, 재고 5·동시 30 → **정확히 5건**만 성공(둘 다 초과판매 0, 최종 재고 0). 지연 없이도 완벽 — DB의 단일 문장 원자성만으로 방어됨을 실증.
- **트레이드오프 메모**: 순수 1문장이라 "재고 부족"과 "이벤트 없음"을 count===0으로 구분 못 함(구분하려면 추가 조회). 학습 단계에선 단순함 우선으로 둘 다 409 처리.
- **락 3종 정리(개념)**: 비관=미리 잠금(대기 비용), 낙관=version CAS+재시도(재시도 비용), 원자연산=단일 조건부 UPDATE(가장 단순, 앱측 틈 없음). 셋 다 초과판매 0. 차이는 **처리량** → k6로 수치화가 남음.
- **다음**: k6 부하테스트로 순진한/비관/낙관/원자연산 처리량·지연 before/after 비교(§8). 이를 위해 4개 전략을 런타임에 선택 가능하게 할지 결정 필요.

## 2026-07-16 · W2 — 4전략 런타임 선택 리팩터(k6 준비) + 순진한 버전 오버셀 재확인

- **결정**: k6로 4방식을 비교하려면 런타임 전환이 필요 → `?strategy=naive|pessimistic|optimistic|atomic` 쿼리파라미터 방식 채택(서버 재시작 없이 전환, 벤치 반복 빠름). 대안(환경변수=재시작 필요, git checkout=수작업)은 반복이 번거로워 탈락.
- **리팩터**: `ReservationsService.create()`를 진입점으로 두고 `switch(strategy)`로 `createNaive/Pessimistic/Optimistic/Atomic` 4개 private 메서드에 분기. 컨트롤러는 `@Query('strategy')`로 받아 전달(생략 시 기본 atomic). `ReservationStrategy` 타입 export. 알 수 없는 값은 `BadRequestException`.
- **벤치마크용 지연 제거**: 재현용 `setTimeout(50ms)`를 전 전략에서 삭제(있으면 처리량 측정이 그 지연에 묶여 왜곡). 초과판매 재현은 이미 앞 단계에서 증명 완료.
- **검증(재고 1·동시 20, 전략별)**: naive **4건 판매(초과판매 3)**, pessimistic/optimistic/atomic 각 **1건**. → **순진한 버전이 인위적 지연 없이도 로컬 PG 실제 동시성만으로 오버셀**함을 재확인(“지연은 창을 넓혔을 뿐, 버그는 원래 존재”를 실증).
- **다음**: k6 설치·스크립트 작성 → 4전략 동일 부하로 RPS·p95·오버셀 여부 계측 → `docs/`에 before/after(§8).

## 2026-07-16 · W2 — k6 부하테스트로 4전략 처리량 계측(§8)

- **셋업**: k6 v2.1.0 설치(brew). 스크립트 2개를 레포에 포함(재현성§9) — `apps/api/test/load/reservations_load.js`(부하), `bench.sh`(전략별 이벤트 생성→k6→오버셀 확인→요약표).
- **프리셋**: VU 30 · 15초 · 재고 20만(매진 방지, 순수 차감 처리속도 측정) · 단일 재고 행에 전 VU 경합 · 로컬 PG.
- **결과(RPS / 성공 / 실패 / 실제차감 / p95)**:
  - naive: 2460 / 36,918 / 0 / **1,498** / 31.5 → **35,420건 lost update**(빠르지만 완전히 틀림)
  - pessimistic: 1206 / 18,120 / 0 / 18,120 / 60.5 → 정확하나 락 대기로 느림
  - optimistic: 885 / 4,783 / **8,503** / 4,783 / 60.2 → 재고 남았는데 **재시도 소진 8,503 실패**(thrashing)
  - atomic: 2030 / 30,470 / 0 / 30,470 / 41.3 → **거의 최고 속도 + 완벽 정확(승자)**
- **결론**: hot row 고경합(=선착순)에선 **atomic > pessimistic > optimistic**. naive는 속도만 빠르고 정확성 상실. 낙관적 락은 단일 hot row 고경합에서 재시도 낭비로 부적합 — 앞서 개념으로 예측한 트레이드오프가 수치로 확인됨. → 문서: `docs/perf/2026-07-16-w2-lock-comparison.md`.
- **다음**: Redis 원자 차감(`DECR`/Lua) 추가 후 동일 프리셋 비교 → DB 원자연산과의 처리량 차이 확인.

## 2026-07-16 · W2 — Redis 인메모리 원자 차감(5번째 전략) + 5전략 k6 비교

- **개념 정리**: Redis는 RAM 기반 key-value 저장소. **단일 스레드라 명령 하나가 원자적**(명령 "사이"엔 다른 요청을 받지만 명령 "도중"엔 안 끼어듦) → `DECRBY`가 그 자체로 lost update가 없다. DB atomic(조건부 단일 UPDATE)과 같은 "읽기+쓰기를 쪼갤 수 없게 묶기"의 다른 구현. 메모리 계층(디스크<RAM<L3~L1 캐시<레지스터)에서 RAM에 사는 게 속도 이점의 근거.
- **구현**: `RedisService`(ioredis 5.11, PrismaService와 같은 `OnModuleInit/Destroy` 생명주기, `@Global` RedisModule) 추가. `createRedis`: `DECRBY stock:event:<id>` 반환값이 **음수면 `INCRBY`로 보상 후 409**, 아니면 `reservation.create`만 DB에 기록. `DECRBY`엔 재고 조건이 없어 0 밑으로 깎이므로 **반환값으로 매진 판정 + 넘친 만큼 되돌리기**가 핵심.
- **스모크(재고 2·3요청)**: 201·201·409, Redis 재고 정확히 0 안착(음수 안 샘), DB 예매행 2건.
- **결과(RPS / 성공 / 실제차감 / p95, VU30·15s·재고20만)**:
  - naive 2338 / 35,091 / **1,441** / 33.6 → lost update 33,650(틀림)
  - pessimistic 1126 / 16,912 / 16,912 / 67.9 ✅
  - optimistic 859 / 4,640 / 4,640 / 61.9 ⚠️(재시도 8,262 실패)
  - atomic 2024 / 30,374 / 30,374 / 41.8 ✅
  - **redis 9354 / 140,329 / 140,329 / 4.4 ✅ — atomic의 4.6배, p95 최저, 완벽 정확**
- **왜 redis가 atomic보다 4.6배?**(핵심 교훈): 둘 다 단일 카운터에서 직렬화되지만 ①**직렬 구간 1건당 비용**이 다름 — Postgres hot row UPDATE엔 행 락·MVCC·WAL이 붙고 Redis `DECRBY`는 RAM 정수 감산뿐. ②경합하는 차감을 DB에서 들어내니 **DB엔 경합 없는 병렬 INSERT만** 남음. 병목은 "DB가 느려서"가 아니라 **단일 재고 행 쓰기의 직렬화**였음.
- **비용(정합성)**: 재고 진짜 값이 Redis에만 있어 DB `inventories`와 어긋남 + Redis 유실 시 재구성 필요. 실서비스는 Redis로 선착순 관문만 통과시키고 DB 반영은 큐로 뒤에서 맞춤(→ BullMQ 복선).
- **삽질**: zsh에서 명령을 변수에 담아 `$R ping` 호출 시 "command not found"(통째로 명령명 취급) → 풀어서 실행. 옛 dev 서버가 ioredis 설치 전 상태로 떠 있어 kill 후 재기동(RedisModule 연결 반영).
- **다음**: 재고 정합성(Redis 관문 + 큐 반영) 설계는 W3~. 최종 예매 전략은 ADR로 확정. 문서: `docs/perf/2026-07-16-w2-lock-comparison.md`.

## 2026-07-18 · W2 마무리 — 개념 재확인 + 최종 예매 전략 ADR 0014 확정

- **개념 딥다이브(사용자 자기설명으로 검증)**: 왜 Redis가 atomic의 4.6배인가를 밑바닥까지 재정리 — ①선착순 재고는 구조적으로 단일 hot 숫자라 병렬화로 못 피함(재고 행을 흩어도 이벤트 재고는 하나) → ②DB든 Redis든 직렬화되지만 **직렬 1건당 비용**이 갈림. DB 비용의 정체: **행 락**(Isolation), **MVCC 새 버전 생성**(읽기 안 막고 쓰기 = 옛 버전 조회), **WAL fsync**(Durability, 물리 디스크 동기 왕복 = 최종 병목). ③Redis는 이 보장들을 (일부) 포기 — 락·MVCC 없음 + fsync를 요청 경로 밖(AOF everysec=초당 1회 비동기)으로 빼서 요청이 디스크를 안 기다림. **"인메모리라 빠르다"만이 아니라 "동기 지속성을 비동기로 미뤄서 빠르다"**가 정확. 대가가 곧 정합성·유실.
- **오해 교정 2건**: (1) "커넥션 오버헤드"는 풀 재사용이라 주범 아님. (2) "재고 20만으로 흩어져도 똑같이 느림"은 틀림 — 행 락이 사라지면 **병렬화**되어 처리량 폭증(계산대 1개 vs 20만 개). DB가 느린 진짜 이유는 건당 비용이 아니라 **hot row 직렬화로 병렬화가 원천 차단**된 것.
- **ADR 0014 작성**: `docs/decisions/0014-reservation-strategy.md` — **Redis 인메모리 관문(`DECRBY`+보상) + DB 비동기 기록** 채택. 5전략 비교표를 "채택하지 않은 이유"로, 위 논리 사슬을 근거(Rationale)로, 대가(정합성·유실·경계검사·중복방지)를 결과(Consequences)로. DB 단독 시 차선은 atomic(폴백). README 인덱스 갱신.
- **다음**: 정합성 설계(W3~) — Redis 관문 + BullMQ 비동기 반영, Redis↔DB 정합성·유실 재구성·멱등(중복 방지). 후속 ADR로 확정.

## 2026-07-19 · W3 정합성 설계 개념 완주 + ADR 0015 확정

- **개념 설계(사용자 자기설명으로 검증)**: ADR 0014의 Redis 관문 뒤를 어떻게 신뢰성 있게 처리할지 세 문제(①Redis↔DB 어긋남 ②Redis 유실 재구성 ③멱등성)를 파이프라인으로 풀었다. 관문(Redis DECRBY) → **HELD 선기록(DB INSERT)** → 큐(BullMQ) → 즉시 응답 → 워커가 HELD→CONFIRMED UPDATE → SSE push. + HELD TTL 만료 + Redis 재구성(`총재고 − (HELD+CONFIRMED)`).
- **핵심 개념 4개**: (1) **큐/워커** — 작업을 Redis에 영속 저장(프로세스 밖·재시작 견딤), 생산자(API)/소비자(워커) 분리. (2) **at-least-once → 멱등성** — "작업 성공했는데 응답 유실"로 재시도 시 중복 INSERT. 막는 법 = **멱등성 키 + unique 제약**(DB가 원자적으로 거부, 워커는 P2002를 "이미 됨=성공"으로 삼킴). read-check-write 함정을 DB 한 방으로 = W2 atomic과 같은 철학. (3) **확정 통보** — 폴링(요청 폭탄) 대신 **SSE**(0006). (4) **Redis 유실 재구성** — Redis 재고는 진실 아닌 DB 파생 사본. 단 HELD가 Redis/큐에만 있으면 재구성 부정확 → **최초 시점부터 DB에 HELD 선기록**해야 정확 복구.
- **정밀화(헷갈리기 쉬운 자리)**: ① 재고 차감은 hot row라 Redis로 뺐지만 ② HELD INSERT는 서로 다른 행이라 **경합 없는 병렬** — "직렬화 병목 ≠ INSERT 비용". 세 단계의 '원자성'은 각기 다른 문제(초과판매/중복/확정)를 푼다. unique는 DB가 자동 거부하되 그 에러를 성공으로 해석하는 건 워커 코드의 몫.
- **문서**: `docs/decisions/0015-reservation-consistency-design.md`(ADR 0014의 정합성 편), README 인덱스 갱신.
- **다음**: 구현 착수 — 스키마 변경(reservation.status HELD/CONFIRMED/EXPIRED + 멱등성 키 unique + heldAt) → 마이그레이션 → BullMQ 큐/워커 + SSE + HELD 선기록/확정/TTL/재구성.

## 2026-07-20 · W3 구현 착수 — ADR 0015 멱등성 부분 개정 + 스키마 변경

- **구현 착수 직전 검토에서 ADR 0015의 오류 2건 발견**(설계를 코드로 옮기려니 앞뒤가 안 맞아 드러남):
  1. **"워커 재시도의 중복 INSERT를 unique가 막는다"가 성립 안 함.** 이 파이프라인에서 INSERT는 요청 경로(②)에서 한 번만 일어나고 워커(⑤)는 UPDATE만 한다. job이 재시도돼도 "두 번째 INSERT"는 발생하지 않는다 → unique가 막을 대상이 없었다. 워커 UPDATE는 `WHERE status=HELD`라 재실행 시 0건, **본래 멱등**이라 별도 장치가 애초에 불필요.
  2. **"멱등성 키는 API 서버가 발급"이 틀림.** 서버는 "재전송"과 "진짜 두 번째 주문"을 요청 내용만으로 구분할 수 없다(userId·eventId·quantity가 전부 동일). 판정 근거는 재전송을 실행한 **클라이언트만** 가진다. 서버가 발급하면 매 요청 새 키가 나와 "같음"을 정의할 수 없음 → 그건 멱등성 키가 아니라 PK. (Stripe `Idempotency-Key` 헤더가 클라이언트 발급인 이유.)
- **원인 추정**: "워커가 INSERT하는" 일반적 멱등성 키 패턴을 그대로 옮겨 적었는데, ADR 0015가 고른 설계는 INSERT를 요청 경로로 **당겨온**(HELD 선기록) 구조라 전제가 달랐다. **패턴을 아는 것 ≠ 이 설계에 맞는지 판단한 것.**
- **새로 드러난 구멍(기존 ADR에 아예 없던 것)**: 관문(①)이 INSERT(②)보다 먼저라 **재전송도 `DECRBY`를 한 번 더 깎고 나서야** unique에 걸린다 → 그대로 두면 **주문 없이 재고만 증발**. 특히 고약한 건 ①조용함(DB는 정상, Redis 카운터만 틀림 → DB 검증으로 안 잡힘) ②네트워크 불안할 때 몰려서 누적 ③끝은 **거짓 품절**(자리 남았는데 관문이 막음). → **unique 위반 시 `INCRBY` 보상**(Redis 전략의 음수 보상과 같은 동작).
- **보상 vs 재구성은 대체 관계가 아님**: 증발분은 Redis 재구성 잡(`총재고−(HELD+CONFIRMED)`)이 언젠가 고치지만 그건 **주기적 사후 안전망**이고, 그 사이 거짓 품절이 방치된다. 선착순은 바로 그 몇 분이 승부처 → **보상=1차 방어, 재구성=안전망** 둘 다 필요.
- **재전송의 응답은 409가 아니라 성공**: 그 사용자는 실패한 게 아니라 이미 성공했고 응답만 유실됐다. 첫 요청과 **동일한 결과(같은 예매 ID)** 를 반환해야 한다 — "몇 번을 호출하든 결과가 같다"가 멱등성의 정의. 막는 게 아니라 **같은 답을 반복해서 주는** 것.
- **unique 범위는 복합으로**: `@@unique([userId, idempotencyKey])`. "같은 요청"의 정의가 **같은 사람 + 같은 이름표**이므로 정의를 스키마에 그대로 옮긴 것. 단독 unique(`idempotencyKey`만)는 키를 전 세계 유일 신원으로 취급해 정의와 어긋나고, 남의 키와 충돌하는 상황(→ 복구 조회에서 타인 예매 유출)을 만들어낸다. 복합이면 그 충돌 자체가 발생하지 않음. **보안은 부산물이고, 진짜 이유는 의미론.**
  - ⚠️ `Payment.idempotencyKey`는 여전히 단독 unique — 같은 문제를 안고 있을 수 있음. W3 결제 단계에서 재검토할 것.
- **스키마 변경**: `Reservation`에 `idempotencyKey String`(필수) + `@@unique([userId, idempotencyKey])` 추가. **`status`·`heldUntil`·`@@index([status, heldUntil])`은 W1(0009)에 이미 있어 추가 불필요** — STATUS.md의 "status/heldAt 추가" 항목은 착오였다.
- **막힌 곳(다음 세션 첫 작업)**: 마이그레이션 미실행. `reservations`에 W2 벤치가 만든 수만 행이 있어 **기본값 없는 NOT NULL 컬럼 추가가 실패**한다. 선택지 A) `migrate reset`으로 로컬 DB 초기화(추천 — 로컬 한정 쓰레기 데이터, 시드로 재현 가능) B) nullable로 완화(스키마에 구멍) C) 임시 기본값 후 제거(정공법이나 과함). **사용자 승인 대기 중.**
- **진행 방식 반성**: (A)/(B) unique 범위를 멱등성 키의 기본 개념보다 먼저 꺼내 사용자가 길을 잃음. 되돌아가 "응답 유실 → 재시도 → 2장 예매" 구체 시나리오부터 다시 쌓아 해결. **한 번에 여러 새 개념을 쌓지 말 것.**

## 2026-07-21 · W3 2.2 — 마이그레이션 적용 + HELD 선기록/멱등성 보상 구현

- **마이그레이션 적용(막혔던 것 해소)**: `idempotencyKey` NOT NULL 추가가 기존 행과 충돌한다던 STATUS의 걱정은 **이 기기엔 해당 없었다** — 로컬 DB가 방금 docker로 새로 생성돼 비어 있었음(그 "수만 행"은 다른 기기 얘기). A(reset)는 불필요했고 Prisma AI 안전장치가 reset을 막았지만 안 해도 됐다.
- **삽질 3건**: ① 활성 `node` v22.12.0 → pnpm이 v22.13+ 요구하며 거부 → nvm의 v22.23.1로 PATH 전환. ② `.env`가 이 기기엔 없음(gitignore) → 로컬 값으로 재생성. ③ `prisma migrate dev`가 **비대화형(에이전트) 환경에서 거부**(`--create-only`도 unique 경고 확인 프롬프트 때문에 막힘) → **`migrate diff --from-url → migration.sql → migrate deploy → generate`** 로 우회. (사람이 터미널에서 직접 하면 `migrate dev`가 정상.) 적용 SQL은 `ADD COLUMN idempotencyKey NOT NULL` + `CREATE UNIQUE INDEX (userId, idempotencyKey)` 두 줄. diff에 `status`/`heldUntil`/기타 인덱스가 없어 **W1에 이미 있었음이 역으로 검증**됨.
- **NOT NULL 여파 발견**: 마이그레이션 후 tsc가 5곳 전부 에러 — 5전략의 `reservation.create`가 `idempotencyKey` 없이 만들어 필수 필드 누락. 즉 스키마만 바꾸면 기존 코드가 조용히 깨진다는 걸 타입체크가 잡아줌.
- **설계 결정(B안 선택)**: W2 5전략을 벤치 재현용으로 **보존** + held를 6번째로 추가. 5전략엔 서버가 `randomUUID()`로 `idempotencyKey` 자동 발급 — 멱등성이 목적이 아니라 NOT NULL 만족 + (같은 유저 수만 건에서) unique 충돌 회피용. held만 **클라이언트 발급 키**를 그대로 씀.
- **createHeld 구현**: 관문(DECRBY) → status=HELD INSERT → `try` 안 `await` + `Prisma.PrismaClientKnownRequestError`·`code==='P2002'`로 재전송 판별 → `INCRBY` 보상 + `findUniqueOrThrow({ userId_idempotencyKey })`로 첫 예매 반환. DTO는 `idempotencyKey?`(`@IsUUID`), held에서만 서비스가 필수 강제.
- **통합 테스트로 검증(mock 아님)**: 멱등성은 실제 DB unique(P2002)·실제 Redis DECRBY 원자성이 핵심이라 mock은 자작극이 됨 → 로컬 PG/Redis에 붙는 통합 테스트 4종. 정상(HELD·재고 5→4)/재전송(예매 1건·재고 4 = 2번 깎고 1번 보상)/재고부족(409·재고 0 원복·예매 0건)/키누락(400). 전체 12개 그린.
- **사용자 개념 검증(집요하게)**: **DECRBY 음수 체크 = 재고부족(초과판매), P2002 = 재전송(중복)** 을 세 번에 걸쳐 분리시킴. 사용자가 반복해서 둘을 "관문에서 재전송도 체크"로 합쳤으나, 최종적으로 "DECRBY는 재고부족만, 재전송은 흘려보내고 INSERT의 P2002에서 감지 후 보상"으로 정확히 정리. 두 INCRBY의 의미 차이(재고부족=실패 후 원복 / 재전송=이미 성공, 초과 차감분만 원복)도 확인.
- **판단 근거로 남긴 것**: `heldUntil`은 2.2에서 세팅 생략 — 만료 스윕(2.5)이 아직 없어 값만 넣으면 데드 값이라 2.5에서 함께 도입(단순성).
- **커밋**: 마이그레이션 `1c0afd2`, held 구현+테스트 `6c5ed23`.
- **다음**: 2.3 BullMQ 큐/워커 — createHeld가 HELD INSERT 후 job enqueue + 즉시 응답, 워커가 `WHERE status=HELD`로 CONFIRMED UPDATE(재실행 0건 = 본래 멱등).

## 2026-07-21 · W3 2.3 — BullMQ 큐/워커로 HELD→CONFIRMED 확정

- **개념 선(先)정리(사용자 자기설명 검증)** — "왜 큐인가"를 코드 전에 밑바닥까지:
  - **큐의 두 가치**: ①신뢰성(job이 Redis에 영속 → 실패 시 자동 재시도 = at-least-once = 최종 일관성) ②비동기 분리(빠른 응답). 사용자는 처음 ②만 답하고 ①을 빠뜨렸고, "UPDATE가 무거워서(행락+MVCC+WAL)"라고 오답 → **교정**: 그 비용은 hot row **직렬화**일 때 문제고, 확정 UPDATE는 주문마다 다른 행이라 경합 없는 병렬. 2.2에서 HELD INSERT를 이미 요청 경로에 두고 "경미"라 통과시켰으니 "무게가 이유"면 앞뒤가 안 맞음. (W2 hot row 교훈을 안 맞는 데 옮긴 실수 — ADR 0015 때와 같은 유형.)
  - **왜 INSERT는 동기, UPDATE만 큐로?** 핵심은 "**누가 기다리는가**". INSERT는 사용자가 대기 중 → 실패해도 사용자가 재시도 엔진(재전송). UPDATE는 "접수됨" 응답 후 사용자가 떠난 뒤 → 실패해도 재시도할 주체가 없음 → **영속 job이 그 자리를 대신**. 또한 INSERT는 (a)멱등 판정=사용자에게 줄 답이라 미룰 수 없고 (b)큐 job이 가리킬 닻이라 먼저 존재해야 함(그래서 "선기록").
  - **왜 HELD인가(바로 CONFIRMED INSERT 안 하고)?** INSERT 시점엔 관문만 통과·**미결제**. CONFIRMED는 거짓. HELD=유효기간(heldUntil) 붙은 임시 확보, 사이에 결제가 끼고 CONFIRMED(결제 완료)/EXPIRED(시간 내 미결제)로 갈림. **정직한 지적**: 지금 워커는 결제 없이 즉시 뒤집어 두 단계가 과해 보이나, HELD는 결제가 들어올 seam. 결제 없으면 INSERT-as-CONFIRMED가 더 단순한 정답임을 명시.
  - **job엔 id만**: 요청 시점 스냅샷이 아니라 처리 시점 진짜 상태로. DB=진실의 원본, job=포인터. TTL 스윕이 먼저 EXPIRE했거나 재시도 job이 이미 CONFIRM했을 수 있어, 워커는 `WHERE status=HELD`로 현재 상태를 DB에 재확인.
- **구현**: `@nestjs/bullmq`+`bullmq` 설치. `BullModule.forRootAsync`(app.module, REDIS_URL을 host/port로 파싱 — 옵션을 넘기면 BullMQ가 워커 블로킹 연결용 maxRetriesPerRequest:null을 알아서 세팅) + `registerQueue('confirm', defaultJobOptions: attempts3·지수백오프·removeOnComplete)`. `ConfirmProcessor`(WorkerHost)가 `updateMany(WHERE status=HELD → CONFIRMED)` — count===0은 멱등 no-op. `createHeld`가 HELD 커밋 후 `queue.add('confirm',{reservationId})` 하고 즉시 HELD 반환.
- **알려진 틈(코드 주석·STATUS 명시)**: create 커밋과 queue.add 사이 크래시 시 job 없는 HELD 고아 발생(DB·큐 이중 쓰기라 비원자적). Outbox는 과함 → **2.5 TTL 회수 잡이 안전망**으로 정리(EXPIRE)하기로.
- **테스트**: 통합 4→6. 추가 2 = ①held 접수 후 워커가 CONFIRMED로 확정(폴링 대기) ②이미 CONFIRMED에 확정 job 중복 투입해도 1건 유지(워커 멱등). 테스트 모듈에 실 BullMQ 인프라+ConfirmProcessor 등록해 end-to-end(실 DB/Redis). **전체 14개 그린.**
- **삽질 2건**: ①`pnpm exec`가 실행 전 deps 점검을 하는데 `msgpackr-extract`(bullmq 선택적 네이티브 최적화) 빌드 스킵을 정책 위반으로 봐 tsc/jest가 막힘 → `./node_modules/.bin/`으로 직접 호출해 우회. 기능엔 무해(JS 폴백). ②이 기기 로컬 DB(docker 볼륨, 기기별)에 마이그레이션 미적용이라 `idempotencyKey 컬럼 없음` 런타임 에러 + Prisma Client도 stale → `prisma generate` + `prisma migrate deploy`로 해결. (STATUS의 "적용 완료"는 다른 기기 얘기였음.)
- **다음**: 2.4 SSE(확정 실시간 push) → 2.5 안전장치(TTL 회수 + Redis 재구성).

## 2026-07-21 · 공개 데모 모드 설계 (ADR 0016)

- **동기(사용자 질문에서 출발)**: "최종 형태가 포트폴리오로, 누구나 실시간 티케팅/순번을 테스트해볼 수 있나?" → 두 리스크 발견. ① 핵심 가치(초과판매 0·실시간 순번)는 방문자 **혼자 클릭으론 체감 불가** → 데모 장치 필요. ② 공개 URL이 **무료 티어(Neon/VM)** 위라 봇·시뮬 남발이 한도를 태움 → 진입 통제 필요.
- **사용자 결정 2건**: 진입 통제 = **게이트 + 시뮬 상한**(게이트만/전면 rate limit 아님). 리셋 = **자동 주기 + 수동 버튼 둘 다**(자동만/수동만/방문자별 격리 아님).
- **설계 확정(ADR 0016)** 세 축 + 한도 보호:
  - **A. 진입 게이트**: 공유 비번(env) → 서버가 데모 토큰 발급 → API가 검사. **게이트는 반드시 API 계층**(프론트 게이트만이면 봇이 VM API 직격 — 무료 티어 부하는 백엔드에서 나므로 신뢰 경계도 백엔드). **게이트 ≠ 로그인**(진입 차단 vs 신원 식별, 층위 다른 별개 막). 비번은 포트폴리오에 공개.
  - **B. 데모 장치**: ① 서버측 부하 시뮬(가상 유저 N명 → 순번·재고 소진 실시간 재현) ② 실시간 판매 대시보드(SSE stats) ③ 리셋.
  - **C. 리셋**: 자동 주기(스케줄러) + 수동 버튼. seed 미비(STATUS의 알려진 틈)를 여기서 함께 해소.
  - **D. 한도 보호**: 시뮬 상한(최대 VU) + 쿨다운(Redis rate limit, 429).
- **설계상 연결고리**: 데모 대시보드의 stats 스트림 = **W3 2.4 SSE와 같은 메커니즘** → 2.4 SSE 설계 때 "확정 push"뿐 아니라 "stats 스트림"도 염두에 두기로(STATUS 다음 할 일에 명시).
- **구현 시점**: W4(배포와 함께). 이번엔 설계·문서만.
- **반영 파일**: ADR 0016 신규 + decisions/README 인덱스 + 로드맵(01) W4·성공기준 + 기획안(02) §12·화면정의 + `.env.example` 키 4개(`DEMO_GATE_PASSWORD`·`DEMO_SIM_MAX_VU`·`DEMO_SIM_COOLDOWN_MS`·`DEMO_RESET_INTERVAL_MS`) + STATUS.
- **다음**: 예정대로 W3 2.4 SSE(확정 push + stats 스트림 대비).

## 2026-07-22 · W3 2.4 — SSE로 확정 실시간 push

- **개념 선(先)정리(사용자 자기설명 검증, 집요하게)** — 코드 전에 "왜 SSE인가"부터:
  - **문제 체감**: 사용자는 HELD 응답 받고 떠나는데 CONFIRMED는 그 뒤 워커가 비동기로 뒤집음 → 이미 응답받은 클라에 서버가 어떻게 알리나? HTTP는 클라가 물어야 답하는 구조라 서버가 먼저 말 못 검.
  - **폴링/WebSocket/SSE 비교**: 폴링=낭비+타이밍, WebSocket=양방향이라 오버스펙, **SSE=서버→클라 단방향 push + HTTP 그대로 + EventSource 자동 재연결**. 우리 요구(단방향 상태 push)에 정확히 맞음. 사용자 3문항 자기설명 통과.
  - **날카로운 질문 2건(사용자 발)**: ① "왜 워커의 CONFIRMED를 받아야? 결제 완료만 받으면 되지 않나" → **CONFIRMED가 곧 '결제 완료된 확정'**. 지금 워커 즉시 flip은 결제 stub. 실제론 결제 확정이 사용자 요청과 **분리된 경로(PG 웹훅)** 로 와서 서버가 먼저 알려야 함 → SSE 정당. ② "confirm job이 큐에서 웹훅을 기다리나?" → **아니오, 안티패턴**. job은 "지금 할 수 있는 일"만. 실제 설계선 createHeld에서 enqueue 안 하고 **웹훅 도착이 confirm job을 태움**. 워커는 기다린 적 없음. 지금 createHeld enqueue는 "결제 즉시 성공" 흉내며, 결제 붙으면 enqueue 위치만 웹훅으로 옮기면 됨(워커 코드 불변).
  - **핵심 개념(방송국)**: 워커와 SSE 통로는 서로 **참조 없는(loose coupling)** 별개 실행 맥락 → 값 직접 못 넘김 → 프로세스 내 **이벤트 버스**를 경유. RxJS `Subject`=방송국(next 송출+subscribe 수신), `Observable`=시간에 걸쳐 흘러오는 값의 수도관(Promise=값1개 vs Observable=여러개, subscribe≈addEventListener). NestJS `@Sse`는 내가 반환한 Observable을 구독해 emit을 `data:...\n\n` 전선포맷으로 변환·flush·연결관리. **내 책임 경계 = Observable 출구까지**, 그 뒤 전송은 NestJS.
  - **정직한 한계 명시**: 인메모리 Subject라 워커=웹서버 같은 프로세스일 때만 통함. 별도 프로세스 분리 시 Redis pub/sub 필요(운영 관심사, 후순위).
- **구현**:
  - **신규 `reservation-events.service.ts`**: `private Subject<ReservationStatusEvent>` + `publish()`(next) + `ofReservation(id)`(filter로 그 예약만 거른 구독전용 Observable). Subject를 숨기고 구독 스트림만 노출(외부 next 차단).
  - **워커(`confirm.processor.ts`)**: `updateMany` 후 **`count>0`(진짜 이번에 확정)일 때만** `events.publish({id, CONFIRMED})`. count===0(중복/이미확정)은 return — 헛 신호 방지.
  - **서비스(`reservations.service.ts`)**: `assertOwned`(스트림 열기 전 존재·소유권 확인 → 404/403) + `streamStatus`(경합 방지 핵심).
  - **신규 `reservation-stream.controller.ts`**: `@Controller('reservations')` + `@Sse(':reservationId/stream')` async 핸들러 = `await assertOwned` 후 `streamStatus` 반환. URL `/reservations/:id/stream`(create는 events 하위라 경로 달라 별도 컨트롤러).
- **경합(race) 처리 — 이번 설계의 정수**: 파이프라인이 빨라 클라가 SSE 여는 시점엔 워커가 **이미 확정 끝냈을 수** 있음(미래 방송만 기다리면 영영 안 옴). → `merge(future$, current$)`: future$=버스 방송(아직 HELD면 이걸로), current$=`defer`로 **구독 순간 DB 조회**해 이미 종료상태면 즉시 흘림(따라잡기). merge가 future$부터 구독 → 버스 구독이 DB읽기보다 먼저 성립 + 워커는 "DB기록 후 방송" 순서 → 틈에서 확정돼도 current$가 잡거나 future$가 받음(누락 0). `take(1)`로 종료상태 1건 받으면 수도관 close → NestJS가 연결 종료.
- **검증(무거운 e2e 대신 소스로 확정)**: `async @Sse` 핸들러가 안전한지 NestJS 11.1.28 `router-response-controller.js` 확인 — `Promise.resolve(result).then(구독)` + `.catch(reject)`. 정상 경로는 Promise 풀어 구독, 예외(assertOwned throw)는 헤더 전송 전이라 예외필터가 정상 404/403으로 변환. → HTTP e2e 불필요 판단(seed 미비로 수동 e2e 비쌈).
- **테스트**: 통합 6→9. 추가 3 = ①버스 필터링(지정 예약번호 이벤트만 전달) ②이미 CONFIRMED면 구독 즉시 따라잡기(current$) ③HELD 접수 직후 구독 시 워커 확정이 스트림으로 흘러옴(future$/current$). `firstValueFrom`으로 Observable 첫 emit 검증. **전체 14→17 그린.**
- **삽질**: 이 기기에 `@nestjs/bullmq`·`bullmq` 미설치(fresh) → tsc가 그 두 모듈만 에러(내 신규 코드는 무에러) → `pnpm install`로 해소. Docker 데몬 꺼져 있어 `open -a Docker`로 기동 후 infra up.
- **정직한 남은 틈(2.4에서 의도적으로 안 함)**: ① 브라우저 `EventSource`는 `Authorization` 헤더 못 붙임 → 지금 JWT 가드 유지라 실제 프론트 붙일 때(W3 후반) 쿼리파라미터 토큰/쿠키로 해결 필요. ② 하트비트 없음(프록시 유휴 끊김) → 운영 관심사 후순위. ③ ADR 0016 데모 stats 스트림도 같은 @Sse+Observable 메커니즘이라 재사용 가능(이번에 일반 배관을 만들어둠).
- **다음**: 2.5 안전장치(HELD TTL 만료 회수 = 고아 안전망 + `heldUntil` 실제 세팅 + Redis 유실 재구성).

## 2026-08-01 · W3 2.5 — HELD TTL 만료 회수 + Redis 재구성

- **개념 설계(사용자 자기설명으로 검증)**: 두 안전장치를 별개 문제로 다뤘다.
  - **BullMQ job/queue/worker 재정리**: job=주문서(처리할 일 1건의 데이터), queue=주문서가 쌓이는 곳(실제 저장은 Redis), producer=`queue.add`로 넣는 쪽, worker=`process()`로 꺼내 처리하는 쪽. `bullmq`(엔진, Redis와 직접 통신)와 `@nestjs/bullmq`(Nest 데코레이터 어댑터, 리플렉션으로 메타데이터를 읽어 워커 인스턴스화를 대신해줌)를 구분.
  - **TTL 스윕 트리거 — delayed job(예매당 1개) vs 주기적 스윕 비교**: 사용자가 처음 "정확한 타이밍엔 delayed job"이라 판단했으나, **BullMQ의 `delay`는 "그 전엔 안 함"이라는 하한선만 보장하지 "정확히 그때 처리"는 워커 동시성에 달림**을 짚어내면서 뒤집음 — 선착순 특성상 다수 HELD가 동시에 만료되면 delayed job은 오히려 순차 처리로 밀리는 반면, 벌크 UPDATE 1번(주기적 스윕)은 건수와 무관. **TTL은 원래 여유시간이라 몇십 초 오차가 무해**하다는 판단까지 사용자가 스스로 도달 → **주기적 스윕(BullMQ repeatable job)**으로 확정.
  - **원자적 claim — `UPDATE...RETURNING`**: "대상 찾기(SELECT)"와 "상태 바꾸기(UPDATE)"를 분리하면 그 사이 confirm 워커가 같은 행을 채가는 경합이 재발함을 사용자가 직접 도출(ConfirmProcessor의 `WHERE status=HELD` 패턴과 동일 원리 — "판단과 쓰기를 한 문장으로"). Prisma `updateMany`는 `RETURNING`을 지원 안 해 `$queryRaw` 필요. **정정 필요했던 오해**: `RETURNING`이 eventId별로 합산해준다고 생각했으나, 실제론 바뀐 행을 그대로(합산 없이) 돌려줄 뿐 — 합산은 애플리케이션 코드(TS)에서.
  - **DB 먼저, Redis는 나중(순서)**: 반대 순서(Redis 먼저)로 크래시 나면 "이미 팔린 좌석"이 Redis에서 되돌려져 재판매되는데 원래 HELD도 뒤늦게 CONFIRMED될 수 있어 초과판매 재발. DB 먼저면 크래시 나도 뒤늦은 confirm job이 `WHERE status=HELD`에 안 걸려 자동 무시(안전한 방향의 실패).
  - **재구성 잡의 트리거 — "Redis가 죽었을 때만"이 아니다**: 처음엔 "Redis 재시작/flush 시에만 필요"라 생각했으나, TTL 스윕의 "DB 먼저→Redis 나중" 자체도 그 사이 크래시 나면 Redis가 살아있는 채로 어긋날 수 있음을 재발견 — **DB·Redis 두 저장소에 걸친 쓰기(dual-write)는 하나의 트랜잭션으로 못 묶어 이 틈이 구조적으로 항상 존재**. 그래서 재구성도 "서버 기동 시 1회"가 아니라 **상시 주기적** 잡이어야 함.
- **구현**:
  - **`reservations.service.ts`**: `HELD_TTL_MS = 5분` 상수 추가(`MAX_RETRIES`와 같은 자리). `createHeld`가 HELD 예매 생성 시 `heldUntil: now + HELD_TTL_MS` 세팅(2.2/2.3에선 미사용이라 생략했던 부분). 지금은 confirm job이 즉시 확정돼 TTL이 정상 결제 시나리오보단 "HELD 고아"(create 커밋~queue.add 사이 크래시) 회수용 안전망에 가깝다는 것도 확인.
  - **신규 `sweep.processor.ts`**: `SWEEP_QUEUE`(30초 주기). `$queryRaw`로 `UPDATE reservations SET status='EXPIRED' WHERE status='HELD' AND heldUntil<now() RETURNING id, eventId, quantity` 원자적 claim → eventId별 quantity 합산(TS `Map`) → 이벤트별 `INCRBY` 보정.
  - **신규 `reconcile.processor.ts`**: `RECONCILE_QUEUE`(1분 주기). Prisma `groupBy`(HELD+CONFIRMED를 eventId별 quantity 합산, raw SQL 불필요 — RETURNING과 달리 groupBy는 Prisma 기본 지원)로 총재고−합산을 계산해 이벤트별 `SET`(상대 연산 아님 — DB가 진실이라 절대값 덮어쓰기).
  - **두 프로세서 모두 `OnModuleInit`에서 자기 큐에 `{repeat:{every:...}}`로 자기 자신을 등록** — confirm과 달리 외부 producer가 없어 스스로 반복 예약. 같은 repeat 설정이면 재기동해도 BullMQ가 중복 스케줄러를 안 만듦(멱등 등록).
  - `reservations.module.ts`: 큐 2개 추가 등록(`defaultJobOptions: removeOnComplete만` — sweep/reconcile은 이번 틱이 실패해도 다음 틱이 전체를 다시 훑어 스스로 만회하므로 confirm과 달리 attempts/backoff 불필요).
- **테스트**: 신규 파일 2개(`sweep.processor.spec.ts` 3건, `reconcile.processor.spec.ts` 4건) — 둘 다 반복 타이머(30초/1분)를 기다리지 않고 `processor.process()`를 직접 호출해 검증. `afterAll`에서 `queue.obliterate({force:true})`로 `onModuleInit`이 남긴 repeatable 스케줄러까지 정리. **전체 17→24 그린.**
- **삽질 — 통합 테스트 파일이 늘며 드러난 병렬 실행 문제**: 실DB를 쓰는 통합 스펙 파일이 `reservations.service.spec.ts` 하나뿐일 땐 안 드러났는데, `sweep`/`reconcile` 스펙을 추가하니 Jest 기본 병렬 워커가 세 파일을 동시에 돌리면서 서로의 `beforeEach` 전체삭제(`deleteMany`)가 다른 파일이 방금 만든 행을 지워버려 카운트 단언이 들쭉날쭉 실패. **원인은 같은 로컬 DB를 공유하는 여러 통합 테스트 파일 + Jest 병렬 실행의 조합** → `package.json`의 jest 설정에 `maxWorkers: 1` 추가(직렬 실행)로 해결. 프로젝트 성격(로컬 개발용 통합 테스트, CI 규모 아님)상 속도보다 정확성 우선.
- **다음**: W4 공개 데모 모드(ADR 0016) — 진입 게이트 + 서버측 부하 시뮬 + 실시간 stats 대시보드(2.4 SSE 배관 재사용) + 데이터 리셋. `Payment.idempotencyKey` unique 재검토는 결제 단계에서.

## 2026-08-01 · W4 착수 — 데이터 리셋 + seed (ADR 0016 축 C, 부분 개정)

- **범위 조정**: ADR 0016 원안은 "자동 주기 리셋 + 수동 리셋 둘 다"였으나, 구현 착수 시점에 **수동 리셋만**으로 좁힘(자동/주기·활동기반 idle-timeout 모두 보류). 근거는 아래 "개정" 참고. 자동 리셋이 필요해지면(예: 실제 배포 후 방치 시간이 길어 매진 상태가 오래 남는 문제가 실제로 보이면) 그때 다시 추가.
- **설계 중 정정된 판단 2건(사용자 발)**:
  1. **Admin 계정 불필요**: 처음엔 seed가 admin 계정도 만들어야 한다고 제안했으나, 이는 STATUS.md의 옛 메모(개발자 본인의 로컬 수동 e2e 편의)를 데모 요구사항과 혼동한 것이었다. 실제로는 seed가 API가 아니라 Prisma로 DB에 직접 쓰고, 데모 방문자는 로그인이 아니라 게이트 토큰으로 들어오므로(ADR: "게이트 ≠ 로그인") ADMIN role이 쓰일 자리가 없다 → **드롭**.
  2. **리셋 트리거 재검토**: "주기적(고정 간격)" 대신 "활동 기반 idle-timeout"(마지막 활동 후 N분 조용하면 리셋 — BullMQ delayed job을 활동마다 다시 거는 디바운스 패턴)을 검토했으나, 사용자가 최종적으로 **"게이트 진입 후 수동 리셋"** 하나로 확정 — 자동화 자체를 이번 범위에서 제외.
- **데모 이벤트 식별**: `Event.isDemo Boolean @default(false)` 추가(마이그레이션 `20260801121614_add_event_is_demo`). 로컬 개발 중 수동으로 만든 다른 이벤트와 섞이지 않도록 명시적 플래그로 리셋 대상을 특정(느슨한 "이벤트가 1개뿐이라 가정"보다 안전).
- **구현**:
  - **신규 `demo/` 모듈**(ADR: "데모 전용 코드는 격리하라"에 따라 별도 모듈로 분리). `demo.service.ts`의 `resetDemoEvent()`가 seed와 수동 리셋 엔드포인트가 공유하는 핵심 로직 — ①`isDemo:true` 이벤트 조회(없으면 404) ②예매 전체 삭제 ③재고 `remainingQty=totalQty`로 원복(`version` 증가로 낙관적 락 전략과 충돌 방지) ④Redis `stock:event:{id}` SET ⑤`openAt=now, status=ON_SALE`.
  - **`prisma/seed.ts`**: `isDemo:true` 이벤트가 없을 때만 생성(idempotent) — 리셋과 역할 분리(seed=최초 존재 보장, reset=초기 상태로 되돌림). `package.json`에 `prisma.seed` 필드 + `prisma:seed` 스크립트 등록.
  - **`POST /demo/reset`**: 가드 없음(의도적) — 진입 게이트(ADR 축 A, 다음 작업)가 아직 없어서다. 컨트롤러에 "게이트 구현 시 그 가드로 보호 예정" 주석 명시. 배포 전 로컬 개발 단계라 당장 위험 없음.
- **테스트**: 신규 `demo.service.spec.ts` 4건(리셋 정상 동작, 데모 이벤트 없을 때 404, openAt/status 복구, `isDemo=false` 이벤트는 건드리지 않음). **전체 24→28 그린.** 서버 기동 후 `curl -X POST /demo/reset`으로 실제 HTTP 경로까지 수동 검증.
- **다음**: W4 진입 게이트(ADR 축 A) — `DEMO_GATE_PASSWORD` 검증 + 데모 토큰 발급 + 전역 가드. 이후 서버측 시뮬레이션(축 B-1) + stats SSE(축 B-2) 순.

## 2026-08-01 · W4 진입 게이트 (ADR 0016 축 A)

- **방문자 식별 방식 논의가 세 번 바뀜(사용자 발, 기록만 — 오늘 범위엔 미반영)**: "admin 불필요" → "로그인 없이 게스트로 예매" → "Google SSO로 로그인" 순으로 검토되다, **오늘은 게이트만 구현하고 인증 방식(방문자가 어떻게 자기 예매를 남기는가)은 다음 세션으로 분리**하기로 확정. 이유: 게이트("누구든 들어올 수 있냐")와 방문자 식별("이 예매가 누구 것이냐")은 서로 다른 헤더·다른 토큰을 쓰는 독립적인 문제라 오늘 안 섞어도 됨 + 인증 방식은 ADR 0013(기존 인증 설계)과 맞닿은 무게 있는 결정이라 별도 세션에서 제대로 다뤄야 함.
- **개념 정리**: 이 프로젝트의 **첫 전역 가드**(지금까진 전부 라우트별 `@UseGuards`). "게이트 ≠ 로그인"(ADR) — 게이트는 "서비스 전체 진입 자격", 로그인은 "이 요청이 누구 것인가"로 층위가 다르다. 그래서 헤더도 분리(`X-Demo-Token` vs `Authorization`) — 나중에 로그인이 붙어도 방문자가 두 증거를 각자 자기 헤더에 실어 보내면 되므로 안 엉킨다.
- **구현**:
  - **토큰**: 기존 `JwtService` 재사용(새 시크릿 불필요) — `{ type: 'demo' }` payload로 sign, `JWT_EXPIRES_IN`(1h) 그대로. 로그인 payload(`sub/email/role`)와 모양이 달라 서로 안 섞임. `AuthModule`이 `JwtModule`을 `exports`하도록 수정해 `DemoModule`이 `JwtService`를 주입받게 함.
  - **`POST /demo/gate`**: 비번 검증 후 `demoToken` 발급(`DemoService.enterGate`). `@Public()`(신규 데코레이터, `@Roles`와 같은 `SetMetadata`+`Reflector` 패턴)으로 전역 가드를 우회 — 게이트 자신은 토큰 없이도 열려있어야 발급이 가능하다(닭-달걀 문제 회피).
  - **`DemoGateGuard`**: `APP_GUARD`로 `AppModule`에 등록한 전역 가드. `X-Demo-Token` 헤더를 읽어 검증하고, payload의 `type !== 'demo'`면 거부(로그인 JWT를 게이트 토큰으로 재사용 못 하게). **`DEMO_GATE_PASSWORD`가 설정 안 돼 있으면 자동으로 게이트를 비활성화**(로컬 개발·기존 테스트·`bench.sh`가 안 깨지게) — 새 on/off 플래그를 추가하는 대신 이미 있는 env의 유무 자체를 신호로 재사용.
  - **`/health`**에 `@Public()` — 인프라 모니터링이 데모 토큰을 가질 수 없으므로.
- **테스트**: `DemoGateGuard`는 실제 HTTP/DB 없이 `canActivate()`만 검증하는 순수 단위 테스트(가짜 `ExecutionContext`)로 작성 — supertest 같은 e2e 의존성을 새로 안 들여도 충분(6건). `DemoService.enterGate`도 통합 테스트 3건 추가. **전체 28→37 그린.** 서버 기동 후 curl로 5단계(헬스체크 공개→토큰 없이 차단→틀린 비번 거부→올바른 비번 발급→토큰으로 통과) 전부 수동 검증, 비번 미설정 시 자동 우회도 별도 확인.
- **다음**: 방문자 식별/예매 방식 결정(위 "논의가 세 번 바뀜" 참고, 별도 ADR 검토) → W4 서버측 부하 시뮬레이션(축 B-1) → stats SSE(축 B-2) → 프론트엔드(apps/web) → 배포.

## 2026-08-01 · 방문자 예매 인증 — Google SSO 도입

- **결정**: 세 번 바뀌었던 방문자 식별 방식(admin 불필요 → 게스트 → Google SSO) 논의를 오늘 마무리. 기존 이메일/비번 로그인(W1, 새 코드 불필요) 재사용을 더 단순한 대안으로 제안했으나, **사용자가 Google SSO를 명시적으로 선택**해 그대로 진행.
- **개념**: OAuth 2.0 authorization code flow. 이메일/비번 로그인과 달리 **브라우저 리다이렉트가 필수**(①`/auth/google`→Google 로그인 페이지 ②Google이 code를 실어 `/auth/google/callback`으로 되돌림 ③서버가 code로 프로필을 받아 자체 JWT 발급). API 요청 하나로 끝나지 않는다는 점이 지금까지의 JSON API 패턴과 다름.
- **⚠️ 설계 중 발견 — 게이트와의 충돌**: 브라우저의 페이지 이동(리다이렉트)은 커스텀 헤더(`X-Demo-Token`)를 못 붙인다. `/auth/google`과 `/auth/google/callback`은 구조적으로 게이트를 우회(`@Public()`)해야만 동작 — 로그인 "시작"은 게이트 없이 가능해지지만, 실제 비용이 드는 작업(예매 등)은 여전히 게이트+로그인 토큰 둘 다 필요해 안전하다고 판단하고 진행.
- **스키마**: `User.password`를 `String?`(nullable)로 변경(마이그레이션 `20260801133308_user_password_nullable_for_google`) — Google 계정은 비밀번호가 없음.
- **정직한 버그 하나(구현 중 발견)**: `GoogleStrategy`가 `clientID: '' `처럼 빈 문자열 fallback을 쓰면 `passport-oauth2`가 생성자에서 바로 throw해 **앱 부팅 자체가 죽었다**(JWT_SECRET처럼 "설정 안 해도 부팅은 된다"를 의도했는데 반대로 동작). 빈 문자열이 아니라 `'not-configured'` 같은 의미 있는 플레이스홀더로 고쳐 해결 — 미설정 시에도 부팅은 되고, 실제 `/auth/google` 시도 시에만 Google이 invalid_client로 거부한다.
- **구현**: `google.strategy.ts`(신규, `passport-google-oauth20`), `GoogleAuthGuard`(`AuthGuard('google')`), `AuthController`에 `GET /auth/google`(리다이렉트 시작)·`GET /auth/google/callback`(토큰 발급, 프론트 없어 지금은 JSON 직접 반환). `AuthService`에 `findOrCreateGoogleUser()`(이메일로 찾거나 `password:null`로 생성) + `login()`/Google 콜백이 공유하는 `issueToken()` 추출. `login()`에 `!user.password` 가드 추가(Google 전용 계정으로 비번 로그인 시 `argon2.verify(null,...)` 크래시 대신 깔끔한 401).
- **작은 리팩터**: `@Public()` 데코레이터가 `demo/`뿐 아니라 `auth/`(Google 라우트)에서도 필요해져 `common/decorators/`로 이동 — demo 전용이 아니라 앱 전역 관심사였음이 드러남.
- **새 env 3개**: `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_CALLBACK_URL`. `.env.example` 반영. 사용자가 직접 Google Cloud Console에서 OAuth 클라이언트를 발급(OAuth 동의 화면 → 테스트 사용자 등록 → 웹 애플리케이션 클라이언트 → 리디렉션 URI 등록)하고 `.env`에 값을 채움.
- **테스트**: `auth.service.spec.ts`에 4건 추가(null-password 로그인 401, `findOrCreateGoogleUser` 3건). **전체 37→41 그린.** 실제 Google Cloud Console 발급 자격증명으로 **브라우저에서 실제 로그인까지 end-to-end 검증** — 발급된 토큰으로 `/auth/me` 통과 확인, DB에 `password IS NULL` 계정 생성 확인.
- **다음**: W4 서버측 부하 시뮬레이션(축 B-1, 게이트를 통과한 방문자가 자기 계정으로 직접 예매도 가능해졌으니 시뮬레이션과 나란히 노출 가능) → stats SSE(축 B-2) → 프론트엔드(apps/web) → 배포.

## 2026-08-05 · W4 서버측 부하 시뮬레이션 (ADR 0016 축 B-1)

- **설계 논의(사용자 발, ADR 0016에 2026-08-05 개정으로 반영)**: 세부 4가지를 이 세션에서 확정.
  1. **투입 리듬**: 처음엔 "점진적"(순차)으로 답이 나왔으나, 이 프로젝트의 목적(W2가 증명한 "동시 쓰기 경합")을 다시 짚으면서 재검토 — 순수 순차 투입은 동시성 경합 자체가 안 걸리고, 순수 동시 발사는 Redis 원자 감산이 워낙 빨라 대시보드가 보여줄 새도 없이 끝난다. **묶음(batch) 발사**로 절충(짧은 간격마다 일정 묶음을 동시 발사) — 묶음 안은 진짜 동시 요청, 묶음 사이 간격은 사람이 눈으로 따라갈 여유.
  2. **가상 유저 신원**: "가짜 값 먼저 시도"로 나온 답을, 스키마 확인(`Reservation.userId`가 `User` FK) 결과 가짜 id는 **시도할 필요 없이 확정적으로 실패**함을 짚어 바로 "가상 유저마다 실제 `User` 레코드 생성"으로 확정.
  3. **쿨다운 강제 위치**: "프론트 버튼으로 수동 처리"라는 초기 답이 ADR 자체의 "신뢰 경계는 백엔드"(§A와 동일 논리) 원칙과 상충할 수 있어 재확인 → **백엔드 Redis 쿨다운 유지 + 프론트는 그 결과(429)만 반영**으로 정리(스코프는 전역 — 게이트 통과자면 누구든).
  4. **가상 유저 정리**: 재고/예매 리셋(§C)만으론 시뮬레이션이 만든 `User` 행이 무료 티어 DB에 리셋마다 계속 쌓인다는 점을 짚어, 이메일 접두사(`sim-`)로 식별해 리셋 시 함께 삭제하도록 추가(스키마 변경 없이 `isDemo` 식별 취지 재사용).
- **구현**:
  - `ReservationsModule`이 `ReservationsService`를 `exports`(기존엔 모듈 내부 전용) → `DemoModule`이 이를 import해 가상 유저의 예매를 실제 파이프라인(`create(eventId, userId, 1, 'held', idempotencyKey)`)으로 그대로 흘림. 시뮬레이션 전용 우회 경로를 새로 안 만들고 **실제 서비스 코드를 재사용**하는 게 핵심 — 그래야 이 시뮬레이션이 "진짜" 동시성 문제를 재현한다.
  - `DemoService.simulateLoad()`: ①`DEMO_SIM_MAX_VU` 상한 검사 ②Redis `SET NX PX`로 쿨다운을 원자적 확인+잠금(확인과 잠금을 분리하면 그 사이 경합 가능) ③데모 이벤트 조회 ④`202` 즉시 응답, 실제 투입은 `void`로 fire-and-forget(컨트롤러가 기다리지 않음, 실패는 `Logger.error`로만).
  - `runSimulationBatches`: `SIM_BATCH_SIZE=20`·`SIM_BATCH_INTERVAL_MS=300` 상수로 묶음 발사. `injectVirtualUser`: `sim-<uuid>@sunchak.demo`·`password:null` User 생성 후 `held` 전략 호출. 재고 소진(`ConflictException`)은 이 데모가 보여주려는 정상 시나리오라 조용히 넘기고, 그 외 예외만 로그.
  - `resetDemoEvent()`에 `sim-` 접두사 `User` 삭제 추가(예매 삭제 다음 순서 — FK 걱정 없이 지울 수 있게).
  - `POST /demo/simulate`(`SimulateDto`, `@HttpCode(202)`) — 전역 게이트 가드가 이미 보호(별도 가드 불필요, `reset`과 같은 패턴).
- **검증**: 유닛 테스트 4건 추가(`demo.service.spec.ts` — 상한 초과 400, 정상 요청 202+백그라운드 실제 투입, 쿨다운 중 429, 리셋 시 `sim-` 유저만 삭제·일반 유저는 보존). `ReservationsService`는 모킹(큐 인프라까지 테스트 모듈에 안 들여도 됨). **전체 41→45 그린.**
  - **실제 서버로 e2e 수동 검증**(모킹 없이): seed 후 `POST /demo/simulate {virtualUserCount:45}` → 72ms만에 202 응답 확인(블로킹 안 됨) → 3초 후 Redis `stock:event:254` 100→55, DB `reservations` 45건 전부 CONFIRMED(워커가 이미 처리), `sim-` User 45건 생성 확인. 즉시 재요청은 429(쿨다운) 확인. 301명 요청은 400(상한) 확인. `POST /demo/reset` 후 `sim-` User·예매 0건, Redis 재고 100 원복 확인.
- **다음**: W4 실시간 stats 대시보드(축 B-2) — 2.4의 `@Sse`+`Subject` 배관을 재사용해 재고·HELD/CONFIRMED·큐 적체를 스트리밍. 지금 시뮬레이션은 결과를 curl로만 확인했지만, B-2가 붙으면 방문자가 이 과정을 실시간으로 지켜볼 수 있게 된다.

## 2026-08-05 · W4 실시간 stats 대시보드 (ADR 0016 축 B-2)

- **설계 결정 — 폴링 vs 이벤트 기반**: 2.4의 SSE는 이벤트 기반(워커가 확정하는 그 순간에만 `publish`)이었지만, stats는 소스가 3개(Redis 재고·DB의 HELD/CONFIRMED 합계·BullMQ 큐 적체)라 이벤트 기반으로 엮으려면 관문 DECRBY·HELD INSERT·sweep·reconcile·confirm 워커 등 **이미 완성된 W3 코드 여러 곳**에 손을 대야 해서 결합도가 커진다. 대신 **1초 주기 폴링 스냅샷**(그때그때 다시 조회해서 그대로 흘려보냄)으로 결정 — 기존 코드 무변경, 새 파일도 없이 `DemoService`에 메서드 2개 추가로 끝남. 시뮬레이션이 어차피 배치 단위(300ms 간격)로 진행돼 "놓치면 안 되는 찰나의 이벤트"가 없다는 점도 폴링이 무난한 이유.
- **구현**: `DemoModule`에 `BullModule.registerQueue({ name: CONFIRM_QUEUE })` 추가(같은 이름으로 여러 모듈이 등록해도 큐가 중복 생성되지 않음 — `AppModule`의 `forRootAsync` 연결 설정을 공유) → `DemoService`가 `@InjectQueue(CONFIRM_QUEUE)`로 큐를 주입받아 `getWaitingCount()`/`getActiveCount()`로 **적체만 읽음**(job을 넣지 않음, `ReservationsService`의 몫과 분리 유지).
  - `streamStats()`: 구독 시작 전 데모 이벤트 존재를 한 번만 확인(리셋은 같은 이벤트 행을 갱신할 뿐 id는 안 바뀌므로 매 틱마다 다시 찾을 필요 없음) → `timer(0, 1000)`(구독 즉시 1회 + 이후 1초마다)를 `switchMap`으로 `getStats()`에 연결.
  - `getStats(eventId)`: Redis 재고 + Prisma `groupBy(status)`(HELD/CONFIRMED quantity 합, reconcile.processor와 같은 패턴) + 큐의 waiting/active 카운트를 `Promise.all`로 병렬 조회.
  - `DemoController`에 `@Sse('stats/stream')` 추가(전역 게이트 가드로 이미 보호, JWT 불필요 — 개인 데이터가 아니라 게이트 통과자 전원이 같은 화면을 봄).
- **테스트**: `demo.service.spec.ts`에 2건 추가(이벤트 없으면 404, HELD/CONFIRMED/큐 적체 스냅샷이 기댓값과 일치) — `getQueueToken(CONFIRM_QUEUE)`로 큐를 가벼운 목(mock)으로 대체(실제 워커·Redis 큐 인프라 불필요, `reservations.service.spec.ts`가 이미 그 책임을 검증 중). **전체 45→47 그린.**
- **실서버 e2e 수동 검증**: 리셋 → `curl -N`으로 SSE 스트림을 열어두고 1초 뒤 60명 시뮬레이션 투입 → 실시간으로 `재고 100→40, HELD 5→0, CONFIRMED 0→55→60, 큐 적체 0→6→0`으로 움직이는 스냅샷을 그대로 확인. ADR이 그리던 "숫자가 실시간으로 움직이는 대시보드"가 실제로 동작함을 확인.
- **발견(수정은 다음으로 미룸)**: `simulateLoad()`가 쿨다운을 먼저 잠그고 그다음에 데모 이벤트 존재를 확인해서, 이벤트가 없는 상태(seed 전)에서 호출하면 404를 받으면서도 쿨다운은 이미 소비돼버린다. 사소한 엣지케이스(정상 운영에선 이벤트가 항상 있음)라 지금은 기록만 해두고 보류.
- **다음**: W4 프론트엔드(apps/web) — 게이트 화면 + Google 로그인 버튼 + 데모 컨트롤(시뮬 버튼)·stats 대시보드 화면. 이제 백엔드 조각(게이트·리셋·시뮬·stats)이 다 갖춰졌다.

## 2026-08-05 · W4 프론트엔드 착수 — 워크스페이스 + 쿠키 기반 인증 전환

- **pnpm 워크스페이스 신설**: ADR 0012가 "apps/web 추가 시점에 정식 워크스페이스 도입"이라 못박아둔 그 시점이라 별도 논의 없이 진행. 루트 `package.json`+`pnpm-workspace.yaml` 신설, `apps/api`/`apps/web`를 `@sunchak/api`/`@sunchak/web`로 워크스페이스 스코프 통일, 개별 lockfile 2개를 루트 단일 lockfile로 통합.
  - **삽질**: `apps/api/pnpm-lock.yaml`을 지워도 이후 `pnpm --filter @sunchak/api add ...`를 돌리면 계속 되살아남 → 원인은 `apps/api/package.json`이 루트와 별개로 자기만의 `packageManager` 필드를 여전히 갖고 있었던 것(워크스페이스 멤버가 중복 선언하면 그 디렉토리를 준-루트처럼 취급해 로컬 lockfile을 미러링하는 것으로 보임). 그 필드를 지우니 재발 안 함.
  - 네이티브 빌드 승인(`pnpm-workspace.yaml`의 `allowBuilds`)은 apps/api 단독 개발 때 이미 STATUS.md에 남겨둔 판단(Prisma·argon2만 승인, msgpackr-extract·unrs-resolver는 생략)을 그대로 재적용.
- **Next.js 스캐폴딩**: App Router + TypeScript + Tailwind + TanStack Query. `Providers`(`'use client'`, `QueryClient`를 `useState`로 1회 생성)를 `layout.tsx`에 배선. **이 Next.js 버전(16.3.0)은 학습 데이터보다 최신**이라 `node_modules/next/dist/docs/`를 실제로 읽어 확인 — `LayoutProps<'/route'>` 같은 새 전역 타입 헬퍼가 생겼음(params/searchParams가 route별로 자동 타입 생성, `next dev`/`build`/`typegen` 시점에 생성됨).
- **SSE 인증 방식 전환 — 쿼리파라미터 대신 쿠키(사용자 정정)**: 브라우저 `EventSource`는 커스텀 헤더를 못 붙여(기존부터 STATUS.md에 기록된 틈) `X-Demo-Token`/`Authorization` 헤더 인증이 그대로는 브라우저에서 안 열린다. 처음 "쿼리파라미터로 토큰 전달"을 수정 범위가 적다는 이유로 1순위 추천했으나, **사용자가 "데모라고 적은 수정 범위를 무조건 고르지 말고 표준·성능을 우선하라"고 정정** — URL 노출(서버 로그·브라우저 히스토리에 토큰이 남음)이라는 비표준적 절충이었음을 지적받고 쿠키(HttpOnly) 기반으로 방향을 바꿈. 이 원칙은 메모리에 기록해 앞으로도 적용.
  - **구현**: `cookie-parser` 추가, `main.ts`에 등록 + CORS 활성화(`credentials:true`+구체적 origin — 와일드카드는 쿠키와 함께 못 씀). 공유 헬퍼 `common/auth-cookie.ts`(쿠키 이름 상수 + `httpOnly`/`secure`/`sameSite` 정책, `NODE_ENV`로 배포 시 `SameSite=None+Secure` 분기 미리 심어둠). `JwtStrategy`/`DemoGateGuard`가 **쿠키를 우선 확인하고 없으면 헤더로 폴백**(브라우저는 쿠키 자동 전송, curl 등 STATUS.md에 기록된 기존 수동 검증 절차는 헤더로 계속 동작 — 표준을 택하되 기존 워크플로우를 안 깨는 절충). `login`/`enterGate`/Google 콜백이 JSON 응답과 별개로 쿠키도 심음. **Google 콜백의 오래된 TODO(프론트 없어 JSON 임시 반환)를 이제 실제로 해소** — 쿠키 심고 `WEB_APP_URL`로 리다이렉트.
  - **쿠키 만료 안 박음**: `maxAge`를 `JWT_EXPIRES_IN`("1h" 같은 문자열)에 맞추려 `ms` 파싱 의존성을 늘리는 대신, 브라우저 세션 쿠키로 두고 실제 만료는 JWT 서명 검증(`ignoreExpiration:false`)에 맡김 — 쿠키가 남아있어도 서버가 만료된 토큰은 어차피 거부.
  - **새 env**: `WEB_APP_URL`(CORS origin + Google 리다이렉트 대상).
  - **검증**: `tsc`+전체 47개 테스트 그린(서비스 계층 테스트라 컨트롤러의 쿠키 로직은 영향 없음). **실서버 e2e로 쿠키만(헤더 없이) 통과 확인**: 게이트(`POST /demo/gate` → `Set-Cookie` 확인 → 쿠키만으로 `GET /demo/stats/stream` SSE 통과, 쿠키 없인 401), 로그인(`POST /auth/login` → `Set-Cookie` 확인 → 쿠키만으로 `GET /auth/me` 통과). `reservation-stream`은 같은 `JwtStrategy`를 타므로 별도 재검증 없이 동일 보장 적용됨.
- **다음**: 실제 화면(게이트 진입 → Google 로그인 → 데모 컨트롤 + stats 대시보드) 구현. `fetch`/`EventSource` 호출엔 `credentials:'include'`/`withCredentials:true` 필요.

## 2026-08-06 · W4 프론트 실제 화면 + "내 예매"(실제 티케팅) 기능

- **구현**: 게이트 진입 폼(`gate-form.tsx`) → Google 로그인 버튼 → 데모 대시보드(`demo-dashboard.tsx`, 시뮬 컨트롤+stats 스탯 타일). `page.tsx`가 상태 하나(`checking/gate/login/dashboard`)로 세 화면을 조건 렌더링 — 전용 상태 확인 엔드포인트를 새로 안 만들고 기존 `GET /auth/me`의 401 메시지 문자열(게이트 실패 한글 메시지 vs JWT 실패 `"Unauthorized"`) 차이로 판별(백엔드 에러 문자열에 약하게 결합되는 트레이드오프를 인지하고 사용자 확인 후 채택).
- **dataviz 스킬 참고**: stats 대시보드는 "헤드라인 숫자 몇 개"라 차트가 아니라 KPI row(스탯 타일)로 판단. 색은 텍스트에 안 쓰고 재고 소진(매진) 같은 실제 상태에만 status 팔레트(`#d03b3b`) + 라벨 병기.
- **Vitest+RTL 도입**: `apps/web`에 테스트 프레임워크가 아예 없어서(CLAUDE.md §7 요구사항) 이 세션에서 처음 설치. `vitest.config.mts`(jsdom, `resolve.tsconfigPaths:true` — 별도 플러그인 없이 Vite 8 내장 옵션), `vitest.setup.ts`(jest-dom + `afterEach(cleanup)` — globals를 안 켰으므로 RTL 자동 cleanup이 안 붙어 직접 등록해야 함, 안 하면 테스트 간 DOM 잔존으로 "여러 개 찾힘" 에러). jsdom엔 `EventSource`가 없어 `src/test/fake-event-source.ts`(최소 흉내 클래스)로 대체.
- **"내 예매" 기능 추가 — 진짜 티케팅 기능이 빠져 있었다는 걸 사용자가 직접 써보고 발견**: 데모 대시보드가 "관찰"(가상 유저 시뮬+통계)만 가능했고, 로그인한 방문자 본인이 실제로 예매하는 화면이 없었다 — ADR 0014/0015가 만든 실제 파이프라인(HELD→큐→확정→SSE)을 프론트가 한 번도 호출하지 않고 있었다. `booking-form.tsx` 신규: `GET /events`로 `isDemo` 이벤트 조회 → 수량 입력+예매하기 → `POST /events/:eventId/reservations?strategy=held` → 응답 `reservation.id`로 `GET /reservations/:id/stream`(SSE) 구독해 HELD→CONFIRMED 실시간 반영.
- **실서버 e2e로 실제 확인**: 로그인 → 예매 → 확정까지 실제로 동작. **로컬은 관문→HELD→큐→확정이 너무 빨라(수백ms) "HELD" 문구가 화면에 거의 안 보이고 곧바로 확정으로 넘어감** — 버그 아니라 로컬 워커 지연이 거의 없어서 그런 것.
- **git 위생 버그 발견·수정**: `apps/web/.gitignore`(create-next-app 기본 생성분)의 `.env*` 패턴이 `.env.example`까지 무시하고 있었다 — 이대로면 새로 만든 env 예시 파일이 영원히 git에 안 올라가 다른 기기에서 필요한 env를 알 방법이 없었을 것. 루트 `.gitignore`와 같은 좁은 패턴(`.env`/`.env.local`/`.env.*.local`)으로 교체.
- **다음**: 배포. 단, 이 세션 후반에 "티케팅 기능 자체가 이거 다인가?" 질문으로 더 큰 gap이 드러남(아래 항목).

## 2026-08-06 · 선착순 입장 대기열 (ADR 0017) — 원래 기획과의 gap 재확인에서 시작

- **계기**: "이 프로젝트 원래 목적이 뭐였는지 먼저 체크하자"는 사용자 요청으로 PRD(`02_서비스_기획안.md`)·로드맵·ADR 0006/0014/0015를 다시 대조. **발견**: PRD 원안의 핵심 시나리오("대기열에서 실시간 순번을 보며 기다린다")가 ADR 0014/0015 구현 과정에서 "즉시 판정 관문"으로 조용히 대체됐다 — 더 나은 엔지니어링 선택(k6로 증명)이었지만 어떤 ADR도 이 대체를 명시적으로 남기지 않았다. 추가로 코드 확인 결과 모의결제(Payment 테이블은 스키마에 있으나 서비스 없음)·오픈시각 검사·이벤트 목록 화면·내 예매 내역도 PRD 대비 빠져 있음(전부 STATUS.md에 기록, 대기열만 이번에 착수).
- **"즉시 판정 vs 대기열" 논의**: 사용자가 "대기열이 관문보다 얼마나 강력한지"를 물어 정정 — **둘은 경쟁 관계가 아니라 계층 관계**다. 대기열은 관문(hot row 직렬화 자체)의 처리량을 올리지 않는다, 오히려 의도적으로 입장 속도를 늦추는 장치다(다운스트림 보호+낭비 방지+재시도 폭탄 억제+봇 완화가 목적이지, "더 빠른 판정"이 목적이 아님). 이 프로젝트 실사용자는 소수(개발자+공유 대상)뿐이라 대기열의 진짜 존재 이유는 방어가 아니라 **원래 PRD 학습 포인트 완결 + 포트폴리오 서사 확장**임을 ADR에 솔직하게 명시.
- **설계(ADR 0017)**: Redis Sorted Set `queue:event:{id}`(FIFO) + BullMQ repeatable job(`AdmissionProcessor`, 2초 주기로 20명씩 `ZPOPMIN`) + `admitted:event:{id}:{userId}` 키(TTL, sweep 잡 없이 Redis가 자연 만료 — 입장 허가는 아직 재고를 안 건드린 상태라 유실돼도 정합성 문제 없음). `QueueService.assertAdmitted()` 한 곳을 실사용자(컨트롤러)·가상 유저(데모 서비스) 둘 다 통과해야 함 — 특수 경로 없음.
- **가상 유저 현실성**: 사용자 요청으로 "입장해도 랜덤하게 포기하거나 늦게 시도"를 추가(20% 포기 + 랜덤 지연). 이게 오히려 설계를 단순하게 만듦 — 가상 유저도 실사용자와 똑같이 허가창을 놓칠 수 있어 특수 케이스 없이 같은 코드 경로로 통합됨.
- **삽질 — 파라미터 두 개의 비율을 안 맞춰서 생긴 버그 아닌 버그**: 입장 허가창을 "더 짧게"(30초→8초) 줄이면서, 가상 유저 랜덤 지연 범위(1~35초, 원래 30초 창 기준으로 잡았던 값)를 같이 안 줄여서 실서버로 30명 시뮬레이션했더니 확정이 5명뿐이었다(기대는 대부분 성공). 원인 분석: 지연 범위가 허가창보다 훨씬 넓어 "느려서 놓침" 비율이 원래 의도(일부만)가 아니라 압도적 다수가 돼버림(확률 계산상 성공률 ~16%). **교훈**: 두 시간 상수(허가창 vs 지연 범위)는 독립적으로 못 정한다 — 지연 범위는 반드시 허가창보다 "살짝만" 넓어야 "대부분 성공, 느린 일부만 놓침"이 된다. 지연을 500ms~10초로 좁혀 재검증 → 30명 중 22명 확정(~73%), 의도한 그림이 나옴. ADR·`.env.example` 수치도 실제 값으로 정정.
- **테스트**: 신규 `queue.service.spec.ts`(8건)·`admission.processor.spec.ts`(4건)·`reservations.controller.spec.ts`(3건, 대기열 연동은 컨트롤러 레이어라 이 프로젝트 첫 컨트롤러 단위 테스트) + `demo.service.spec.ts`에 2건 추가(포기/허가창 만료). 지연·허가창 시간을 실제 값(수 초~수십 초)으로 기다리면 테스트가 느려져, `DEMO_SIM_ABANDON_PROBABILITY`/`DEMO_SIM_MIN·MAX_BOOKING_DELAY_MS`/`QUEUE_ADMISSION_WINDOW_MS`를 env로 빼 테스트에서만 아주 작은 값으로 덮어씀(기존 `DEMO_SIM_MAX_VU` 패턴과 동일). **전체 47→64 그린.**
- **프론트**: `booking-form.tsx`를 확장해 "대기열 진입부터 예매 결과까지" 한 패널에서 문구만 바뀌는 상태 하나로 표현(`idle→queued→admitted→held→confirmed/expired/error`) — 화면을 안 갈아탐(사용자 요청). 신규 테스트 3건 추가. **`apps/web` 9→12 그린.**
- **실서버 e2e**: 실제 로그인 사용자로 대기열 입장→순번 표시→(2초 뒤)입장 허가→예매→확정 전 흐름 확인. 가상 유저 30명으로 위 파라미터 버그도 실측으로 발견·재검증.
- **다음**: 배포(Dockerize → CI/CD → VM+Nginx → Grafana 관측 → 최종 k6 리포트 → README/회고, `docs/STATUS.md` 참고).

## 2026-08-06 · 모의 결제 (ADR 0018) — "대기열 다음은?"에서 시작

- **계기**: 사용자가 대기열 순번 계산 방식을 확인하는 질문 끝에 "다음 작업은 모의결제로 갈게요"로 방향을 정함. PRD(§4-5, §6)를 다시 보니 "모의 결제 버튼, 비동기 pending→done"이 이미 명시돼 있었고, ADR 0015가 스스로 "결제 stub"이라 부르며 이 자리를 예고해뒀던 걸 재확인.
- **설계**: `createHeld()`가 즉시 넣던 confirm job을 제거하고, "결제하기" 클릭이 새 `payment` 큐에 job을 넣게 함. `PaymentProcessor`가 80%/20% 확률로 성공/실패 판정 — 성공 시 **기존 `confirm` 큐에 job만 투입**(`ConfirmProcessor` 완전 재사용), 실패 시 Reservation→CANCELLED + Redis 재고 즉시 반환(사용자가 "즉시 반환 vs TTL 대기" 중 즉시 반환 선택).
- **`Payment.idempotencyKey` 재검토**: STATUS 백로그에 "재검토 필요"로 남아있던 항목. 확인해보니 Reservation과 달리 실제 버그가 아니었음 — `Payment.reservationId`가 이미 `@unique`(1:1)라 결제 재시도 자체가 그걸로 원자적으로 막힘. 별도 복합 unique 불필요로 결론.
- **⚠️ 삽질 — 이번 세션 최대 시간 소모**: 결제 구현 자체는 순조로웠는데, 테스트에서 SSE 관련 딱 1건("HELD 접수 직후 구독하면...")만 계속 5초 타임아웃. 처음엔 또 "백그라운드 서버가 큐를 나눠 먹나?" 의심 → `lsof -ti:3001 -sTCP:LISTEN | xargs kill`로 정리했는데도 **여전히 실패** — 여기서 한 단계 더 파고들어야 했다.
  - **디버깅 과정**: 테스트에 직접 `console.log`를 심어 확인 → DB는 정확히 CONFIRMED로 바뀜(워커는 돌고 있음) → 그런데 `ReservationEventsService.ofReservation()`을 테스트에서 직접 구독해도 이벤트가 안 옴 → 즉 "누군가 job을 가져가 DB는 고치지만, 그 publish()는 다른 프로세스의 메모리 안에서만 터진다"는 뜻.
  - **진짜 원인**: `ps aux`로 확인하니 이번 세션 동안 여러 번 띄운 `pnpm start:dev`의 **자식 프로세스(`nest.js start --watch`) 5개가 좀비로 남아있었다.** `pnpm start:dev`는 껍데기 래퍼일 뿐이고, 실제 서버(BullMQ 워커 포함)는 그 자식이다 — 포트를 점유한 프로세스(래퍼 또는 자식 중 하나)만 죽이면, **포트를 못 잡은 나머지 자식들은 포트 없이도 Redis에는 계속 연결된 채 살아남아** confirm 큐 job을 경쟁적으로 가져간다. `lsof -ti:포트`는 "포트를 점유한" 프로세스만 찾으므로 이 좀비들을 못 잡는다.
  - **해결**: `ps aux | grep "nest.js start" | grep -v grep | awk '{print $2}' | xargs kill -9`로 자식까지 직접 찾아 죽임 → 그 즉시 테스트 전체 그린. **교훈**: 이 프로젝트에서 백그라운드로 `pnpm start:dev`를 띄웠다 지울 땐 포트가 아니라 프로세스 이름(`nest.js start`)으로 찾아야 한다 — 포트 기준 정리는 "겉보기 정리"일 뿐이었다.
- **테스트**: 기존 `reservations.service.spec.ts`의 "HELD 접수 후 자동 확정" 전제 테스트들을 "confirm job을 직접 넣어(=결제 성공 흉내) 확인"으로 수정 + "결제 없이는 자동 확정 안 됨" 회귀 테스트 추가. 신규 `payments.service.spec.ts`(4)·`payment.processor.spec.ts`(3, 성공/실패/이중반환 방지). `Payment`가 `Reservation`에 FK를 걸게 되며 여러 스펙 파일의 `beforeEach`에 `payment.deleteMany()`를 `reservation.deleteMany()`보다 먼저 넣어야 했다(안 그러면 FK 제약 위반). **API 65→72 그린.**
- **프론트**: `booking-form.tsx`의 `held` 단계에 "결제하기" 버튼 추가, `paying`(PENDING)·`cancelled`(실패+반환) 상태 신설. 예매 SSE 스트림이 CONFIRMED뿐 아니라 CANCELLED도 받아 분기하도록 수정(기존 "누가 confirm을 일으켰든 방송 하나만 듣는다" 설계 그대로 재사용). 신규 테스트 2건. **web 11→13 그린.**
- **실서버 e2e**: 같은 브라우저 흐름을 5회 반복 → 4번 확정 / 1번 취소(80% 설계값과 근접) 둘 다 실제로 확인, 콘솔 에러 0.
- **다음**: 배포 6단계 또는 남은 PRD 갭(오픈 시각 검사·이벤트 목록 화면·내 예매 내역·관리자 대시보드) 중 선택.

## 2026-08-06 · PRD 갭 재검토 + 리셋 500 버그 발견·수정

- **PRD 갭 재검토(사용자 주도)**: 남은 4개 갭을 하나씩 "이 데모에 실제로 필요한가"로 재검토.
  - **오픈 시각 검사**: "오픈 전엔 대기열 자체가 안 서고, 오픈 시점에 한꺼번에 입장"까지 그려봤지만, 지금 시뮬 버튼이 이미 몰림을 보여주고 있어 **불필요로 결론**. 이 과정에서 나온 "대기열 순번을 랜덤화하면 어떨까" 제안도 검토했으나, 혼자 테스트할 때 가상 유저가 실사용자를 못 앞서게(=항상 0번) 만들면 "부여받은 순번만큼 기다린다"는 대기열의 학습 포인트 자체가 사라진다는 사용자의 반박이 맞아 **기각** — 지금처럼 도착 순서 그대로(FIFO, 실사용자·가상유저 동등)가 정답.
  - **이벤트 목록/상세**: 데모뿐 아니라 실제 제품에도 필요하다고 판단 변경 — 여러 이벤트를 보여주되 의도적으로 하나만 판매중, 나머지는 마감 처리. 리셋 방식(전체 재생성 vs 열린 이벤트 하나만 재예매 가능하게)은 학습/UX에 영향 없다고 판단해 편리성 기준으로 선택 위임받음 → 다음 세션에서 "나머지는 정적 시드, 리셋은 지금처럼 열린 이벤트 하나만" 방향으로 결정 예정(기존 코드 재사용 극대화).
  - **내 예매 내역**: 불필요로 최종 확정.
  - **관리자 대시보드**: "판매 현황"이 기존 공개 stats 대시보드와 실질적으로 같다고 확인 → 별도 화면 없이 **기존 대시보드에 통합**, 결제 성공/실패(PAID/FAILED) 집계만 추가하기로 확정. 이벤트 등록 폼·admin 역할 분기는 불필요로 확정.
- **⚠️ 발견 — 사용자가 브라우저로 직접 써보다 리셋 버튼이 500을 내는 걸 포착**: 원인 조사 결과 이번 세션에 추가한 `Payment→Reservation` FK(ADR 0018)가 `resetDemoEvent()`의 `reservation.deleteMany()`를 막고 있었다(`P2003`). 여러 테스트 스펙 파일의 `beforeEach` 정리 순서는 그날 바로 고쳤는데, **정작 실제로 리셋 버튼이 호출하는 프로덕션 메서드 자체는 놓쳤던 것** — 테스트가 전부 그린이어도 "그 로직을 실제로 호출하는 경로"가 빠지면 못 잡는다는 교훈. `payment.deleteMany({ where: { reservation: { eventId } } })`를 예매 삭제 앞에 추가해 해결, 회귀 테스트 1건 추가. **API 72→73 그린.** 실서버로 즉시 재검증(재고 100/100 정상 리셋 확인).
- **다음**: 이벤트 목록/상세 화면 구현(정적 마감 이벤트 시드 + 목록 UI) + stats 대시보드에 결제 집계 타일 추가 → 그다음 배포.

## 2026-08-06 · 이벤트 목록/상세 + 판매 현황 통합 (PRD 갭 마무리)

- **구현**: `prisma/seed.ts`에 정적 마감(`SOLD_OUT`, `isDemo:false`) 이벤트 2개를 idempotent하게 추가 — 제목으로 존재 여부를 확인해 중복 생성 방지. `resetDemoEvent()`는 `isDemo:true` 하나만 대상이라 이 둘은 리셋에 전혀 영향받지 않는다(직전 대화에서 편리성 기준으로 확정한 설계 그대로).
  - 신규 `event-list.tsx`: `GET /events` 전체를 카드로 나열, 상태(`ON_SALE`/`SOLD_OUT`/`UPCOMING`/`CLOSED`)를 한글 라벨+색으로 표시. 이벤트가 몇 개뿐이라 목록과 상세(제목/설명/가격/상태)를 한 화면에 합쳤다 — 별도 라우팅/페이지 없음.
  - `DemoStats`에 `paidCount`/`failedCount` 추가(`Payment.groupBy`). "관리자 판매 현황"이 기존 공개 stats와 실질적으로 같다는 판단(직전 대화)에 따라 별도 화면 없이 기존 대시보드 그리드에 타일 2개만 추가(`grid-cols-3`, 6타일). 헤딩 "실시간 판매 현황" 추가.
- **테스트**: `event-list.test.tsx`(1건, 판매중/매진 라벨 구분 확인). `demo.service.spec.ts`의 stats 스냅샷 테스트에 CANCELLED 예매 + Payment 2건(PAID/FAILED)을 추가해 새 집계 필드까지 검증하도록 갱신. `demo-dashboard.test.tsx`/`page.test.tsx`의 `/events` mock도 `description`/`price`/`status` 필드를 채워야 했다(EventList가 그 필드들을 쓰므로 — 안 채우면 `undefined.toLocaleString()`으로 렌더 자체가 깨짐).
- **⚠️ 삽질 — 코드가 아니라 이 세션의 셸 환경 문제**: 프론트 테스트를 돌렸는데 관련 없는 파일까지 전부 `TypeError: React.act is not a function`으로 깨짐. 원인은 이 대화의 bash 환경에 `NODE_ENV=production`이 어디선가 새어들어와 있었던 것 — react-dom이 production 빌드(테스트용 `act` 없음)로 로드됨. `unset NODE_ENV`(또는 `NODE_ENV=test` 강제)로 즉시 해결, 코드는 무관했다. Jest(백엔드)는 자체적으로 `NODE_ENV=test`를 강제해서 영향 없었다.
- **실서버 e2e**: 게이트→로그인 후 이벤트 목록(매진 2·오픈예정 1·판매중 1)과 통합 대시보드 6타일이 실제로 정상 렌더되는 것 스크린샷으로 확인. 콘솔 에러 0.
- **결과**: `docs/STATUS.md`가 기록해온 PRD 갭 4개(오픈시각·이벤트목록·예매내역·관리자대시보드) + 모의결제까지 전부 처리 완료 — 남은 건 배포뿐.

## 2026-08-06 · 실사용 중 발견한 버그 2건(테스트가 실서버를 파괴 / 가상 유저 결제 누락) + 리셋 버튼

- **계기**: 사용자가 직접 브라우저로 데모를 돌리다 세 가지를 리포트 — "가상 유저 투입했는데 재고가 안 줄어요", "큐 적체도 그대로 0", "리셋은 버튼이 안 보이는데요?". 셋 다 실사용 중 발견이라 이론적 리뷰로는 못 잡았을 버그였다.
- **원인 조사**: `docker exec sunchak-redis redis-cli`로 `queue:event:1079` Sorted Set을 보니 201명이 쌓여만 있고 전혀 빠지지 않고 있었다. `KEYS bull:admission:repeat:*`가 **아예 비어 있음** — admission 잡의 반복 스케줄러 자체가 등록 안 돼 있었다는 뜻.
- **버그 A — 진짜 원인, 이번 세션에서 가장 심각한 발견**: `admission.processor.spec.ts`·`sweep.processor.spec.ts`·`reconcile.processor.spec.ts` 세 파일 모두 `afterAll`에서 `queue.obliterate({ force: true })`를 호출하고 있었다. 문제는 이 테스트들과 실행 중이던 `pnpm start:dev`가 **같은 Redis, 같은 큐 이름을 그냥 공유**한다는 것 — `obliterate`는 "그 큐에 관한 모든 것"(대기 job·완료 이력뿐 아니라 **반복 job 스케줄러 등록 자체**)을 지우는 API라, 이번 세션 동안 `jest`를 여러 번 돌릴 때마다 매번 **실서버가 등록해둔 admission 스케줄러가 통째로 사라졌다.** 겉보기엔 서버가 멀쩡히 살아 있고 다른 기능(예매·결제)은 정상이라 원인 특정에 시간이 걸렸다.
  - **수정**: 3개 spec 파일에서 `obliterate()` 호출과, 그것 때문에만 필요했던 `queue`/`Queue`/`getQueueToken` import·변수를 전부 제거. `moduleRef.close()`(연결만 닫음)만으로 테스트 정리는 충분하다 — 반복 스케줄러는 "같은 큐 이름+같은 옵션으로 등록하는 모든 프로세스가 공유하는 멱등 자원"이라 테스트가 굳이 지울 이유가 없었다(지우면 오히려 그 이름을 쓰는 다른 프로세스까지 함께 망가짐).
  - **복구**: API 서버를 재기동해 스케줄러를 재등록하니, 막혀 있던 201명이 몇 초 안에 정상적으로 20명씩 소진됐다.
  - **교훈**: 여러 프로세스가 같은 이름으로 공유하는 Redis/큐 자원에 대해 테스트 정리 코드에서 "전체 삭제" 계열 API를 쓸 때는, 그 자원이 정말 그 테스트만의 소유인지 먼저 확인해야 한다.
- **버그 B — 별개의 두 번째 원인**: 버그 A를 고쳐 admission이 다시 돌게 했는데도 `heldCount`만 늘고 `confirmedCount`가 0에 머물렀다. 원인은 ADR 0018(모의결제)에서 confirm 트리거를 "HELD 생성 시점"에서 "결제 성공 시점"으로 옮겼을 때, `DemoService.simulateBookingAttempt()`(가상 유저 시뮬레이션 코드)는 그대로 안 고쳐졌던 것 — 여전히 `reservations.create()`만 부르고 끝나서, 아무도 결제를 안 하니 confirm job이 영원히 안 들어가 HELD에 멈춰 있었다(5분 TTL 스윕 전까지).
  - **수정**: `ReservationsModule`이 `PaymentsService`도 `exports`하도록 하고, `DemoService`에 주입 → `simulateBookingAttempt()`가 예매 성공 뒤 `payments.pay(reservation.id, userId, randomUUID())`를 호출하도록 수정(실사용자와 완전히 같은 결제 경로, 시뮬레이션 전용 분기 없음). `demo.service.spec.ts`의 관련 목·단언 갱신.
  - **검증**: 실서버에서 가상 유저 30명 투입 → `remainingQty:89, confirmedCount:11, paidCount:11, failedCount:5`로 내부적으로 일관된 값 확인(100-11=89, PAID+FAILED=16이 결제 시도 전체와 일치).
- **리셋 버튼**: 백엔드 `POST /demo/reset`은 ADR 0016 때부터 있었지만 그걸 누를 UI가 화면에 없었다(재고가 소진돼도 되돌릴 방법이 안 보임). `demo-dashboard.tsx`의 "실시간 판매 현황" 헤딩 옆에 "데이터 리셋" 버튼 추가(`useMutation`, 성공/실패 문구 표시). 신규 테스트 1건.
- **검증**: API는 위험한 정리 로직만 제거한 것이라 테스트 수 변화 없이 73개 그린. web은 리셋 테스트 추가로 **14→15개 그린**(`tsc --noEmit`·`eslint` 클린). 실서버 curl로 게이트 통과 후 `POST /demo/reset` 호출 → 재고가 정확히 100/100으로 원복되는 것 확인.
- **별개(사용자 자신의 터미널 문제)**: 사용자가 자기 터미널에서 `pnpm exec prisma studio` 실행 중 "pnpm requires Node v22.13+, 현재 v22.12.0" 에러를 만남 — 코드 문제가 아니라 시스템 기본 `node`가 낡은 버전이라 발생. 이 기기에는 이미 nvm으로 v22.23.1이 설치돼 있어(`docs/STATUS.md` "이 기기 로컬 환경" 참고) `export PATH="$HOME/.nvm/versions/node/v22.23.1/bin:$PATH"`를 명령 앞에 붙이거나 `nvm use v22.23.1`로 해결 가능.
- **다음**: 배포 6단계(Dockerize → CI/CD → VM+Nginx → Grafana 관측 → 최종 k6 리포트 → README/회고) — PRD 갭·이번에 발견된 버그 모두 처리 완료.

## 2026-08-06 · 리셋↔대기열 경합 수정 + "재고소진 실패" 지표 — 세 번째 실사용 시나리오

- **계기**: 위 두 버그를 고친 뒤, 사용자가 이번엔 대기열까지 포함한 시나리오를 직접 재현: "대기열 순번 0 → 200명 투입 → 즉시 98개+2개 예매 → 멈춘 것처럼 보임(결제 실패도 안 늘어남) → 리셋 → 근데 리셋 후에도 재고가 또 깎임". 두 가지를 물었다 — ① 정말 멈춘 건가 계속 진행 중이었나, ② 리셋 시 대기열도 비워야 하지 않나.
- **① 분석**: 안 멈췄다. 200명은 입장 허가(20명/2초)+랜덤 지연(0.5~10초)으로 처리에 최대 30초 걸리는 게 정상 설계다. 재고가 소진된 뒤 나머지 시도는 `demo.service.ts`의 `catch (ConflictException) { return; }`에서 조용히 삼켜진다(재고소진이 이 데모가 보여주려는 정상 종료 시나리오라 의도적으로 조용함) — 그래서 "멈춘 것처럼" 보였을 뿐. **"결제 실패가 안 늘어난 이유"도 발견**: `failedCount`(결제 실패)는 결제 단계까지 도달한 뒤 20% 확률로만 나오는 지표인데, 재고소진으로 막힌 시도는 결제 단계에 도달조차 못 해 아예 다른 지표다 — 지금까지 이 둘을 구분할 UI가 없어서 "결제 실패가 왜 안 늘지?" 혼란이 생겼다.
- **② 확인 — 진짜 버그였다**: `resetDemoEvent()`가 재고·예매·유저는 리셋하면서 Redis 대기열(`queue:event:{id}` Sorted Set, ADR 0017)은 손 안 댔다. 그래서 리셋 전에 대기 중이던 사람들이 리셋 후에도 계속 입장 허가를 받아, 방금 원복한 재고를 또 깎았다.
- **설계 옵션을 사용자에게 물어봄**: "대기열 ZSet만 비우기(간단, 대부분 해결)" vs "세대(generation) 기반 완전 무효화(완벽하지만 여러 파일에 개념 전파 필요)" — **사용자가 A(ZSet만 비우기)를 선택**. 부가로 "재고소진 실패"를 스탯에 노출할지도 물어 **노출하기로 확정**.
- **구현**: `QueueService.purge(eventId)`(ZSet `DEL` + `queues:active`에서 `SREM`) 신규 → `resetDemoEvent()`가 재고 원복과 함께 호출. `reservations.service.ts`의 `createHeld()` 재고소진 분기(관문 DECRBY 음수)에 `soldout:event:{id}` Redis 카운터 추가(`INCR`) — `Payment.idempotencyKey` 논의 때처럼 W2 벤치마크 전략(naive~redis)은 안 건드리고 실제 파이프라인(held)에만 추가. `DemoStats.soldOutCount`로 노출, `demo-dashboard.tsx`에 "재고소진 실패" 타일 추가.
- **실서버로 한계까지 실측**: 30명 투입 1초 후 즉시 리셋 → 대기열 ZSet은 그 순간 0으로 비워졌지만, **이미 입장 허가를 받아 `admitted:event:{id}:{userId}` TTL 키를 든 소수는 여전히 통과** — 리셋 후 수 초간 재고가 100→97로 더 깎였다. 이건 사전에 예상하고 사용자에게 공지한 대로다: 그 키는 대기열 ZSet에 없어 `purge()`가 못 건드리고, 입장 허가창(기본 8초) TTL이 지나야 자연히 막힌다. 예전 버그(200명 중 대다수가 새던 것) 대비 훨씬 좁은 창(최대 8초, 실측 3명 정도)으로 줄었고, 그 창이 지난 뒤(약 10~15초 후) 다시 리셋하면 완전히 깨끗하게 100으로 복구되는 것도 확인. **완전 차단(세대 기반 무효화)은 범위 초과로 보류** — 사용자가 A안의 이 트레이드오프를 인지하고 선택.
- **테스트**: `queue.service.spec.ts`(+1, `purge`) · `demo.service.spec.ts`(+1, 리셋 시 대기열 비움 / stats 테스트에 `soldOutCount` 포함) · `reservations.service.spec.ts`(재고부족 테스트에 soldout 카운터 단언 추가). **API 73→75 그린.** 프론트 테스트 갱신(신규 타일 값 확인). **web 15개 유지.**
- **부수 작업**: 사용자가 자기 터미널에서 `nvm: command not found`를 만나 원인을 확인 — `~/.nvm/nvm.sh`는 있었는데 `~/.zshrc`가 그걸 source하는 코드가 없었다. 사용자 동의 후 `~/.zshrc`의 `# NVM` 주석 아래에 `NVM_DIR` export + `nvm.sh` source 3줄을 추가해 실제 터미널에서 `nvm use v22.23.1`이 정상 동작하는 것까지 확인.
- **다음**: 배포 6단계. PRD 갭·발견된 버그 3건 모두 처리 완료.

## 2026-08-06 · 이벤트 목록 별도 페이지 분리 + 결제 TTL 30초 + `demo` 도메인 개념 정리

- **`demo` 도메인 질문**: 사용자가 "`demo`가 테스트/프로덕션을 가르는 경계가 아니라 가상유저 플로우를 의미하는 거라면 무방하다"고 확인 요청. `app.module.ts`에 `DemoGateGuard`가 전역 가드로 박혀 있고 `DemoModule`이 배포 서버 자체에서 상시 동작하는 걸 근거로 확인 — 이 프로젝트는 "데모와 분리된 진짜 프로덕션"이 따로 없고 **배포된 사이트 자체가 데모**라, `demo`는 환경 분기가 아니라 "가상유저 시뮬레이션+게이트+stats+리셋" 기능 도메인의 이름이 맞다고 결론. 리네이밍은 안 함(비용 대비 이득 적음, 나중에 README에 이 정의만 한 줄 남기면 충분).
- **결제 TTL 30초로 변경**: PRD 원안은 5분이지만 방문자가 만료를 체감하기엔 너무 길다는 판단(사용자 요청) — `reservations.service.ts`의 `HELD_TTL_MS`를 `5 * 60 * 1000` → `30 * 1000`으로 변경. **부수 조정**: sweep 주기(`SWEEP_INTERVAL_MS`)가 기존 30초 그대로면 "30초 후 반환"이 최악의 경우 최대 60초까지 늘어져 체감이 어긋난다 — TTL과 비슷하거나 넓은 sweep 주기는 원래 설계 의도("TTL보다 훨씬 촘촘")를 깨는 것이라 5초로 같이 좁혔다. 실서버로 HELD 생성 후 결제 없이 방치 → 정확히 heldUntil(30초 뒤) 근처에서 sweep이 회수해 DB는 EXPIRED, Redis 재고는 원복되는 것 확인(생성 01:31:22 → 회수 확인 01:31:56, TTL+sweep 텀 이내).
- **이벤트 목록을 별도 페이지로 분리 + 판매중만 진입 가능**: 지금까지 `event-list.tsx`는 데모 대시보드 안에 인라인으로 박혀 있었고, 클릭해도 아무 동작이 없었다(어떤 이벤트를 클릭하든 `booking-form.tsx`는 항상 `isDemo` 이벤트로 예매를 진행 — 목록의 선택과 예매 대상이 서로 무관했음). 사용자 요청으로 실제 내비게이션을 만들었다:
  - 신규 라우트 `app/events/page.tsx`(목록, `EventList` 렌더) + `app/events/[id]/page.tsx`(상세, `useParams()`로 id를 읽어 `GET /events/:id` 조회 후 `status==='ON_SALE'`일 때만 `BookingForm`을 렌더 — 아니면 "지금 판매중이 아닙니다" 안내).
  - `event-list.tsx`: `ON_SALE` 카드만 `next/link`로 `/events/{id}`에 연결, 나머지는 그대로 클릭 불가 div.
  - `booking-form.tsx`: 더 이상 `/events`를 스스로 조회해 `isDemo` 이벤트를 찾지 않는다 — 호출부(`[id]/page.tsx`)가 이미 확인한 `eventId`/`eventTitle`을 props로 받는다(불필요한 자체 탐색 제거, 진입 시점에 이미 검증된 이벤트라 다시 찾을 이유가 없음).
  - `demo-dashboard.tsx`: 인라인 `EventList`/`BookingForm`을 제거하고 "이벤트 목록 보기 →" 링크로 대체 — 대시보드는 이제 순수하게 관측/운영 패널(stats·시뮬레이션·리셋)이고, 예매는 이벤트 상세 페이지에서만 일어난다.
  - **회귀 발견·수정**: `/events/[id]`가 독립 라우트가 되며 로그인 전 상태(게이트만 통과)로도 도달 가능해졌다 — 예전엔 대시보드(로그인 후에만 렌더)를 통해서만 `BookingForm`에 닿을 수 있어서 문제가 안 됐지만, 지금은 로그인 안 된 방문자가 "대기열 입장"을 눌러도 서버가 401을 주는데 `handleJoinQueue`가 응답 성공 여부를 확인 안 해서 화면은 태연히 "대기 중입니다"를 보여주는 버그가 될 뻔했다. `res.ok` 체크를 추가해 실패 시 에러 문구+재시도 버튼을 보여주도록 고침.
- **테스트**: `event-list.test.tsx`(+1, ON_SALE만 링크) · 신규 `events/[id]/page.test.tsx`(3, ON_SALE 진입/비ON_SALE 차단/404) · `booking-form.test.tsx`(props 방식으로 전면 수정 +1, 401 회귀 테스트) · `demo-dashboard.test.tsx`(EventList/BookingForm mock 제거, "이벤트 목록 보기" 링크 테스트로 교체) · `page.test.tsx`(대시보드 분기 mock 단순화). **web 15→21 그린.** 백엔드는 TTL 상수만 바꾼 것이라 테스트 수 불변, **API 75개 유지**(sweep/reconcile spec은 heldUntil을 직접 세팅해 상수값에 안 의존).
- **다음**: 배포 6단계(Dockerize → CI/CD → VM+Nginx → Grafana 관측 → 최종 k6 리포트 → README/회고).

## 2026-08-06 · 예매·판매현황 화면 재통합 + "포기(abandoned)" 지표 신설

- **계기**: 직전 작업에서 이벤트 목록을 별도 페이지로 분리하며 예매(BookingForm)와 판매현황(DemoDashboard)도 함께 떼어놨는데, 사용자가 실제로 `/events/*`에 들어가보니 대기열 입장 UI만 보이고 판매현황·가상유저투입이 안 보여서 "제대로 테스트가 안 된다"고 지적. 같은 시점에 캡처와 함께 "200명 투입했는데 재고소진(121)+결제성공(10)이 200과 안 맞는다"는 두 번째 이슈도 제기.
- **화면 재구성 방향**: 사용자 피드백의 핵심은 "내가 누른 예매 결과를 그 자리에서 지켜봐야 테스트가 의미 있다"는 것 — 예매와 관측(stats)을 분리한 게 오히려 테스트를 방해했다. `BookingForm`과 `DemoDashboard`를 다시 한 화면에 두되, 위치는 예전(루트 대시보드)이 아니라 **`/events/[id]`(이벤트 상세)**로 옮겼다 — 이벤트가 하나뿐인 데모라 stats도 사실상 이 이벤트 전용이라 자연스러운 배치. 루트 `/`는 게이트+로그인 통과 후 직접 화면을 그리지 않고 `useRouter().replace('/events')`로 넘긴다 — 목록 렌더링 코드가 root와 `/events` 두 곳에 중복되는 걸 피하기 위함(이벤트 목록 렌더링의 단일 소스는 `event-list.tsx` 하나).
- **숫자 불일치 진단**: 사용자가 "200명인데 왜 재고소진+결제성공 합이 200이 아니냐"고 물어서 코드를 추적해보니 원인이 세 겹이었다.
  1. **입장 허가를 받고도 확률적으로 포기하는 20%(`DEMO_SIM_ABANDON_PROBABILITY`)가 어떤 카운터에도 안 잡힌다** — `reservations.create()` 자체를 호출 안 하니 재고소진(soldOutCount)에도 안 걸리는, 완전히 투명한 버킷이었다.
  2. **더 미묘한 두 번째 투명 버킷**: 가상 유저의 랜덤 사전지연(최대 10초, `DEMO_SIM_MAX_BOOKING_DELAY_MS`)이 입장 허가창(기본 8초, `QUEUE_ADMISSION_WINDOW_MS`)보다 길 수 있어, 그 사이 허가가 자연 만료되면 `assertAdmitted`가 `ForbiddenException`을 던지고 `simulateBookingAttempt`의 catch가 이걸 재고소진(`ConflictException`)과 같이 "조용히 넘어간다"고 뭉뚱그려 처리하는데, 코드 주석은 "포기(허가창 만료 포함)"라고 이미 이 둘을 같은 개념으로 취급하고 있었음에도 **집계는 안 하고 있었다** — 주석과 실제 동작이 어긋나 있던 셈.
  3. 200명은 입장 허가(20명/2초)+랜덤 지연 처리에 최대 30초 걸리는데 캡처는 그 중간 시점이었고, 모든 카운터가 이번 라운드가 아니라 **리셋 이후 누적치**라 혼란을 더했다.
- **수정**: `soldout:event:{id}`를 만들 때 세운 패턴 그대로 `abandoned:event:{id}` Redis 카운터를 신설. `simulateBookingAttempt()`의 확률적 포기 분기와, catch 블록의 `ForbiddenException`(허가창 만료) 분기 — 둘 다 여기 누적(`ConflictException`/재고소진은 이미 `createHeld()`에서 별도로 세고 있어 그대로 둠). `DemoStats.abandonedCount` 필드 추가, `resetDemoEvent()`가 0으로 리셋, 프론트 `demo-dashboard.tsx`에 "포기(미시도)" 타일 추가.
- **실서버로 등식 검증**: 리셋 후 40명 투입 → 대기열(`queues:active`)이 빌 때까지 + 최대 지연(10초) 여유까지 기다린 뒤 최종 스냅샷 확인 → `paidCount(27)+failedCount(1)+soldOutCount(0)+abandonedCount(12) = 40`, 투입 인원수와 정확히 일치. 수정 전에는 같은 방식으로 재본 40명 라운드가 `16+9+0+9=34`로 6명이 비어 있었다(그 6명이 바로 위 2번 버킷, 허가창 만료).
- **테스트**: `demo.service.spec.ts`의 기존 포기·허가창만료 테스트 2건에 `redis.get(abandonedKey())` 단언 추가, stats 스냅샷 테스트에 `abandonedCount` 포함(신규 `it` 없이 기존 보강이라 **API 75개 유지**). 프론트: `demo-dashboard.test.tsx`(SSE 스냅샷에 abandonedCount 포함) · `events/[id]/page.test.tsx`(DemoDashboard가 `useMutation`을 쓰므로 `QueryClientProvider`로 감싸는 `renderPage()` 헬퍼 추가, ON_SALE 케이스에서 "실시간 판매 현황"도 함께 뜨는지 확인) · `page.test.tsx`(`next/navigation`의 `useRouter`를 `vi.mock`으로 대체해 `/events`로 `replace` 호출되는지 검증 — 예전엔 대시보드 렌더 결과를 직접 봤지만 이제 리다이렉트만 하므로 방식을 바꿈). **web 21개 유지**(기존 테스트 보강 위주).
- **부수 발견(회귀 아님, 확인만)**: `/events/[id]`가 로그인 전에도 도달 가능해진 채로 있어(직전 세션에서 `handleJoinQueue`의 401 미처리는 이미 고쳤음), 이번엔 `DemoDashboard`가 함께 렌더되며 `useMutation`이 `QueryClientProvider` 없이 쓰이면 크래시한다는 걸 테스트 작성 중 발견 — 실제 앱은 `RootLayout`이 이미 `Providers`로 감싸므로 문제없지만, 격리된 단위 테스트에서는 직접 감싸야 한다는 걸 놓쳤던 것(테스트 인프라 이슈, 프로덕션 코드는 무관).
- **다음**: 배포 6단계. PRD 갭·발견된 버그 모두 처리 완료, 데모 스탯 정합성도 실측으로 검증됨.

## 2026-08-06 · "입장 대기중" 지표 추가

- **계기**: 사용자가 "대기열 진입 전 가상 유저 수도 집계할 수 있나요"라고 질문. 파이프라인을 다시 보니 두 가지 다른 해석이 가능했다 — ① "요청은 했지만 아직 대기열에 join도 안 된 수"(투입 배치가 20명/300ms로 나가는 진행률), ② "이미 대기열엔 들어갔지만 아직 입장 허가를 못 받은 수"(ADR 0017 Sorted Set의 크기). 구현 비용과 개념이 서로 꽤 다르므로 침묵하지 않고 두 옵션을 사용자에게 제시 → **②**로 확정.
- **구현**: `QueueService`에 `size(eventId)`(`ZCARD queue:event:{id}`) 메서드 신규 추가 — `deactivateIfEmpty()`가 이미 같은 걸 내부적으로 하고 있어서 그 패턴을 그대로 공개 메서드로 뽑은 것. `DemoStats.admissionQueueCount`로 노출. 리셋 시 별도 처리 불필요 — `QueueService.purge()`가 이미 이 ZSet 자체를 지우므로 `size()`는 그냥 0을 반환하게 된다.
- **네이밍 함정**: 기존 `queueBacklog` 필드가 이미 있어서 헷갈리기 쉬웠는데, 그건 BullMQ의 `confirm` 큐(결제 확정 job)이고 이번에 추가한 건 Redis Sorted Set 기반 입장 대기열로 **완전히 다른 큐**다. 주석과 필드명(`admissionQueueCount`)에 이 구분을 명시했다.
- **구현 도중 사용자가 던진 별도 질문**: "재고 잔량이 있는데도 포기(미시도)가 뜨는 건 왜?" — 확인해보니 `abandonedCount`는 재고와 완전히 무관하게 두 경로로만 증가한다: ① 확률적 포기(기본 20%, `demo.service.ts:275`)는 예매 시도 자체를 안 하므로 재고 확인 단계에도 못 간다. ② 입장 허가창(8초)보다 랜덤 사전지연(최대 10초)이 길게 뽑히면 허가가 자연 만료돼 `ForbiddenException`으로 막히는데(`demo.service.ts:299`), 이것도 순전히 타이밍 문제라 재고와 무관하다. 재고가 넉넉해도 이 두 경로는 항상 일정 비율로 발생하는 게 정상 — 사용자에게 코드 위치와 함께 설명.
- **실서버 검증**: 리셋 후 40명 투입 1초 후 스냅샷 → `admissionQueueCount:20, abandonedCount:4`. 정확히 첫 배치 20명만 허가 처리되고 나머지 20명이 대기 중이며, 허가받은 20명 중 4명(정확히 20%)이 벌써 포기 판정까지 끝난 상태 — 파이프라인의 각 단계가 실시간으로 눈에 보이는 것을 확인.
- **테스트**: `queue.service.spec.ts`(+1, `size` — join 2명 후 1명 popNext로 빼서 나머지 1명만 남는지 확인). `demo.service.spec.ts`의 stats 스냅샷 테스트에 실제 `QueueService.join()`을 호출해 대기 인원을 만들고 `admissionQueueCount` 단언 추가(신규 `it` 없이 기존 보강, **API 75→76**은 `size` 테스트 1건 추가분). 프론트 `demo-dashboard.test.tsx`(SSE 스냅샷에 admissionQueueCount 포함 + "입장 대기중" 타일 텍스트 확인). **web 20개 유지**(기존 테스트 보강 위주).
- **다음**: 배포 6단계. 데모 스탯이 이제 파이프라인 전 구간(대기열→HELD→결제→확정, 그리고 포기·재고소진 두 이탈 경로까지)을 실시간으로 빠짐없이 보여준다.

## 2026-08-07 · 환경 정리(nvm/스크립트) + 방문 즉시 자동 대기열 입장 + 전체 커밋

- **레이아웃 지시**: 가상 유저 투입 버튼 클릭 시 입력값 0 초기화, "입장 대기중"을 스탯 그리드에서 빼 버튼 옆 별도 UI로, 스탯 그리드를 3줄(재고/HELD/CONFIRMED · 결제성공/결제실패/포기 · 재고소진/큐적체)로 재배치 — 그대로 구현, `demo-dashboard.tsx`/`.test.tsx` 수정.
- **"50 vs 200" 숫자 재검증**: 사용자가 "50 가상유저 투입→...→100 가상유저 투입" 흐름 캡처를 주며 재고소진(22)+결제성공(96) 등이 이상해 보인다고 확인 요청. DB(`sim-` 유저 count)·Redis 카운터를 직접 조회해보니 실제 투입량은 200명(사용자 기억 150명과 차이)이었고, `96+12+22+71=201=200(가상)+1(실제)`로 **정확히 일치** — 버그 아님, 실측으로 결론. `confirmedCount`가 건수가 아니라 수량 합이라는 단위 차이도 함께 짚음. 검증 후 이 세션 전체 작업(대기열+결제+버그수정+지표+화면구성, 55개 파일)을 한 커밋(`c8275a7`)으로 정리 — 파일들이 여러 기능이 겹겹이 섞여 있어 기능별로 잘게 쪼개는 건 현실적으로 위험(중간 상태가 컴파일/테스트를 보장 못 함)하다고 판단해, 리포 기존 관행(큰 기능 단위 통짜 커밋)을 따름.
- **환경 질문 2건 처리**:
  1. "Node 버전을 IDE 켤 때마다 맞춰야 하나" — `nvm alias default`만으론 새 셸에 자동 적용 안 됨을 직접 실측(`zsh -ic 'node -v'`로 확인)한 뒤, `~/.zshrc` **맨 끝**에 `nvm use default --silent` 배치로 해결. 처음엔 파일 중간(NVM 초기화 블록 바로 뒤)에 넣었다가 그 아래 있던 하드코딩 PATH 줄에 덮어써져 무효화되는 걸 다시 실측으로 발견 — "모든 PATH 조작이 끝난 뒤"라는 위치 조건이 핵심이었다.
  2. "docker:up은 스크립트인데 왜 `cd infra`가 필요했나" — `docker compose`가 pnpm 워크스페이스와 무관한 별도 CLI라 `-f` 옵션 없이는 cwd의 파일만 본다는 걸 설명. 루트 `package.json`에 `docker:up`/`docker:down`/`dev:api`/`dev:web` 4개 스크립트 추가, 각 옵션(`-f`/`-d`/`--wait`)의 의미와 "왜 어떤 옵션은 짧고 어떤 건 긴 형태만 있는지"(`-w`가 이미 `docker run`의 `--workdir`로 예약돼 있어 `--wait`엔 못 씀 — 실측으로 확인)까지 상세 설명.
- **방문 즉시 자동 대기열 입장 + 랜덤 크라우드**: 사용자가 "이벤트 상세에 들어가도 어떻게 테스트할지 직관적이지 않다"고 지적하며, "이벤트를 선택하는 순간 자동으로 대기열+경쟁 상황에 놓이면 좋겠다, 단 순번을 가짜로 랜덤화하는 게 아니라 앞에 서는 인원수를 랜덤화해서 결과적으로 순번이 달라지게" 요청. ADR 0017에서 이미 기각했던 "가상 유저 순번 랜덤화"와 원칙적으로 다르다는 걸 사용자가 먼저 명확히 구분해줌.
  - `booking-form.tsx` 마운트 시 자동으로 ① 5~40명 랜덤 규모 가상 유저 fire-and-forget 투입 ② 방문자 본인 대기열 자동 입장. 수동 "대기열 입장" 버튼 제거.
  - **린트 삽질**: `useEffect` 안에서 setState하는 `handleJoinQueue()`를 직접 호출했더니 `react-hooks/set-state-in-effect`(React 19/Next 16 신규 규칙)에 걸림 — "effect에서 setState 함수를 직접 부르지 말고, 외부 이벤트(fetch 응답)에 반응하는 콜백 안에서 setState하라"는 게 요지. `apiFetch(...).then(async (res) => setState(...))` 형태로 인라인해 해결(동작 동일).
  - `events/[id]/page.tsx`에 좌우 2단 레이아웃(`lg:flex-row`) 추가.
  - 테스트: `booking-form.test.tsx` 전면 개정(자동 입장 전제 + `/demo/simulate` 공통 mock), 신규 1건. **web 20→21 그린.**
  - **실서버 e2e(Playwright)**: 클릭 없이 자동으로 "대기 중입니다 — 현재 순번 0" 표시 + 좌우 레이아웃 스크린샷 확인. 콘솔에 429가 한 번 찍혔는데 직전 테스트의 쿨다운이 안 풀려 이번 회차 크라우드 투입만 스킵된 것(대기열 입장 자체는 정상) — 버그 아님으로 결론.
- **다음**: 배포 6단계.

## 2026-08-07 · 자동 크라우드 투입 전용 쿨다운 분리

- **계기**: 사용자가 직접 재현 — "뒤로 가기 후 재입장하면 새 크라우드가 안 들어가고 바로 0번으로 대기열에 들어간다. 대기열 초기화+재투입 플로우가 빠졌나?" 확인해보니 두 가지가 겹친 정상 동작이었다: ① `AdmissionProcessor`가 2초마다 20명씩 허가 처리해 5~40명 규모 크라우드는 2~4초 안에 대기열에서 다 빠짐(사람이 뒤로 가기 누르는 속도보다 빠름) ② `POST /demo/simulate`의 기존 30초 쿨다운(수동 "가상 유저 투입" 버튼용으로 설계됨)을 자동 투입도 그대로 공유해서, 30초 안에 재입장하면 새 크라우드 자체가 안 들어감. "대기열을 마운트마다 초기화"하는 로직은 애초에 계획에 없었다고 답변 — 대기열은 그 순간 실제로 대기 중인 모든 방문자가 공유하는 자원이라 마운트마다 초기화하면 다른 사람 순번을 지우는 부작용이 생긴다.
- **해결책 선택**: "자동투입 전용 짧은 쿨다운"(추천) vs "전역 쿨다운 자체를 단축" vs "그대로 둠" 세 옵션을 제시 → 사용자가 전자를 선택. 수동 버튼(최대 300명 요청 가능, 오남용 방지가 더 중요)의 30초는 그대로 두고, 마운트 자동 투입만 훨씬 짧게(3초) 별도 쿨다운을 쓰기로.
- **구현**: `SimulateDto`에 `auto?: boolean` 추가. `DemoService.simulateLoad(count, auto=false)`가 `auto` 여부에 따라 **다른 Redis 키**(`demo:sim:cooldown` vs `demo:sim:auto-cooldown`)와 **다른 쿨다운 값**(`DEMO_SIM_COOLDOWN_MS`=30000 vs 신규 `DEMO_SIM_AUTO_COOLDOWN_MS`=3000)을 사용하도록 분기. `booking-form.tsx`의 마운트 시 자동 투입 호출에 `auto: true` 추가.
- **테스트**: `demo.service.spec.ts`에 신규 테스트 1건 — 수동 쿨다운을 건 상태에서 `auto=true` 호출은 안 막히고, 그 반대(auto 쿨다운 건 상태에서 수동 호출)도 안 막히는 것을 확인(서로 완전히 독립). **API 76→77 그린.**
- **실서버 검증**: `auto=true`로 연속 2회 즉시 요청 → 1번째 성공, 2번째 429(3초 쿨다운) → 3초 대기 후 3번째 성공. 수동 요청 직후 곧바로 auto 요청도 성공(서로 안 막음) 확인.
- **다음**: 배포 6단계.

## 2026-08-07 · 자동 투입 시 방문자가 항상 0번을 받던 순서(race condition) 버그 수정

- **계기**: 사용자가 "테스트해보니 항상 사용자 대기열 순번이 0인 것 같음. 사전 투입 가상유저 수를 좀 더 늘려야 할까요?"라고 질문 — 표면적으로는 "크라우드가 부족해서"로 보이는 증상이었지만, 코드를 추적해보니 인원수와 무관한 구조적 순서 문제였다.
- **원인**: `booking-form.tsx`의 마운트 effect가 `/demo/simulate`(가상 유저 투입)와 방문자 본인의 `/events/:id/queue`(대기열 입장) 두 요청을 서로 기다리지 않고 각각 fire-and-forget처럼 거의 동시에 쏘고 있었다. `/demo/simulate`는 ADR 0016 설계상 202를 **즉시** 반환하고 실제 투입(User row INSERT + Redis ZADD)은 그 응답 이후 백그라운드에서 진행되는 반면, 방문자 본인의 입장은 이미 로그인된 유저라 DB write 없이 **ZADD 한 번**뿐이라 훨씬 가볍고 빠르다. 그 결과 "가벼운 작업"(방문자)이 "무거운 작업"(가상 유저 대량 INSERT+ZADD)보다 항상 먼저 끝나버려, 크라우드 규모를 아무리 키워도 방문자는 늘 0번을 받았다 — **문제는 수가 아니라 순서**였다.
- **수정**: `DemoService.simulateLoad(count, auto)`가 `auto=true`일 때는 신규 상수 `AUTO_JOIN_GUARANTEE_MAX`(100명)까지의 대기열 입장을 **응답 전에 동기로 완료**하도록 바꿨다(그 이상은 기존처럼 백그라운드). `booking-form.tsx`도 `/demo/simulate` 응답을 받은 **다음에야** 방문자 본인의 `/events/:id/queue`를 호출하도록 `.then()` 체이닝으로 순서를 강제 — 두 수정이 짝을 이뤄야 실제로 순서가 보장된다(백엔드만 고쳐도 프론트가 여전히 동시에 쏘면 소용없고, 반대도 마찬가지).
- **후속 조정**: 처음엔 기존 `SIM_BATCH_SIZE`(20, 수동 대량 시뮬레이션의 "대시보드에서 관찰 가능한 리듬"용 상수)를 재사용해 상한을 20으로 뒀는데, 사용자가 "시뮬레이션 첫 배치를 최대 100으로 늘릴게요"라고 요청 — `SIM_BATCH_SIZE` 자체를 100으로 바꾸면 수동 버튼(최대 300명)의 관찰 리듬까지 함께 바뀌어버리는 부작용이 있어, 목적이 다른 별도 상수 `AUTO_JOIN_GUARANTEE_MAX=100`으로 분리했다. 현재 프론트의 자동 투입 랜덤 범위(5~40명)가 전부 이 상한 안에 들어오므로, 사실상 매번 크라우드 전원이 방문자보다 먼저 대기열에 서게 된다.
- **테스트**: `demo.service.spec.ts` 신규 1건 — `simulateLoad(5, true)`가 resolve된 직후(추가 대기 없이) `queueService.size(eventId)`가 정확히 5를 반환하는지 확인(응답 시점에 이미 전원이 대기열에 서 있어야 한다는 계약을 그대로 검증). **API 77→78 그린.**
- **실서버 검증**: `simulate(20명, auto)` 호출이 0.136초 만에 응답했고, 그 직후 `ZCARD`로 확인한 대기열 크기가 정확히 20 → 방문자 본인이 join하니 정확히 **20번**을 받음(수정 전엔 크라우드 규모와 무관하게 항상 0번이었음). 90명(100 상한 이내)도 0.138초 만에 전원 동기 처리되는 것 확인.
- **다음**: 배포 6단계.

## 2026-08-07 · "여전히 0번" — 진짜 원인은 React StrictMode 이중 마운트

- **계기**: 백엔드 순서 보장(위 항목)까지 붙였는데도 사용자가 "가상 투입 유저를 100으로 늘렸는데도 여전히 0번, 대기열이 45로 찍혀도 0번"이라고 재현. 직전 세션에서 `curl`로 백엔드 로직만 단독 검증했을 땐 정확히 의도대로(20명 투입 → 20번 수신) 동작했는데, 실제 브라우저에서만 재발한다는 게 결정적 단서였다 — "백엔드는 옳은데 프론트에서만 깨진다"는 뜻이고, 둘의 차이는 **브라우저가 React를 개발 모드로 돌린다**는 것뿐이었다.
- **원인**: Next.js App Router는 개발 모드에서 `reactStrictMode`가 기본 `true`다(Next 13.5.1부터). StrictMode는 부수효과(side effect) 버그를 잡아내려고 컴포넌트 마운트 시 **모든 effect를 의도적으로 두 번 실행**한다(마운트→클린업→재마운트를 초기에 한 번 더 돈다). 가드 없이 짠 크라우드투입+본인입장 effect가 이 두 번째 실행에 취약했다:
  1. **1번째 실행**: `/demo/simulate` 요청이 자동 쿨다운(3초) 락을 획득 → 최대 100명의 실제 대기열 입장을 기다림(수백 ms 걸림).
  2. **2번째 실행**(StrictMode 전용): `/demo/simulate` 요청이 방금 걸린 쿨다운에 **즉시 거부**(429, `.catch()`로 조용히 삼켜짐) → 기다릴 크라우드가 없으니 그대로 이어서 본인의 `/events/:id/queue`를 호출.
  3. JS 이벤트 루프상 2번째 실행의 "즉시 거부→바로 join" 체인이 1번째 실행의 "수백 ms 기다림→join" 체인보다 먼저 완료되는 게 사실상 항상 보장됨 — 결국 본인이 크라우드보다 **먼저** 큐에 서버린다. `queueService.join()`의 ZADD가 NX(최초 타임스탬프만 유지)라 이 "새치기"가 그대로 영구 순번으로 굳어버렸다.
  4. 그 결과: 크라우드는 뒤늦게 45명이든 100명이든 들어와 "입장 대기중" 스탯엔 정직하게 찍히지만, 이미 그보다 앞서 등록된 방문자 본인의 순번은 항상 0에 가까웠다 — 정확히 사용자가 관찰한 "대기열 다수 찍혀도 0번" 현상과 일치.
- **수정**: `booking-form.tsx`에 `const hasStartedRef = useRef(false)` 가드를 추가 — effect 맨 앞에서 `if (hasStartedRef.current) return; hasStartedRef.current = true;`로 실질 실행을 1회로 강제한다. ref는 클린업 함수로 리셋되지 않으므로 StrictMode의 "클린업 후 재마운트"를 그대로 통과해 두 번째 호출을 완전히 무력화한다 — analytics 1회 호출, 결제 트리거 등 "정확히 한 번만" 실행돼야 하는 마운트 effect에 흔히 쓰이는 표준 패턴.
- **겸사겸사 처리한 요청**: 사용자가 "가상 투입 유저를 최대 100으로 늘릴게요"라고 하여, 프론트 랜덤 크라우드 범위를 5~40에서 **5~100**으로 확대(`Math.floor(Math.random() * 96) + 5`) — 백엔드의 `AUTO_JOIN_GUARANTEE_MAX=100`과 정확히 맞아떨어져, 이제 매번 크라우드 전원이 확실히 방문자보다 먼저 대기열에 선다.
- **테스트**: `booking-form.test.tsx`에 실제로 `<StrictMode>`로 감싸 렌더링하는 회귀 테스트 신규 추가 — 이중 마운트를 재현한 뒤 `/demo/simulate`·`/events/1/queue` 호출이 각각 정확히 1번씩만 나갔는지 확인(가드 없이 되돌리면 이 테스트가 즉시 실패하도록 설계). **web 21→22 그린.** 참고로 이 버그는 RTL의 기본 `render()`로는 재현이 안 된다 — RTL은 StrictMode로 안 감싸므로, 이 버그를 잡으려면 테스트에서도 명시적으로 `<StrictMode>`를 씌워야 했다.
- **실서버 검증**: 이번엔 `curl` 단독 검증이 아니라 **실제 dev 서버**(StrictMode가 실제로 켜진 상태)를 Playwright로 직접 조작 — 게이트→로그인→이벤트 진입 후 방문자가 "99명 중 78번"을 받는 것으로 확인. 콘솔 에러 0.
- **교훈**: React StrictMode의 이중 effect 실행은 프로덕션 빌드에선 안 일어나지만 **개발 서버에선 상시 활성화**돼 있어, 실제 사용자(사용자 본인)가 매번 겪는 문제였다. 이번 사례처럼 "백엔드 로직만 curl로 검증했을 때 안 보이던 버그가 실제 브라우저에서만 재발"하면, 프론트엔드 쪽 — 특히 마운트 시 네트워크 부수효과가 있는 `useEffect` — 의 StrictMode 내성을 가장 먼저 의심해야 한다.
- **다음**: 배포 6단계.

## 2026-08-07 · "목록↔상세 반복 재진입 시 즉시 허가" 버그 — 이전 admitted 키가 안 지워짐

- **계기**: 사용자가 스크린샷과 함께 재현 — "첫 진입 후 올바르게 대기열을 부여받고 예매 가능한 상태에서, 이벤트 목록으로 돌아갔다가 다시 상세에 진입하면 가상유저 대기열이 수십 개 쌓여있어도 대기 없이 바로 예매 인풋이 표시된다. 목록↔상세를 반복하면 두 상황이 번갈아 나오는데, 그것도 1:1 비율은 아니다."
- **추측하지 않고 두 가설을 직접 재현해 검증**: 후보가 두 개였다 — ① Next.js의 클라이언트 라우터 캐시가 뒤로가기/재방문 시 이전 컴포넌트 인스턴스(state·ref)를 그대로 재사용하는 경우, ② 이전 방문에서 받은 입장 허가(`admitted` 키, TTL 8초)가 재입장 시점에도 아직 살아있어 그대로 응답에 실리는 경우. `next dev`의 캐싱 모델(Cache Components 문서, `staleTimes`, Client Cache 용어집)을 먼저 확인했으나 이 프로젝트는 `cacheComponents`를 켜지 않았고 해당 페이지는 `"use client"` 순수 클라이언트 렌더링이라 ①은 근거가 약했다. `curl`로 직접 재현: 실사용자로 ①입장 허가 받기 → ②다른 가상유저 40명 즉시 투입(재입장 시 앞에 서야 할 사람들) → ③곧바로 재입장(`join` 재호출) → 결과 `{"rank":20,"admitted":true}`. **새 순번(20)은 정상 부여됐는데, 이전 허가(`admitted:true`)가 같이 살아남아 응답에 실린 것**을 그대로 확인 — ②가 진짜 원인이었다.
- **원인**: `QueueService.join()`은 대기열 Sorted Set에 ZADD만 할 뿐, 그 사용자의 `admitted:event:{id}:{userId}` 키는 전혀 안 건드렸다. `booking-form.tsx`의 SSE 콜백은 `admitted`를 `rank`보다 먼저 검사하므로(`if (snapshot.admitted) {...} else if (snapshot.rank !== null) {...}`), 새로 부여된 rank=20이 있어도 무시하고 곧장 "허가됨" 화면으로 튀었다. "번갈아 나오고 1:1이 아니다"라는 관찰도 이걸로 설명된다 — 재진입 시점이 이전 허가의 8초 TTL 안에 들면 버그가 재현되고, TTL이 이미 끝난 뒤 재진입하면 정상적으로 다시 대기하니 재현이 안 된다. 사용자의 클릭 리듬이 이 8초 창을 왔다갔다 걸치면서 불규칙하게 두 결과가 섞여 나온 것.
- **수정**: `join()`이 ZADD하기 전에 그 사용자의 admitted 키를 먼저 `DEL`하도록 한 줄 추가 — "재입장 = 허가 상태 초기화 + 새 순번 부여"로 의미를 통일했다.
- **테스트**: `queue.service.spec.ts` 신규 1건 — 허가(`admit`) 후 재입장(`join`)하면 이전 허가가 사라지고 새 순번으로 다시 대기 상태가 되는지 확인. 처음 작성할 때 `admit()`만 부르고 `popNext()`를 빼먹어 테스트 자체가 실패했는데(실제 흐름은 `popNext`로 큐에서 뺀 뒤 `admit`), 이건 내 테스트 설정 실수였지 프로덕션 버그가 아니었다 — `popNext` 추가해 바로잡았다. **API 78→79 그린**(같은 실행에서 무관한 `admission.processor.spec.ts`의 배치 카운트 테스트가 1회 flake 났으나 재실행 시 79/79 — 이전에도 문서화된 것과 같은 패턴, 이번 수정과 무관).
- **실서버 검증**: Playwright로 실제 게이트→로그인 후 이벤트 목록↔상세를 5회 반복 재진입 → 즉시허가로 튄 경우 **0/5**(수정 전엔 재현 가능했음). 반복 중 자동 쿨다운(3초) 429가 몇 번 찍혔지만 그때도 대기열 재입장 자체는 정상 처리됨(설계대로).
- **다음**: 배포 6단계.

## 2026-08-07 · 자동 대기열 입장 — "제품 관점 정답은 아님"을 ADR에 정직하게 기록

- **논의**: 사용자가 "실제 대량 트래픽 티케팅 순번 부여·예매 구조가 테스트 편의 때문에 잘못된 방향으로 가고 있는 건 아닌지" 우려를 제기. 항목별로 점검한 결과, 핵심 파이프라인(FIFO 대기열·HELD·BullMQ 확정·SSE·모의 결제)과 대부분의 데모 장치(랜덤 크라우드 투입, 이중 쿨다운)는 `DemoService`에 깔끔히 격리돼 있어 문제가 없었다. 다만 **"페이지 진입 즉시 자동으로 대기열에 입장"**하는 동작만은 실제 티케팅 서비스라면 "구경"과 "구매 대기열 참여"를 구분해야 정상인데, 이 프로젝트는 그 둘을 구분 없이 합쳐뒀다는 점을 인정 — 동기가 PRD 요구사항이 아니라 "포트폴리오 데모를 처음 보는 사람이 헤매지 않게" 하려는 테스트 편의였음을 확인.
- **결정**: 그래도 유지하기로 함 — 버튼 클릭 방식으로 되돌리면 캐주얼한 방문자가 버튼을 안 누르고 이탈해 "가상 유저만 있고 실제 방문자는 한 번도 안 들어와본 대기열"이 되기 쉽고, 이 프로젝트의 핵심 학습 포인트(선착순 경쟁 체감)를 놓치게 된다. 매출이 걸린 실서비스가 아니라 동시성 학습·시연이 목적이므로 "구경 vs 구매 의도" 구분의 실익이 없다고 판단.
- **기록**: `docs/decisions/0017-admission-queue.md`에 "제품 관점 정답은 아닌 의도적 예외"라는 개정 섹션을 추가 — 나중에 이 코드만 보고 "왜 대기열이 클릭 없이 자동 시작되지?"라고 물을 사람(면접관 포함)을 위한 솔직한 기록.
- **부수**: 사용자가 Redis `ZADD`를 학습한 적 없다고 해 개념(정렬된 집합에 멤버+점수를 추가/갱신하는 원자 연산, 대기열의 "입장 시각을 점수로 써서 순서를 유지"하는 용도)을 별도로 설명.

## 2026-08-07 · 유저별 데모 격리 + 예매를 티켓 카드로 시각화

- **계기**: 두 기기(회사/집)에서 동시에 데모를 테스트하는 이 프로젝트의 재현성 요구(§9)와, 데모 이벤트를 `isDemo:true` 하나만 모든 방문자가 공유하는 기존 구조가 정면충돌한다는 걸 사용자가 실측으로 확인 — 한쪽에서 리셋·가상유저 투입을 누르면 다른 쪽 테스트 데이터까지 흔들림. 같은 대화에서 사용자가 스크린샷(PAID 50/FAILED 13/포기 33 등)을 보여주며 "예매하기를 눌러도 HELD가 하나 올라가는 정도로는 안 와닿는다, 티켓 카드로 보여주면 어떨까"도 함께 제안 — "정답이 아니라 이해를 돕는 방법이 궁금하다"는 탐색적 질문으로 시작해, 답변 후 "두 가지 다 구현 진행"으로 확정.
- **구조 판단**: 격리에 필요한 변경 범위를 먼저 가늠 — 모든 Redis 키(`stock:event`, `queue:event`, `soldout:event`, `abandoned:event`, `admitted:event:{id}:{userId}`)가 이미 `eventId`로 네임스페이스돼 있어, "유저마다 세션 분리"가 아니라 "유저마다 `eventId` 하나"만 있으면 다운스트림은 자동으로 격리된다는 걸 확인(ADR 0016이 2026-07-21 당시 이 대안을 "복잡도 과함"으로 기각했던 판단이 Superseded됨).
- **백엔드 구현**:
  - `Event.demoOwnerId Int? @unique` + `User.ownDemoEvent` 1:1 관계 추가(마이그레이션 `20260807180000_add_event_demo_owner`, 비대화형 환경에서 `migrate dev --create-only`가 막혀 `migrate diff`로 정확한 DDL을 뽑아 수동 작성 후 `migrate deploy`로 우회).
  - `EventsService.findOrCreateOwnDemoEvent(userId)` 신설 — "내 데모 이벤트"가 없으면 그 자리에서 만든다. `findAll`도 `(isDemo:false) OR (demoOwnerId:내id)`로 필터.
  - `DemoService`의 `resetDemoEvent`/`simulateLoad`/`streamStats`를 전부 `userId` 인자를 받도록 변경, `EventsController`·`DemoController`의 관련 라우트에 로그인 가드 추가. 시뮬레이션 쿨다운도 전역 키에서 `userId`별 키로 분리(안 그러면 한 유저의 쿨다운이 다른 유저를 막아버려 격리 취지에 반함). 가상 유저 이메일 접두사도 `sim-` 전역에서 `sim-{eventId}-`로 스코프해, 한 유저의 리셋이 다른 유저가 투입한 가상 유저까지 지우지 않게 함.
  - `prisma/seed.ts`에서 전역 데모 이벤트 시딩 제거(이제 첫 방문이 자동으로 만듦), 마감 이벤트 2건 시딩은 유지.
- **실측으로 발견한 버그(같이 수정)**: 위 작업을 실서버 2계정으로 검증하던 중, 새로 만든 데모 이벤트에 가상 유저를 투입했더니 재고가 100/100인데도 전원이 "재고가 부족합니다"로 실패하는 걸 발견. 원인 — `ReservationsService.createHeld`가 재고 차감에 쓰는 Redis 키 `stock:event:{id}`는 원래 `resetDemoEvent()` 안에서만 초기화됐는데, 이벤트가 이제 "리셋 버튼"이 아니라 **첫 방문 시 자동 생성**되면서 이 초기화 경로를 안 거치게 됨 — `ReconcileProcessor`(1분 주기)가 재계산해주기 전까지 신규 이벤트는 실제 재고와 무관하게 모든 예매가 즉시 실패했다. `EventsService.create`/`findOrCreateOwnDemoEvent`가 이벤트 생성 시점에 직접 이 키를 심도록 수정 — 데모뿐 아니라 관리자가 만드는 일반 이벤트에도 잠재해 있던 버그라 함께 고쳤다. ADR 0016에 경위 기록.
- **프론트 구현**: 공용 `TicketCard` 컴포넌트 신설(수량·상태 배지·"내 예매" 표시·상태별 색). `booking-form.tsx`의 HELD/결제중/확정/취소 단계를 이 카드로 교체(수량을 상태 전이 시점에 실어 날라, 카드가 이후 입력값 변경에 영향받지 않게 함). `demo-dashboard.tsx`는 SSE `DemoStats`에 추가된 `tickets: TicketSummary[]`(백엔드가 이미 `isMine` 계산해 내려줌 — 유저별 격리 덕에 안전)를 이 카드로 그리드 렌더링, "예매 티켓 목록 (N건)" 섹션 신설.
- **검증**: 백엔드 tsc/jest 전수(84/84, 2회 연속 클린) + `events.service.spec.ts`에 `findOrCreateOwnDemoEvent`/`findAll` 신규 테스트 추가. 프론트 tsc/vitest 전수(23/23) — 기존 `booking-form.test.tsx`의 문구 단언을 티켓 카드 텍스트("결제 대기"/"결제 처리 중"/"확정"/"취소됨")로 갱신, `demo-dashboard.test.tsx`에 티켓 목록 렌더링 신규 테스트 추가. 실서버 2계정 curl 검증 — A 계정에 가상유저 20명 투입해도 B 계정의 이벤트 재고(100/100)는 전혀 영향받지 않음을 확인, 실제 예매→결제 파이프라인 1건을 끝까지 돌려 SSE `tickets` 응답이 프론트 타입과 정확히 일치함을 확인. **브라우저 시각 확인은 이번엔 못함**(도구 제약) — API 계약과 컴포넌트 로직 검증으로 대체했으니, 다음 세션에서 실제 화면을 한 번 띄워 보는 게 좋음.
- **다음**: 배포 6단계. 티켓 그리드가 실제 화면에서 보기 좋은지(수십 개로 늘어날 때 스크롤 UX 등) 사람 눈으로 한 번 확인 필요.

## 2026-08-07 · 실사용 1차 테스트에서 발견한 재진입 멈춤 버그 + 티켓 목록 좌측 이동

- **계기**: 사용자가 직접 화면을 띄워 "이벤트 진입→대기→12개 예매→결제→결제 실패→대기열 재진입" 흐름을 테스트하다 두 가지를 발견 — ① 결제 실패 후 "다시 예매하기"를 누르면 "대기열 입장 중..."에서 완전히 멈춤 ② 티켓 목록을 "내 예매" 패널과 가까운 좌측에 두고, 내 티켓을 목록 맨 위에 별도로 보여달라는 요청.
- **버그 원인**: "다시 예매하기"(확정/취소 이후)와 "다시 시도"(에러 이후) 버튼이 전부 `setState({phase:"idle"})`로만 되돌리는데, 대기열 자동 입장 effect는 `hasStartedRef` 가드로 **마운트당 정확히 한 번만** 실행되도록 막혀 있어서(StrictMode 이중 마운트 방지용, 2026-08-07 앞선 항목 참고) idle로 돌아가도 재입장이 다시 트리거되지 않았다. 이미 "허가창 만료" 상태의 "다시 대기열 입장" 버튼은 `handleJoinQueue`를 직접 호출해 정상 동작하고 있었으므로, 나머지 세 버튼(확정/취소/에러의 재시도)도 같은 패턴으로 통일 — `idle` 리셋 대신 `handleJoinQueue`를 직접 호출하도록 수정.
- **레이아웃 변경**: 티켓 목록을 `demo-dashboard.tsx`(우측, 판매현황 패널)에서 빼내 좌측(`booking-form.tsx` 바로 아래)으로 옮겼다. 두 컴포넌트가 같은 SSE 스냅샷(`tickets` 포함)을 나눠 써야 해서, 구독 자체를 신규 `use-demo-stats.ts` 훅으로 끌어올려 `events/[id]/page.tsx`가 한 번만 열고 `stats`/`streamError`를 두 컴포넌트에 props로 내려주는 구조로 바꿨다(각자 따로 구독하면 같은 스트림에 커넥션이 2개 열리는 낭비라 지양). 신규 `ticket-list.tsx`는 `tickets`를 `isMine`으로 분리해 "내 티켓" 섹션을 항상 위에, "다른 참가자 티켓"을 아래에 별도로 렌더링.
- **테스트**: `booking-form.test.tsx`의 CANCELLED 테스트에 "다시 예매하기" 클릭 후 실제로 순번을 다시 받는지 확인하는 회귀 단언 추가. `demo-dashboard.test.tsx`를 SSE 자체 구독 전제에서 `stats`/`streamError` props 전제로 전면 재작성(내부 티켓 렌더링 테스트는 새 `ticket-list.test.tsx`로 이동). **web 23→26 그린.** tsc·eslint 클린.
- **다음**: 배포 6단계. 이번에도 브라우저 시각 확인은 사용자가 직접 캡처를 공유해준 것으로 대체(도구 제약은 여전).

## 2026-08-08 · 헤더(로그아웃·도움말) + 대기열 안내 팝업 신설

- **개념 선정리(사용자 확인 질문 포함)**: "로그아웃"을 구현하기 전에, JWT가 상태 없는(stateless) 토큰이라 서버가 발급 후엔 무효화할 방법이 없다는 것 — 그래서 로그아웃은 "서버가 토큰을 지운다"가 아니라 "이 브라우저가 더 이상 그 토큰을 안 보내게 httpOnly 쿠키만 지운다"는 뜻이라는 걸 먼저 설명. "그럼 로그아웃 전에 누가 그 토큰 값을 복사해뒀다면 로그아웃 후에도 그 복사본은 유효한가?"를 확인 질문으로 던져 이해를 점검(답: 유효함 — 진짜 무효화엔 블랙리스트가 필요하고 이 프로젝트엔 없다는 트레이드오프까지 인지시킴).
- **백엔드**: `AuthController`에 `POST /auth/logout` 추가 — `res.clearCookie(ACCESS_TOKEN_COOKIE, authCookieOptions())`로 로그인 쿠키를 지운다. signup/login과 동일하게 `JwtAuthGuard`는 안 걺(로그아웃은 "지금 내가 누구인지" 확인이 필요 없는, 이미 만료된 토큰으로도 눌러야 하는 동작이라는 설계 판단). 신규 `auth.controller.spec.ts`(Nest DI 없이 가짜 Response로 `clearCookie` 호출 검증) — 이 모듈 첫 컨트롤러 단위 테스트. **API 84→85 그린.** curl로 실제 쿠키 왕복까지 검증(로그아웃 전 `/auth/me` 200 → 로그아웃 호출 → 쿠키 파일에서 `access_token` 실제로 사라짐 → 이후 `/auth/me` 401).
- **프론트 — 헤더**: 신규 `header.tsx`(우측 "도움말"·"로그아웃" 버튼) + `events/layout.tsx`(Next.js 세그먼트 레이아웃으로 `/events`·`/events/[id]`에만 헤더를 씌움 — 게이트/로그인 화면(`app/page.tsx`)엔 아직 로그아웃할 대상이 없어 안 보여줌, 별도 auth-state 판별 없이 라우팅 구조로 자연스럽게 해결). "로그아웃" 클릭 → `POST /auth/logout` → `router.push('/')`.
- **프론트 — 팝업 2종(사용자 확인 후 확정, "완전히 별개"로 결정)**:
  1. **도움말 전체 매뉴얼**(`help-modal.tsx`) — 헤더 버튼으로 언제든 열어보는 목적/사용법/케이스 3단 설명서.
  2. **대기열 안내 팝업**(`queue-notice-modal.tsx`) — 이벤트 상세 페이지 진입 시 **자동으로** 한 번 뜨는 짧은 안내("이 대기열은 오류가 아니라 의도된 동작"). `localStorage`(`sunchak:seenQueueNotice`)로 한 번 본 뒤엔 재방문 시 다시 안 뜨게 함(반복 테스트하는 사용자에게 매번 뜨면 방해만 됨).
  - 두 팝업이 공유하는 `modal.tsx`(배경 클릭·Esc로 닫히는 다이얼로그 뼈대) 신설.
- **린트 삽질(기존에 겪었던 것과 동일한 패턴)**: `queue-notice-modal.tsx`가 마운트 시 `localStorage`를 조회해 바로 `setState`했더니 `react-hooks/set-state-in-effect`에 걸림(booking-form.tsx의 자동 대기열 입장 effect와 정확히 같은 이유) — `Promise.resolve().then(() => ...)`로 감싸 "콜백 안에서 setState"하는 이 코드베이스의 기존 관례를 그대로 재사용해 해결. `help-modal.tsx`의 JSX 텍스트에 쓴 `"..."` 따옴표는 `react/no-unescaped-entities`에 걸려 한국어 타이포그래피 관례인 「 」로 교체.
- **테스트**: `header.test.tsx`(도움말 열기/닫기, 로그아웃 호출+이동) · `queue-notice-modal.test.tsx`(최초 자동 표시, 확인 클릭 시 localStorage 기록, 이미 본 상태면 재마운트해도 안 뜸) 신규. **web 26→31 그린.** tsc·eslint 클린.
- **다음**: 배포 6단계. 도움말 매뉴얼 문구는 초안이라 사용자 검토 후 다듬을 수 있음.

## 2026-08-08 · 대기열 안내 팝업과 실제 대기열 입장 순서 조정

- **계기**: 사용자가 방금 만든 대기열 안내 팝업을 실제로 써보고 "안내를 읽는 동안 가상유저 대기열이 다 빠진다"고 리포트 — 팝업이 뜨는 것과 동시에 `booking-form.tsx`의 자동 대기열 입장(크라우드 투입+본인 입장) effect도 마운트 시점에 같이 시작돼서, 입장 허가(2초당 20명 처리)가 안내를 읽는 몇 초 사이에 크라우드를 이미 다 처리해버리는 구조였다.
- **수정**: `QueueNoticeModal`에 `onAcknowledged: () => void` prop을 추가 — 이미 본 적 있으면(localStorage) 마운트 즉시, 처음이면 "확인" 버튼을 눌러야 호출된다. `BookingForm`은 `canJoin: boolean` prop을 새로 받아, 자동 입장 effect의 `hasStartedRef` 가드보다 **먼저** `if (!canJoin) return;`을 체크하도록 순서를 조정 — `canJoin`이 false인 동안의 실행(StrictMode 이중 실행 포함)은 "시작한 것"으로 안 치고, 나중에 true로 바뀐 뒤에야 정확히 한 번 시작한다. `events/[id]/page.tsx`가 `canJoin` state를 갖고 `QueueNoticeModal`의 `onAcknowledged`와 `BookingForm`의 `canJoin`을 이어준다(`useCallback`으로 콜백 identity를 고정해 QueueNoticeModal의 effect가 매 렌더마다 재실행되지 않게 함).
- **idle 문구 분기**: `canJoin`이 false인 동안은 "대기열 안내를 확인해주세요...", true가 된 뒤엔 기존 "대기열 입장 중...".
- **테스트**: `booking-form.test.tsx`에 "canJoin이 false면 요청이 안 나가고, true로 바뀌면 그제서야 시작한다" 회귀 테스트 추가(Wrapper 컴포넌트로 canJoin을 토글). `queue-notice-modal.test.tsx`를 `onAcknowledged` 호출 시점 검증 위주로 갱신. **web 31→32 그린.** tsc·eslint 클린.
- **다음**: 배포 6단계.

## 2026-08-08 · 대기열 안내를 이벤트 목록으로 재조정 + 로그인 후 도움말 자동 표시 + ADR 배포 전 점검

- **대기열 안내 트리거를 상세 페이지 → 이벤트 목록으로 재조정**: 위 항목에서 상세 페이지 마운트 시 안내를 띄우도록 고쳤는데, 사용자가 바로 이어서 "안내 확인 → 이동" 순서로 아예 목록 화면에서부터 처리해달라고 요청 — "이벤트 목록에서 이벤트 선택하면 팝업 뜨고 확인 누르면 이동". 방금 만든 `canJoin`/`onAcknowledged` 게이팅 구조를 상세 페이지(`booking-form.tsx`/`events/[id]/page.tsx`)에서 걷어내고, 대신 `event-list.tsx`의 판매중 카드를 `<Link>`에서 클릭 핸들러로 바꿔 `selectedEventId`를 기억한 뒤 `QueueNoticeModal`을 목록 화면에서 렌더링 — "확인"(또는 이미 본 적 있으면 즉시)에야 `router.push(`/events/${id}`)`로 이동한다. 상세 페이지는 다시 마운트 즉시 자동 입장하는 단순한 형태로 되돌아감(도착했다는 것 자체가 이미 안내를 확인했다는 뜻이라 별도 게이트가 불필요해짐).
- **테스트**: `booking-form.test.tsx`의 canJoin 회귀 테스트 제거(더 이상 그 컴포넌트의 책임이 아님), `event-list.test.tsx`를 클릭→팝업→이동 흐름으로 전면 재작성(3건: 매진 카드 클릭 불가, 안내 확인 후 이동, 이미 본 상태면 즉시 이동).
- **로그인 후 /events 최초 진입 시 도움말 자동 표시**: 신규 `auto-help-popup.tsx` — 헤더의 "도움말"과 같은 내용(`help-modal.tsx`)을 재사용하되, `localStorage`(`sunchak:seenHelp`, 대기열 안내와는 별도 키)로 최초 1회만 자동으로 띄운다. `/events`(로그인 완료 시 항상 도착하는 첫 화면)에 배치.
- **테스트**: `auto-help-popup.test.tsx` 신규(3건). **web 32→36 그린.** tsc·eslint 클린.
- **배포 전 ADR 점검(사용자 요청, STATUS.md "다음 할 일" 7번 항목 수행)**: 서브에이전트에 위임해 `docs/decisions/` 18건 전체를 실제 코드와 대조. 대부분 일치했고(0001~0008, 0012, 0014, 0016, 0017 — 0016/0017은 이미 이전에 최신화됨), 실제 드리프트 4건을 발견해 각 ADR에 "개정 이력"으로 정직하게 기록:
  - **ADR 0013(인증)**: Google SSO(2026-08-01)와 SSE용 쿠키 우선 전송(2026-08-05)이 전혀 반영 안 돼 있었음 — 원안은 "Bearer 헤더만"이라고 적었는데 실제론 쿠키가 우선, Bearer는 폴백. `User.password` nullable화도 함께 기록.
  - **ADR 0011(Infisical)**: 실제로는 W1~W2 초반 이후 한 번도 안 쓰이고, 원안이 명시적으로 기각했던 "로컬 `.env` 수동 공유"가 지금까지 사실상 기본 운영 방식이었음 — 배포 시점에 재검토 필요하다고 명시.
  - **ADR 0010(Neon)**: "W2만 로컬 예외"였는데 실제로는 W2 이후 모든 개발(W3~W4)이 계속 로컬 DB를 씀 — 예외 범위가 조용히 넓어짐.
  - **ADR 0018(모의결제)**: 비교 근거로 인용한 "HELD TTL 5분"이 PRD 원안 숫자 그대로라, 실제 현재값(30초)과 다름 — 방향성 있는 비교라 본문은 안 고치고 정정 메모만 추가.
  - (0009는 대부분 다른 ADR의 "후속 작업" 절이 이미 커버하고 있어 선택적 항목으로 보류.)
- **부수**: 대기열/예매 가시성을 더 높이고 싶다는 요청에 대해, 구현 전에 Artifact로 시각화 옵션 2종(게이지+퍼널 / 레인별 인물 원)을 실제 수치 목업으로 만들어 사용자 검토를 받는 중 — 애니메이션 방식은 "근사치"라 사용자가 선호하지 않아, 정밀 개인 추적(백엔드 신규 필요) vs 게이지·퍼널(지금 데이터로 가능) 중 결정 대기.
- **다음**: 배포 6단계. ADR 0011의 "배포 시 Infisical 재검토 vs 정식 Superseded" 결정은 아직 대기. 대기열 시각화 방향 확정 후 구현.

## 2026-08-08 · 티켓 문구 정정 + ADR 0010 방향 확정

- **"취소됨" → "결제 실패"**: 사용자가 실사용 중 발견 — 결제 실패(20% 확률, `PaymentProcessor`)로 CANCELLED가 된 티켓이 "취소됨"이라고 뜨는 게 "내가 취소한 것" 같은 오해를 준다고 지적. `ticket-card.tsx`의 `statusLabel`에서 문구만 교체(상태값·로직 무변경) — "결제 대기"→"결제 처리 중"→"결제 실패"/"확정"의 자연스러운 진행으로 맞춤. `booking-form.test.tsx`·`help-modal.tsx` 문구도 함께 갱신.
- **ADR 0010 방향 확정**: 배포 전 점검에서 "W2만 로컬 예외"가 실제로는 개발 전체로 넓어졌다고 기록했던 걸, 사용자가 "개발 완료되면 실DB(Neon)로 전환"으로 정식 확정해달라고 요청 — ADR 0010 개정 이력에 "개발 중엔 로컬 기본값, 배포 시 Neon 전환" + 전환 체크리스트(`DATABASE_URL` 교체 → `migrate deploy` → `db seed`)로 반영.
- **시각화 아티팩트 확인 경로 안내**: 지난 턴에 만든 목업 URL을 직접 전달(사용자가 "브라우저에서 보고 싶다"고 요청) — 별도 코드 변경 없음.
- **다음**: 배포 6단계. ADR 0011 결정과 대기열 시각화 방향은 사용자가 검토 후 다시 논의.

## 2026-08-08 · 판매 현황을 게이지+퍼센티지 막대로 재구성(옵션 A 확정 적용)

- **계기**: 사용자가 Artifact 목업(옵션 A: 게이지+퍼널)을 확인 후 "게이지는 최댓값이 있어야 의미 있다"며 범위를 구체화 — 재고(실제 max=totalQty)·대기중(자연스러운 max가 없어 200을 표시 상한으로, 그 이상은 게이지만 가득 채우고 숫자는 그대로)·최종결과(넓이 대비 퍼센티지)만 게이지로, 나머지 세부 수치는 숫자로 같이 보여달라고 요청. 후속으로 "200명 이상은..." 캡션 문구는 빼고 그대로 적용하라고 확정.
- **백엔드**: `DemoStats`에 `totalQty` 필드 추가(재고 게이지의 분모) — 리셋해도 안 바뀌는 `inventory.totalQty`라 매 틱 다시 조회할 필요 없이 `streamStats()` 진입 시 한 번만 읽어 `getStats()`에 넘긴다. `event.inventory`가 Prisma 타입상 `null` 가능이라(1:1 optional 관계) 기존 `resetDemoEvent`와 같은 가드(`NotFoundException`) 추가.
- **프론트 — 판매 현황 재구성**: 스탯 타일 6개 그리드를 걷어내고 3단으로 재구성.
  1. **게이지 2종**(`Gauge` 컴포넌트): 재고 잔량(분모=totalQty, 매진 시 빨간색+"매진" 라벨 유지)·입장 대기중(분모=200 고정 표시 상한, `capAt`으로 게이지만 가득 채우고 실제 숫자는 그대로). 큰 숫자를 게이지 바로 위에 얹어 "그래프이자 숫자"로.
  2. **최종 결과**(`OutcomeBar`): `stats.tickets`에서 HELD를 뺀 "이미 끝난" 예매만 분모로, 확정/결제실패/만료 퍼센티지 막대 + 범례(정확한 건수). 결과가 아직 없으면(전부 HELD) "아직 결과가 없습니다." 안내.
  3. **세부 수치 스트립**(`DetailStrip`): 확보중(HELD)·결제성공(PAID)·결제실패(FAILED)·재고소진 실패·포기(미시도)·큐 적체를 조용한 한 줄로 — "지금 판단"이 아니라 "누적 집계 확인용"이라 위 두 헤드라인보다 시각적 위계를 낮춤. 가상 유저 투입 버튼 옆에 중복으로 있던 "입장 대기중" 표시는 게이지로 흡수돼 제거.
  - `useDemoStats.ts`의 `DemoStats`에도 `totalQty` 반영.
- **테스트**: `demo.service.spec.ts`의 stats 통합 테스트에 `totalQty` 단언 추가(**API 85개 유지**, 기존 테스트 보강). `demo-dashboard.test.tsx` 전면 재작성 — 게이지 값(재고/대기중), 최종결과 퍼센티지+건수(확정 3·결제실패 2·만료 1, 겹치지 않는 수로 설계), 결과 없을 때 안내 문구, 세부 스트립 유지 확인. **web 36→39 그린.** tsc·eslint 클린.
- **실서버 검증**: curl로 SSE 스트림 첫 스냅샷에 `"totalQty":100`이 정확히 실리는 것 확인.
- **다음**: 배포 6단계. ADR 0011 결정은 사용자 검토 후 재논의.

## 2026-08-08 · "서버 재실행 후 새로고침하면 events.map is not a function" 버그 수정

- **계기**: 사용자가 코드 작업 중 서버가 자꾸 자동 종료돼 다시 실행 후 새로고침하면 `EventList`에서 `events.map is not a function`이 반복 재현된다고 리포트(스크린샷: 터미널 로그에 실제 백엔드 에러 `PrismaClientKnownRequestError ... Foreign key constraint violated on the constraint: events_demoOwnerId_fkey`가 같이 찍혀 있었음).
- **원인**: 이 프로젝트는 `jest`를 돌릴 때마다 로컬 DB의 유저·이벤트를 전부 지운다(기존에 문서화된 테스트 관행). 그런데 브라우저의 로그인 쿠키(JWT)는 그대로 남아있어서, 서버를 재기동한 뒤 새로고침하면 **서명·만료는 유효하지만 그 유저 ID가 이제 DB엔 없는** 상태로 인증을 통과했다. `EventsService.findAll → findOrCreateOwnDemoEvent`가 그 존재하지 않는 userId로 `Event.create({ demoOwnerId: userId })`를 시도해 FK 위반(500)이 났고, 프론트(`event-list.tsx`)는 응답이 실패(`res.ok===false`)인지 확인 안 하고 바로 `res.json()`을 이벤트 배열로 취급해 `.map()`에서 `TypeError`로 죽었다 — 백엔드 버그와 프론트 방어 부재가 겹친 문제.
- **수정 1(근본 원인, 백엔드)**: `JwtStrategy.validate()`가 서명·만료만 보고 끝내던 것을, `prisma.user.findUnique`로 그 유저가 지금도 실재하는지 한 번 더 확인하도록 변경 — 없으면 `UnauthorizedException`(401)을 던진다. "무상태 JWT"는 원래 "서버가 세션을 안 들고 있다"는 뜻이지 "유저 존재 여부를 절대 확인 안 한다"는 뜻은 아니라는 걸 재확인 — 요청당 DB 조회 1회를 더 내는 대신 정확성을 얻는 트레이드오프(ADR 0013 개정 이력에 반영).
- **수정 2(방어, 프론트)**: `event-list.tsx`의 `/events` 조회가 `res.ok`를 확인하지 않고 바로 `res.json()`을 배열로 취급하던 걸 고쳐, 실패 시 에러 문구 + "다시 로그인하기" 링크(`/`)를 보여주도록 변경 — 이제 백엔드가 401을 주면 크래시 없이 재로그인 유도로 이어진다.
- **테스트**: 신규 `jwt.strategy.spec.ts`(유저 존재 시 통과 / 없으면 401, 2건). `event-list.test.tsx`에 세션 무효 시 에러 UI 회귀 테스트 추가. **API 85→87, web 39→40 그린.**
- **실서버 검증**: 실제로 유저를 로그인시킨 뒤 DB에서 그 유저 행만 직접 삭제(jest 전체삭제 대신 최소 재현)해서 같은 쿠키로 `/events` 호출 → 수정 전이면 500, 수정 후엔 정확히 401 + "이 계정을 더 이상 찾을 수 없습니다. 다시 로그인해 주세요." 확인.
- **다음**: 배포 6단계. ADR 0011 결정은 여전히 사용자 검토 중.

## 2026-08-15 · "실시간성·대량 트래픽 체감이 떨어진다" — ADR에 추후 개선 백로그로 기록

- **계기**: 사용자가 "도움말에서 실제로 체험 가능하다고 소개했는데 대량 트래픽·실시간성 구현이 떨어지는 것 같다"고 지적. 추측 대신 코드로 확인 — `QueueService.streamStatus()`가 `STATUS_POLL_INTERVAL_MS=1_000`으로 **1초 폴링**이라는 걸 확인. 결제 확정 SSE(ADR 0006 2.4, 워커→RxJS Subject 즉시 push)는 진짜 이벤트 기반인데, 대기열 순번은 같은 "SSE"라는 이름 아래 사실 폴링이라는 비대칭을 발견.
- **개선 방향 4가지 제시 후 사용자 요청으로 ADR에 백로그로 기록**(아직 구현 안 함, 결정만):
  1. 순번 "허가되는 순간"을 이벤트 기반으로 전환(`QueueEventsService` 버스 재사용) — **ADR 0017**
  2. 순번에 예상 대기시간(ETA) 붙이기(기존 처리율 상수로 계산) — **ADR 0017**
  3. 대용량 트래픽 규모 확대(별도 `loadTestOwnerId` 이벤트, Neon 실사용량 API 기반 전역 예산 게이트 — 이 세션 앞서 논의한 설계) — **ADR 0016**
  4. 관측(Prometheus+Grafana) 도입, Neon 예산 잔량까지 대시보드화 — **ADR 0016**
- 두 ADR 끝에 "추후 개선 백로그 (2026-08-15)" 섹션으로 추가 — 결정된 것도, 구현한 것도 아니라는 걸 명확히 하고 다음 논의 시작점만 남김.
- **다음**: 배포 6단계. 이 백로그 항목 중 무엇부터 실제로 붙일지는 사용자 확인 후 결정.

## 2026-08-21 · CI/CD(GitHub Actions) 워크플로우 추가 + 대기열 순번 동점 버그 발견·수정

- **배포 6단계 중 2번(CI/CD) 착수**: `.github/workflows/ci.yml`(push/PR마다 api는 postgres/redis 서비스 컨테이너로 jest 통합 테스트까지, web은 lint+vitest+build) · `cd.yml`(수동 트리거로 api/web Docker 이미지를 빌드해 GHCR push)을 신규 작성. VM에 아직 Docker/Nginx가 없어(3번 작업 대기) `cd.yml`엔 실제 배포(SSH pull&재기동) job은 넣지 않고, 다음 작업에서 VM 쪽 구조가 정해지면 추가하기로 함. GHCR 인증은 별도 PAT 없이 `GITHUB_TOKEN`(job에 `packages: write` 권한만 부여)으로 충분하다는 점, `NEXT_PUBLIC_API_URL`은 GitHub Environments가 아니라 저장소 **Repository variables**로 넣어야 `${{ vars.* }}`로 참조된다는 점을 확인.
- **`ci.yml` 첫 실행에서 `queue.service.spec.ts` 1건 실패 발견**: "허가 후 재입장하면 새 순번으로 다시 대기한다" 테스트가 `rank: 1` 기대했는데 `rank: 0`을 받음. 로컬에선 안 나던 게 GitHub Actions(services 컨테이너라 Redis 왕복이 매우 빠름)에서만 재현됨.
- **원인**: `QueueService.join()`이 대기열 순번(ZSet score)을 `Date.now()`(밀리초)로 매기고 있었다. 두 `join()`이 같은 밀리초 안에 끝나면 score가 동점이 되고, Redis ZSet은 동점을 **멤버 문자열 사전순**으로 정렬해버려(`"1" < "2"`) 나중에 온 유저가 먼저 온 유저보다 앞 순번을 받아버렸다. 로컬은 Redis 왕복 지연으로 우연히 안 겹쳤을 뿐 — **CI 환경 특유의 우연이 아니라, 선착순 트래픽이 실제로 몰릴 때도 똑같이 터질 수 있는 공정성 버그**(이 큐가 존재하는 이유 자체를 훼손하는 문제)로 판단해 제대로 고침.
- **수정**: score를 `Date.now()` 대신 Redis `INCR`로 채번한 단조 증가 정수로 교체(`queue.service.ts`). Redis가 싱글 스레드라 `INCR`은 동시에 여러 `join()`이 들어와도 절대 같은 값을 두 번 못 주므로, 동점 자체가 구조적으로 불가능해진다.
- **검증**: `queue.service.spec.ts` 11/11, 전체 API 87/87 로컬 재실행 통과(회귀 없음).
- **다음**: `cd.yml` 수동 실행으로 GHCR push 확인 → VM 배포+Nginx(3번)로 진행.

## 2026-08-21 · VM Docker 설치 + 프로덕션 compose 준비 + ADR 0019 정정(쿠키 Domain 불필요) + Neon direct 연결 버그 발견·수정

- **CI/CD 검증 완료**: `ci.yml` 재실행 그린(api-test 1m2s·web-check 49s, web 40/40), `cd.yml` 수동 실행으로 GHCR에 이미지 2개 push + 패키지 Public 전환 완료. 배포 6단계 중 2번(CI/CD) 마무리.
- **3번(VM 배포+Nginx) 착수 — 1·2단계**: VM에 Docker Engine을 공식 설치 스크립트(`get.docker.com`)로 설치, `ubuntu` 유저를 `docker` 그룹에 추가(새 SSH 세션부터 적용 — 그룹 변경은 로그인 시점에 반영된다는 걸 재확인). GHCR 이미지 2개를 VM에서 직접 `docker pull`해 공개 전환이 실제로 작동하는지 확인.
- **ADR 0019 정정 — 쿠키 `Domain` 속성은 불필요함을 발견**: ADR 0019가 "배포 시 쿠키 `Domain`을 `.{IP}.sslip.io`로 지정해야 한다"고 적어뒀는데, `common/auth-cookie.ts`를 다시 보니 이 계획 자체가 틀렸다 — 인증 쿠키는 `api.<IP>.sslip.io` 한 곳으로만 오가서(`app.*`는 httpOnly라 읽지도 않음) `Domain` 기본값(발급 호스트에만 전송)이 이미 원하는 동작이고, 서브도메인 간 쿠키 전송은 이미 있던 `sameSite: isProd ? 'none' : 'lax'`가 담당하는 별개 메커니즘이다. **코드 변경 없이** ADR 0019에 개정 이력으로 정정만 기록. (Safari의 서드파티 쿠키 차단이 `SameSite=None`이어도 막을 수 있다는 잠재 리스크는 실제 e2e 검증 때 확인하기로 보류.)
- **Neon pooled/direct 연결 분리 필요 발견**: 프로덕션 compose에서 api 컨테이너가 뜰 때마다 `prisma migrate deploy`를 실행하도록 Dockerfile CMD를 바꾸려던 참에, Neon 공식 문서(WebFetch로 확인)가 "마이그레이션은 pooled 연결(pgbouncer)이 아니라 direct 연결을 써야 한다"고 명시하는 걸 확인. `schema.prisma`에 `directUrl = env("DIRECT_URL")` 추가, `apps/api/.env.example`·이 기기 로컬 `.env`·`ci.yml`에 `DIRECT_URL` 반영(로컬 Postgres는 풀러가 없어 `DATABASE_URL`과 같은 값). 로컬 `prisma generate`/`migrate deploy`/전체 jest(87/87) 재검증 통과.
- **`apps/api/Dockerfile` CMD 변경**: `node dist/main.js` 단독 실행에서 `prisma migrate deploy && node dist/main.js`로 — 밀린 마이그레이션이 없으면 아무 일도 안 하는 멱등 명령이라 컨테이너가 뜰 때마다(=VM 재배포마다) 먼저 돌려도 안전하다. 런타임 이미지가 devDependencies(prisma CLI 포함)를 그대로 갖고 있어(2026-08-19~20 Dockerize 때의 판단) 별도 설치 없이 바로 됨.
- **`infra/docker-compose.prod.yml` 신규 작성**: api·web·redis 3개 컨테이너(Postgres는 없음 — Neon 클라우드). api/web은 `127.0.0.1`에만 바인딩(외부 노출은 Nginx가 대신 함), 비밀값은 파일에 안 쓰고 `env_file: ./api.env`(VM에만 존재, git 미포함)로 분리.
- **`api.env`를 VM에 직접 생성**(비밀값이라 대화창엔 안 올림) — 사용자가 nano로 작성하다가 저장 위치가 `~/sunchak/api.env`가 아니라 `~/api.env`로 어긋난 걸 발견, SSH로 직접 확인(키 이름만 grep, 값은 안 봄) 후 올바른 위치로 이동 + 예전 삽질로 남은 `api.env.save`(nano 비정상 종료 시 자동 생성되는 복구 파일) 삭제 + 권한 600으로 제한.
- **다음**: 변경사항(schema.prisma·Dockerfile·ci.yml 등) 커밋+push → `cd.yml` 재실행으로 새 이미지 반영 → VM에서 `docker-compose.prod.yml` 기동 → Nginx 리버스 프록시 + Let's Encrypt.

## 2026-08-21 · VM 실제 배포 완료 — Docker+compose 기동, Nginx 리버스 프록시, Let's Encrypt HTTPS

- **컨테이너 기동**: `cd.yml` 재실행으로 새 이미지(Neon direct 연결 반영본) push 확인 후, `infra/docker-compose.prod.yml`을 VM에 scp → `docker compose pull && up -d`로 api/web/redis 3개 컨테이너 기동. api 로그에서 모든 라우트가 정상 매핑된 것으로 `prisma migrate deploy`(신규 Dockerfile CMD)가 Neon에 실제로 성공했음을 확인. VM 내부(`127.0.0.1`)에서 api `/health`·web `/` 둘 다 200 확인.
- **`api.env` nano 저장 위치 삽질**: 사용자가 `cd ~/sunchak` 전에 `nano api.env`를 실행해 `~/api.env`(홈 디렉토리)에 저장됨 — SSH로 직접 확인(값은 안 보고 `grep -oE '^[A-Z_]+='`로 키 이름만) 후 올바른 위치(`~/sunchak/api.env`)로 이동, 예전 저장 실패로 남은 `api.env.save`(nano의 비정상 종료 자동 백업) 삭제, 권한을 600으로 제한.
- **Nginx 리버스 프록시 개념 설명 후 설정**: `app.<IP>.sslip.io`→`127.0.0.1:3000`(web), `api.<IP>.sslip.io`→`127.0.0.1:3001`(api) 두 서버 블록(`/etc/nginx/sites-available/`). api 쪽엔 이 프로젝트가 SSE(EventSource)를 여러 엔드포인트(`/reservations/:id/stream`·`/events/:id/queue/stream`·`/demo/stats/stream`)에서 쓰기 때문에 `proxy_buffering off`+`proxy_read_timeout 3600s`를 명시(기본값이면 Nginx가 응답을 버퍼링해 브라우저에 실시간으로 안 흘려보내거나 연결을 조기 종료함).
- **Let's Encrypt**: `certbot --nginx -d app.* -d api.*`로 두 서브도메인 인증서 한 번에 발급 + `--redirect`로 HTTP→HTTPS 자동 전환까지 certbot이 Nginx 설정에 반영. 이메일 등록 여부를 사용자에게 물어 **생략**(개인 프로젝트라 갱신 실패 시 알림 안 받는 대신 외부 서비스에 이메일 안 넘기는 쪽 선택) — `--register-unsafely-without-email`.
- **정정**: 인증서 발급 전 `api.env`의 `DEMO_GATE_PASSWORD` 값을 curl 테스트용으로 조회하려다 Claude Code auto mode 클래시파이어가 차단 — 값을 안 보는 방식(틀린 비밀번호로 401이 나는지만 확인)으로 우회해 검증했다. 프로젝트 문서상 "값 자체는 비밀 아님"이라고 적혀 있어도, 실제 비밀값 파일에서 값을 읽는 행위 자체는 안전장치가 막을 수 있다는 걸 확인.
- **재현성**: 방금 작성한 Nginx 설정 원본(certbot 적용 전 상태)을 `infra/nginx/sunchak-{api,web}.conf`로 저장소에도 보관 — VM을 재구성해야 할 때(IP 변경 등) 이 파일을 배치하고 certbot만 다시 돌리면 되게 함.
- **검증**: 외부(로컬 기기)에서 HTTP(80) 200 → certbot 발급 후 HTTPS(443) 200 + HTTP→HTTPS 301 확인. 배포 도메인의 `/demo/gate`에 틀린 비번 curl → 401(전체 요청 경로 — Nginx→컨테이너→가드 — 가 실제로 동작함을 확인). `host` 명령으로 두 서브도메인이 매직 도메인답게 VM IP로 정확히 해석되는 것도 사전 확인.
- **브라우저 e2e 확인(사용자 직접 검증)**: 스크린샷으로 게이트 통과→Google 로그인→이벤트 상세 진입→12매 예매 확정→SSE 실시간 판매 현황(게이지·퍼센티지 막대·세부 카운트) 정상 갱신까지 전부 확인됨. Google Cloud Console 콜백 URI도 이미 등록돼 있었음(로그인 성공이 증거). **배포 6단계 중 1~3번(Dockerize·CI/CD·VM+Nginx) 전부 완료.**
- **다음**: 배포 6단계 중 4번(Prometheus+Grafana 관측)으로 진행.

## 2026-08-26 · 대용량 트래픽 테스트 프론트 화면 신설 + streamStats 클로저 버그 발견·수정

- **계기**: 지난 세션(2026-08-25)에 대용량 트래픽 테스트 백엔드(`/load-test/reset`·`/load-test/simulate`)를 구현했지만 대응하는 프론트 화면이 없어 `/load-test`가 그대로 404였다 — 이번 세션 최우선 작업으로 지정돼 있었음.
- **착수 전 백엔드 갭 재확인**: 캐주얼 데모의 `GET /demo/stats/stream`(SSE)에 대응하는 대용량 쪽 실시간 스탯 엔드포인트가 아예 없었다. 캐주얼의 `getStats()`는 예매 건 하나하나를 `tickets` 배열로 매초 통째로 흘려보내는데, 대량 테스트는 재고·VU가 최대 10,000까지 가능해 그대로 쓰면 매초 최대 만 건짜리 배열을 직렬화하는 꼴이라 부적합 — 사용자와 논의해 **집계 숫자만**(개별 예매 목록 제외) 내려보내는 `LoadTestStats`로 설계 확정.
- **백엔드 구현**: `LoadTestService.streamStats()`/`getStats()`(캐주얼과 같은 1초 주기 패턴, `tickets` 없이) + `GET /load-test/stats/stream`(SSE) 신규. `load-test.module.ts`에 `confirm` 큐 registerQueue 추가(적체 조회용). 신규 통합 테스트 2건(빈 스냅샷, 집계 6종 일치) — **API 118→120 그린.**
- **⚠️ 브라우저 e2e로 발견한 실제 버그 — streamStats 클로저**: 유닛 테스트는 다 통과했지만, Playwright로 실제 화면에서 "재고 리셋" 버튼을 눌러도 게이지 분모(총 재고)가 리셋 전 값(1000)에 계속 고정되는 걸 발견. 원인은 `streamStats()`가 SSE 연결 시작 시점에 `totalQty`를 한 번만 읽어 클로저에 담아두고 매 틱 재사용하던 것 — 캐주얼 데모의 원래 패턴을 그대로 베꼈는데, 캐주얼은 리셋해도 항상 고정값(100)이라 문제가 없었지만 대량 테스트는 리셋마다 유저가 재고를 다르게 정할 수 있어 **이미 열려 있던 SSE 연결이 옛 값을 계속 흘려보내는** 버그가 났다. `getStats()`가 매 틱 `prisma.inventory.findUnique`로 새로 읽도록 수정 후 재검증(리셋 직후 게이지가 정확히 "50/50"으로 반영됨). 유닛 테스트만으로는 못 잡는, "이 값이 세션 동안 불변이라는 전제가 깨지는" 종류의 버그라 브라우저 검증의 가치를 다시 확인.
- **프론트 구현**: `stat-tiles.tsx`(신규, `Gauge`/`DetailStrip`을 `demo-dashboard.tsx`에서 추출 — 두 대시보드가 공유하는 두 번째 사용처가 생겨 공용화, 데모 쪽도 이 파일을 쓰도록 갱신), `use-load-test-stats.ts`(신규, SSE 구독 훅), `load-test-dashboard.tsx`(신규 — 캐주얼의 티켓 기반 `OutcomeBar`와 달리 개별 목록이 없어 확정/결제실패/재고소진/포기 4개 집계 카운터를 곧바로 분모로 쓰는 `AttemptOutcomeBar`로 대체, 이 네 카운터의 합이 투입 인원과 일치하는 것은 지난 세션에 이미 실측 검증된 불변식), `app/load-test/{layout,page}.tsx`(신규 라우트). `/events` 화면에 "대용량 트래픽 테스트 →" 진입 링크 추가(안 넣으면 화면이 있어도 아무도 못 찾는 고아 라우트가 되므로).
- **테스트**: `load-test-dashboard.test.tsx` 신규 10건. **web 40→50 그린.** `next build`로 타입드 라우트(`LayoutProps<"/load-test">`) 생성 확인 후 tsc 클린.
- **삽질 재확인 — BullMQ repeat 스케줄러가 로컬 Redis에 누적되는 문제(기존에 문서화된 이슈, 오늘 재현)**: 브라우저 e2e 검증을 위해 `pnpm start:dev`를 `kill -9`로 내렸는데, BullMQ의 repeatable job 등록은 Redis 서버 쪽에 저장되는 상태라 **프로세스를 죽여도 스케줄 자체는 안 지워진다** — 이후 jest를 돌리면 이 유령 스케줄이 실제 시간에 맞춰 계속 틱을 쏴서 `admission.processor.spec.ts`·`load-test-admission.processor.spec.ts`의 "한 틱에 N명만" 같은 정확한 카운트 단언이 흔들렸다(로컬 전용 휴발성 Redis라 `FLUSHALL`로 즉시 해결, 실서비스 데이터엔 영향 없음). **재발 방지 메모**: 로컬에서 `start:dev`를 정상 종료(Ctrl+C) 없이 강제로 내린 뒤 통합 테스트가 이유 없이 흔들리면, 먼저 `docker exec sunchak-redis redis-cli FLUSHALL`부터 의심할 것.
- **실서버 검증**: curl로 SSE 스냅샷 확인 + Playwright(스크래치패드에 임시 설치)로 게이트→로그인(API 직접 호출로 쿠키 발급 후 브라우저 컨텍스트에 주입, Google OAuth는 헤드리스 환경에서 불가)→`/events`→`/load-test` 이동→재고 리셋(50)→가상 유저 30명 투입→SSE 실시간 반영(재고 50→41, 최종 결과 막대 확정 9·실패 5·포기 3)까지 스크린샷 2장으로 확인. 콘솔 에러 없음.
- **잡일 — 루트 페이지 "확인 중..." 무한 정지 방어 공백 발견·수정**: 로컬 브라우저로 직접 확인하던 중 사용자가 `/`에서 "확인 중..."이 멈춰 보인다고 리포트. 실제 원인은 Next.js dev 서버를 재시작해 브라우저가 옛 HMR(Hot Module Reload) 세션 ID로 계속 재연결을 시도하던 것(개발 전용 현상, 새로고침으로 해결) — 하지만 짚어보니 `page.tsx`의 `checkStatus()`가 `apiFetch("/auth/me")`의 실패(reject)를 전혀 안 잡고 있어서, API 서버 다운·네트워크 단절 같은 **실제로 일어날 수 있는 상황**에서도 `setStatus`가 영영 안 불리고 조용히 멈추는 진짜 방어 공백이었다. `try/catch`로 감싸 `"error"` 상태(에러 문구+재시도 버튼)를 추가 — "이 특정 원인(HMR)을 코드로 감지"하는 게 아니라 "그 증상(fetch 실패 시 무한 정지)의 일반적인 원인"을 고치는 쪽으로 스코프를 잡았다. 회귀 테스트 2건. **web 50→52 그린.**
- **잡일 — `/load-test`에 방문자 본인의 1인칭 대기열 체험 추가**: 사용자가 브라우저로 직접 써보고 "실제 대용량 실시간 티켓팅 서비스의 사용자 관점 경험이 안 느껴진다"고 지적. 원인 파악 — `/load-test`는 지금까지 "가상 유저를 투입하고 집계만 지켜보는" 관리자 콘솔뿐이었고, 캐주얼 데모(`booking-form.tsx`)가 이미 제공하던 "방문자 본인도 자동으로 대기열에 들어가 순번이 실시간으로 줄어드는" 1인칭 체험이 대용량 쪽엔 없었다. ADR 0016의 설계(§Context 1 "방문자가 예매 버튼을 누르면 재현된다")는 틀리지 않았다 — 2026-08-25 대용량 확장이 이 1인칭 요소를 안 가져온 구현 스코프 누락이었다.
  - **백엔드**: `POST /load-test/queue`(입장) + `GET /load-test/queue/stream`(순번 SSE) 신규. `QueueService.status`/`streamStatus`는 이벤트-무관 로직이라 그대로 재사용했지만, **입장(join) 자체는 캐주얼 전용 `queueService.join()`을 재사용하면 안 됐다** — `ACTIVE_QUEUES_KEY`(캐주얼 `AdmissionProcessor` 전용)에 등록되면, `LoadTestAdmissionProcessor`가 이미 순회 중인 이벤트를 두 워커가 동시에 ZPOPMIN하는 경합이 재현된다(2026-08-25 세션이 이 둘을 분리한 이유 그대로). 반드시 `joinLoadTest()`를 호출하는 새 경로가 필요했다.
  - **프론트**: `booking-form.tsx`를 일반화 — `joinQueueUrl`/`queueStreamUrl`(대기열 URL을 props로), `autoInjectCrowd`(캐주얼만 true — 대용량은 "가상 유저 투입" 버튼으로 이미 유저가 규모를 직접 통제하므로 자동 크라우드 투입을 끔), `eventId`를 옵셔널로(대용량은 URL에 :eventId가 없어 join 응답에서 처음 알게 된다). 캐주얼 호출부(`events/[id]/page.tsx`)는 URL을 명시적으로 넘기도록만 갱신, 동작 변화 없음. `/load-test/page.tsx`에 좌측 "내 예매"(BookingForm)+우측 관리자 대시보드(기존 LoadTestDashboard)를 나란히 배치 — events/[id]/page.tsx와 같은 레이아웃 패턴.
  - **⚠️ 브라우저 e2e로 발견한 새 동시성 버그**: 첫 검증에서 500 에러 발생 — `/load-test` 마운트 시 `useLoadTestStats()`(SSE)와 `BookingForm`의 자동 입장 effect가 **동시에** `findOrCreateOwnLoadTestEvent()`를 처음 호출하면서, 둘 다 "없으니 만들자"로 판단해 동시에 `event.create()`를 시도 → `loadTestOwnerId` 유니크 제약(P2002)에서 진 쪽이 500으로 죽는 전형적인 check-then-create 경합. 캐주얼 데모(`findOrCreateOwnDemoEvent`)도 코드는 똑같이 안전하지 않지만, `/events` 목록 조회가 상세 페이지 진입보다 항상 먼저 이벤트를 만들어두는 구조라 실제로는 안 겹쳐서 안 드러났을 뿐이었다 — 대용량은 그런 선행 단계가 없어 이번에 처음 실사용으로 드러났다. `reservations.service.ts`의 `createHeld()`가 idempotencyKey 재전송(P2002)을 다루는 것과 같은 패턴 — try/catch로 잡아 진 쪽이 이긴 쪽이 만든 행을 재조회해 반환하도록 수정.
  - **테스트**: `load-test.service.spec.ts`에 joinQueue/streamQueueStatus 통합 테스트 4건(캐주얼 활성 목록엔 안 등록되는지 포함), `events.service.spec.ts`에 P2002 경합 회귀 테스트 2건. **API 120→126 그린.** `booking-form.test.tsx`에 대용량 모드(크라우드 자동투입 없음, eventId를 join 응답에서 획득) 테스트 2건. **web 52→54 그린.**
  - **실서버 검증**: Playwright로 재고 리셋(500)→가상 유저 20명 투입→방문자 본인이 자동으로 순번 0번을 받고→입장 허가→예매→결제(20% 확률로 실패까지 실제로 재현)까지 좌측 "내 예매" 패널과 우측 집계 대시보드가 동시에 실시간 갱신되는 것 스크린샷으로 확인. 콘솔 에러 없음(수정 전엔 500 에러가 있었음).
- **잡일 — 진입 시 "재고 1만 + 가상 유저 5천" 자동 세팅**: 위 1인칭 체험을 붙인 뒤 사용자가 직접 확인하며 "자동 크라우드 투입이 없다"고 지적 — 의도적으로 껐던 것이었지만(캐주얼처럼 랜덤 5~100명이 아니라 고정 재고 1만/VU 5천을 원함), 원하는 기본값을 명확히 받아 반영했다. `load-test/page.tsx`에 마운트 시 자동으로 `POST /load-test/reset(totalQty:10000)` → **완료를 기다린 뒤** → `POST /load-test/simulate(virtualUserCount:5000)`를 순서대로 실행하는 effect 추가(StrictMode 이중 마운트 가드 포함, booking-form.tsx와 같은 패턴). **순서가 중요한 이유**: 리셋이 대기열을 비우는(`purge`) 파괴적 작업이라, 리셋이 끝나기 전에 본인이 먼저 대기열에 들어가면 리셋이 그 순번까지 지워버린다 — 그래서 리셋이 resolve된 뒤에만 `BookingForm`(본인 대기열 입장)을 렌더하도록 `ready` 상태로 게이팅했다(그 전엔 "대규모 트래픽 상황을 준비 중입니다..." 안내). `LoadTestDashboard`의 수동 재고/VU 입력 기본값도 1000/100 → 10000/5000으로 맞췄다(일관성).
  - **테스트**: `load-test/page.test.tsx` 신규 3건(리셋→시뮬레이션 순서, 리셋 완료 전엔 준비 중 안내만, StrictMode 이중 마운트 방지). **web 54→57 그린.**
  - **실서버 검증**: Playwright로 `/load-test` 진입 → 재고 게이지가 10,000/10,000으로 자동 반영 → 본인이 곧바로 입장 허가를 받아 HELD까지 도달 → 우측 대시보드에 5,000명 투입이 페이스에 맞춰(입장 대기중 151명 등) 진행 중인 것 동시에 확인.
- **잡일 — 재시도 버튼이 "반응 없어 보이는" UX 공백 발견·수정**: 사용자가 로컬 브라우저에서 루트(`/`)가 "서버에 연결할 수 없습니다."로 멈춰 있고 "다시 시도"를 눌러도 아무 일도 안 일어난다고 리포트. 원인은 두 가지가 겹친 것 — ① 진짜 원인은 이 세션이 검증을 위해 API 서버를 내려둔 채 안 올려서 실제로 서버가 꺼져 있었던 것(코드 문제 아님, 세션 만료와도 무관 — 그건 이미 별도 분기로 정상 처리됨). ② 그런데 `refreshStatus()`가 재시도 결과를 그냥 `setStatus`로만 덮어써서, 서버가 여전히 안 떠 있으면 `"error"→"error"`로 상태값이 안 바뀌어 화면에 아무 시각적 변화가 없었다 — 버튼은 실제로 매번 다시 fetch를 실행하고 있었지만, 사용자 입장에서는 눌러도 반응이 없는 것처럼 보이는 진짜 UX 공백이었다. `refreshStatus()`가 재시도 시작 시 먼저 `setStatus("checking")`으로 되돌리도록 수정 — 이미 있는 "확인 중..." 상태를 재사용해 새 UI 없이 해결. 회귀 테스트 1건(재시도가 또 실패해도 "확인 중..."을 거쳐가는지). **web 57→58 그린.**
- **잡일 — 본인 순번이 45로 나와 "대용량" 체감이 안 됨(사용자 실사용 피드백) → 대기열 형성 후 참여로 개선**: 리셋 직후 곧바로 본인이 대기열에 들어가면, 5,000명 목표를 잡아도 아직 몇십 명밖에 안 쌓인 시점이라 순번이 작게 나왔다 — Postgres 보호용 투입 페이스(초당 200명)와 입장 처리 워커의 배출 페이스(1초 주기, 남은 인원의 20%·최소 50명)가 맞물려, 대기열 길이가 결국 평형(재귀식 `R_{n+1}=0.8·R_n+160`으로 근사, 평형점 800 근처)에 수렴한다는 걸 계산으로 확인 — 그래서 그 평형보다 작은 **600명을 임계치**로 잡고, `stats.admissionQueueCount`(이미 SSE로 흐르고 있던 값)가 그 값을 넘을 때까지는 본인 대기열 입장을 미루도록 `load-test/page.tsx`에 게이팅 로직 추가(안 넘으면 15초 타임아웃으로 안전하게 진행). 대기 중엔 "대기열 형성 중 — 현재 N명 대기 중"으로 실시간 증가하는 숫자를 그대로 보여줘 그 자체가 "대용량이 쌓이고 있다"는 서사가 되게 했다.
  - **테스트**: `load-test/page.test.tsx`를 임계치 도달·타임아웃 안전장치·형성 중 실시간 표시까지 5건으로 재작성(기존 3건에서 확장). fake timer로 타임아웃 경로 검증. **web 58→60 그린.**
  - **실서버 검증**: Playwright로 폴링하며 실측 — t=2.2s 138명 → t=7.2s 559명 → t≈9s에 임계치(600) 도달해 본인 참여, **순번 474(예상 대기 약 48초)**로 반영. 기존 45 대비 확연히 "대용량" 체감이 개선됨을 확인.
- **UI 개선 — Artifact 목업 3안 비교 후 "라벨 기반 위계 정리"(옵션 B) 채택·구현 완료**: "처음 입장하면 뭘 봐야 할지 감이 안 온다"는 지적으로 Artifact 캔버스에 실제 데이터(재고 9,869/10,000, 순번 474 등)를 채운 레이아웃 3가지를 그려 비교(2026-08-08 DemoDashboard 재설계 때와 같은 방식) — ① 스토리텔링 헤드라인 배너+순번 초대형 강조, ② "나의 상황"/"전체 현황" 라벨로 위계만 정리, ③ 재고 소진 진행 바를 헤드라인으로. 사용자가 **옵션 B**를 선택.
  - **구현**: `booking-form.tsx`/`load-test-dashboard.tsx`에 공통으로 `sectionLabel?: string` 옵셔널 prop 추가 — 값이 있으면 굵은 세로 악센트 바 + 대문자 소제목을 카드 위에 렌더(캐주얼 호출부는 안 넘겨 기존 모습 그대로 유지, 두 컴포넌트가 공유/단독 사용 구조라 이 방식이 가장 수술적). `load-test/page.tsx`에 페이지 인트로 문구("가상 유저 5,000명이 티켓 10,000장을 두고...") 추가, 좌우 컬럼에 각각 `sectionLabel="나의 상황"`/`"전체 현황"` 전달. `LoadTestDashboard`의 수동 컨트롤(재고 리셋·가상 유저 투입)을 "관리자 설정(선택 사항 — 이미 자동으로 세팅되어 있습니다)" 라벨 + 점선 테두리로 시각적으로 낮춰 자동 세팅이 이미 끝났음을 알림. 내부 `<h2>대용량 트래픽 테스트</h2>`는 페이지 레벨 "전체 현황" 라벨과 중복이라 제거.
  - **순번 폰트만 별도로 키움**: `booking-form.tsx`의 "대기 중입니다 — 현재 순번 {rank}" 문장 구조는 그대로 두고(여러 테스트가 이 조합 문자열을 정확히 매칭하므로 안전하게), 순번 숫자를 감싼 `<span>`의 클래스만 `font-mono text-2xl font-bold`로 키움 — 문장 텍스트(`textContent`)는 안 바뀌어 기존 테스트 무손상, 시각적으로만 확 커짐. 캐주얼 데모도 같은 컴포넌트를 쓰므로 이 폰트 확대는 자연스럽게 같이 적용됨(의도한 부수효과, 가독성 개선이라 해로울 게 없다고 판단).
  - **테스트**: 신규 없음(기존 테스트가 구조 변경 없이 그대로 통과 — booking-form 10건, load-test-dashboard 9건, load-test/page 5건 전부 그린). **web 60개 유지.**
  - **실서버 검증**: Playwright로 `/load-test` 재진입 → 인트로 문구·"나의 상황"/"전체 현황" 라벨·순번 큰 폰트(예: 612)·"관리자 설정" 점선 박스가 목업 그대로 반영된 것 스크린샷 확인. 콘솔에 hydration mismatch 경고 1건 있었으나 `caret-color:transparent`가 원인 — 브라우저 확장 프로그램이 입력창에 주입하는 흔한 오탐 패턴(익스텐션 없는 헤드리스 테스트에선 재현 안 됨), 코드 문제 아님으로 판단.
- **잡일 — 대용량 순번의 예상 대기(ETA)가 널뛰는 버그 발견·수정**: 사용자가 실브라우저로 "예상 대기가 분 단위였다가 52초였다가 30몇초였다가 한다"고 리포트. 원인은 `QueueService.eta()`가 **캐주얼 데모 전용 고정 배치 상수**(`ADMISSION_BATCH_SIZE=20명`/`ADMISSION_INTERVAL_MS=2000ms`)로 계산하는 하나의 공식뿐이었는데, `status()`/`streamStatus()`는 이벤트-무관 로직이라 대용량 순번 SSE(`LoadTestService.streamQueueStatus`)도 그대로 재사용하고 있었던 것 — 정작 대용량은 `LoadTestAdmissionProcessor`가 완전히 다른 리듬(남은 인원의 20%를 포아송으로 근사, 1초 주기, [50,1000] clamp)으로 입장을 처리한다. 캐주얼 공식을 대용량 순번에 그대로 적용하니 배치 크기가 실제보다 10배 이상 작게(20 vs 실제 최대 1000) 가정돼 ETA가 크게 부풀려지고, 매 틱 rank가 포아송 배치+지속 유입으로 불규칙하게 바뀌는 걸 안 맞는 공식으로 재계산하니 값이 널뛴 것.
  - **수정**: `QueueService`에 `AdmissionModel`(meanFraction/minBatch/maxBatch/intervalMs) 옵셔널 파라미터를 추가 — 안 넘기면(캐주얼) 기존 고정 공식 그대로, 넘기면(대용량) `poissonLikeBatchSize()`와 같은 clamp 공식으로 "지금 이 순간" 대기열 크기 기준 기대 배치 크기를 근사해 ETA를 계산한다(무작위성 대신 평균 사용 — 매 틱 갱신되므로 유입·배출 속도 변화에 스스로 맞춰진다). `LoadTestService`가 `LoadTestAdmissionProcessor`와 **정확히 같은 env 키**(`LOAD_TEST_ADMISSION_MEAN_FRACTION`/`MIN_BATCH`/`MAX_BATCH`/`INTERVAL_MS`)를 읽어 이 모델을 만들어 넘긴다.
  - **테스트**: `queue.service.spec.ts`에 admissionModel 분기 3건(minBatch clamp/meanFraction 비율/모델 미전달 시 캐주얼 그대로). `load-test.service.spec.ts`의 `streamQueueStatus` 테스트를 `expect.any(Number)`(느슨한 검증)에서 정확한 값(1초, 캐주얼 공식이었다면 2초가 나왔을 것이라 회귀 증거가 됨) 검증으로 강화. **API 126→129 그린.**
  - **실서버 검증**: curl로 5,000명 투입 후 실제 순번/ETA 흐름 관찰 — 순번 547→372→211→57로 줄며 ETA도 5초→4초→2초→1초로 매끄럽게 감소(수정 전이라면 이 순번들에 캐주얼 공식을 적용해 56초→38초처럼 튀었을 것).
- **잡일 — 대용량 입장 허가창을 캐주얼과 분리해 30초로**: "대기열 입장 후 예매 가능 시간이 너무 짧다"는 실사용 피드백. 확인해보니 `QUEUE_ADMISSION_WINDOW_MS`(기본 8초)를 캐주얼·대용량이 그대로 공유하고 있었는데, 8초는 원래 **캐주얼 가상 유저의 반응 속도**에 맞춰 정한 값(2026-08-06)이라 실제 사람이 대용량 화면의 순번·ETA·전체 현황을 읽고 "예매하기"를 누르기엔 촉박했다. 사용자가 "따로 30초로"라고 확정.
  - **구현**: `QueueService.admit(eventId, userId, windowMs?)`에 옵셔널 파라미터 추가(안 넘기면 기존 캐주얼 공식 그대로, 넘기면 그 값 사용). `LoadTestAdmissionProcessor`가 새 env `LOAD_TEST_ADMISSION_WINDOW_MS`(기본 30,000ms)를 읽어 `admit()` 호출 시 넘긴다. `.env.example`에 키 추가.
  - **테스트**: `queue.service.spec.ts`에 `windowMs` 파라미터가 env 기본값보다 우선하는지 확인하는 회귀 테스트 1건(TTL 실측), `load-test-admission.processor.spec.ts`에 실제 허가 후 Redis PTTL이 캐주얼 기본값(8초)이 아니라 설정한 대용량 값(테스트에선 5초로 설정) 근처인지 확인하는 테스트 1건. **API 129→131 그린.**
  - **실서버 검증**: curl로 실제 허가 후 `PTTL admitted:event:{id}:{userId}` 조회 → 28.4초(2초 경과분 감안하면 정확히 30초) 확인.
- **잡일 — 입장해야 할 시점에 바로 "허가창 만료"로 오판하는 동시성 버그 발견·수정**: 사용자가 "대기 시간이 줄다가 입장해야 하는 시점에 바로 허가 시간이 지났다고 뜬다"고 리포트. 원인은 입장 처리 워커(`AdmissionProcessor`/`LoadTestAdmissionProcessor`)가 한 배치(대용량은 최대 1000명)를 **한 명씩 순차로 `await`하며 admit() 처리**하고 있었던 것 — `popNext()`로 대기열에서는 이미 빠졌지만(`rank`가 null이 됨) 아직 자기 차례의 `admit()`이 안 끝난 사람은 그 틈 동안 `rank:null, admitted:false`라는 "이도저도 아닌" 상태가 된다. 이 순간에 프론트의 1초 폴링이 걸리면 `booking-form.tsx`가 이걸 "허가창이 지나 밀려남(expired)"으로 오판하고 **SSE 구독까지 끊어버려**, 실제로는 정상적으로 허가되고 있었는데도 영영 "허가창 만료" 화면에 갇힌다. 캐주얼(배치 20명)은 이 틈이 짧아 거의 안 걸렸지만, 대용량은 배치가 최대 1000명이라 이 틈이 1초를 넘기기 쉬웠다.
  - **수정**: 두 프로세서 모두 배치 내 `admit()`을 순차 for-loop 대신 `Promise.all()`로 동시에 처리하도록 변경 — popNext 직후 "빠졌지만 아직 허가 안 됨" 창을 O(배치크기×왕복시간)에서 사실상 한 왕복 시간 수준(수 ms)으로 줄여, 1초 폴링이 그 틈을 잡을 확률을 실질적으로 제거했다.
  - **테스트**: 전체 131개 그대로 그린(구조 변경 없이 통과 — 기존 assertion들이 배치 처리 결과만 확인해 동시/순차 여부와 무관).
  - **실서버 검증**: curl로 재고 10,000·VU 5,000 투입 후 대기열이 충분히 쌓인(661명) 상태에서 본인 순번 스트림 관찰 — 661→509→365→192→7로 줄다가 **곧바로 `admitted:true`로 전환**(중간에 애매한 `rank:null/admitted:false` 프레임 없음). 30초 허가창이 지나자(id 37부터) 정상적으로 만료 상태 전환도 확인.
- **다음**: `git push` → CD 자동화(`ci.yml`→`cd.yml`)로 라이브 VM 배포 → 배포 사이트에서 브라우저로 직접 확인.

## 2026-08-27 · "재고가 -2로 고정된다" 버그 발견·수정 (ADR 0021과의 상충)

- **계기**: 어제 발견·수정한 대용량 버그들(streamStats 클로저, 1인칭 대기열, ETA, 허가창, admit() 동시성)을 이어서, 사용자가 "예매 9천을 해버렸더니 이후 입장한 VU 처리 중 재고가 -1이 표시됐다가 0이 됐다가 하고, 대기가 모두 끝난 후 몇 초 지나니까 -2로 고정됐다"고 리포트.
- **1차 확인**: Redis 카운터가 순간 음수로 튀는 것 자체는 이 프로젝트가 다루는 정상적인 동시성 현상(관문 DECRBY 경합)이고, `ReconcileProcessor`(1분 주기, `총재고−(HELD+CONFIRMED)`로 절대값 재계산)가 원래 이걸 계속 바로잡아준다 — 그런데 실제로 Redis를 조회해보니 `stock:event:{id}`가 -2로 멈춰있고, `reconcile:fallback-cooldown` 키가 이미 세팅된 채 TTL이 **23.9시간** 남아있었다.
- **근본 원인**: ADR 0021(2026-08-22)이 "최근 예약 활동이 없으면"(`HELD_ACTIVITY_KEY`, TTL 90초) reconcile을 하루 한 번짜리 완화 모드로 낮췄다 — 원래 취지는 "아무도 안 보는 유휴 시간엔 Neon을 안 깨운다"는 비용 절감이었다. 그런데 대용량 테스트가 끝나면(마지막 가상 유저 예매 시도 후 90초) 정확히 이 "유휴"로 오판되고, 그날 여러 번 테스트하며 하루짜리 완화 게이트가 이미 소진돼 있어 다음 보정까지 최대 24시간이 걸리는 상태였다 — **정작 사용자는 화면을 실시간으로 보고 있는 정반대 상황인데, 유휴 판정 로직이 그걸 몰랐다.**
- **수정**: `LoadTestService.getStats()`/`DemoService.getStats()`가 매 틱(1초 주기)마다 `HELD_ACTIVITY_KEY`를 함께 갱신하도록 추가 — 방문자가 stats SSE를 열어두고 있는 한 "활동 중"으로 계속 인식되어 reconcile이 1분 주기로 계속 실제 보정을 수행한다. 이 stats 쿼리 자체가 이미 매초 Postgres를 두드리고 있어 이 한 줄(Redis SET)로 비용이 추가되지 않는다 — ADR 0021의 원래 비용 절감 취지(진짜 유휴 시간엔 안 건드림)는 그대로 유지된다. ADR 0021에 개정 이력으로 기록.
- **테스트**: `load-test.service.spec.ts`/`demo.service.spec.ts`에 stats 조회 시 활동 신호가 갱신되는지 확인하는 테스트 2건. **API 131→133 그린.**
- **실서버 검증**: Redis에 사용자가 겪은 것과 똑같은 상황(재고 -2, 활동 신호 없음, 24시간 완화 게이트 이미 소진)을 인위적으로 재현한 뒤 stats 스트림을 75초 관찰 — 27초까지는 -2로 보이다가 **자동으로 100(정상값)으로 회복돼 유지**됨을 확인(수정 전이었다면 24시간 그대로였을 것).
- **잡일 — 게이트 토큰 만료 시 "다시 시도"가 영원히 막다른 길이던 버그 발견·수정**: 사용자가 "예매 수량 9,000으로 진행했더니 결제 실패, 다시 예매하려 했으나 '유효하지 않거나 만료된 데모 토큰입니다' 문구가 뜨고 다시 시도할 수 없다"고 리포트. `JWT_EXPIRES_IN`(기본 1시간)이 로그인 JWT뿐 아니라 게이트 토큰(`X-Demo-Token`/`demo_token` 쿠키, 같은 `JwtService`로 서명)에도 그대로 적용되는데, 장시간 테스트로 게이트 토큰이 만료되면 `DemoGateGuard`가 이후 **모든** 요청을 이 메시지로 거부한다 — `booking-form.tsx`의 "다시 시도" 버튼은 무조건 같은 예매 액션(`handleJoinQueue` 등)을 그대로 재실행할 뿐이라, 게이트가 안 풀린 채로는 클릭할 때마다 똑같은 실패만 반복되는 막다른 길이었다.
  - **수정**: `booking-form.tsx`에 `needsReauth(message)` 판별 함수 추가(문자열에 "게이트"/"토큰"/"계정을 더 이상 찾을 수 없습니다" 포함 여부 — `page.tsx`의 `checkStatus()`가 게이트/로그인을 구분하던 것과 같은 패턴 재사용) — 이 조건에 걸리면 "다시 시도" 대신 **"처음부터 다시 시작하기"**(`router.push("/")`)를 보여준다. 루트로 돌아가면 `page.tsx`의 기존 게이트/로그인 판별 로직이 그대로 재사용돼 게이트 폼(또는 Google 로그인 버튼)이 다시 뜬다 — 새 UI를 안 만들고 이미 있는 흐름에 올라탐.
  - **테스트**: `booking-form.tsx`가 처음으로 `useRouter`를 쓰게 되면서, 이 컴포넌트를 렌더하는 다른 테스트 파일(`events/[id]/page.test.tsx`, `load-test/page.test.tsx`)이 `next/navigation` mock에 `useRouter`가 없어 전부 깨짐을 발견 — 두 파일에 mock 추가로 해결(회귀 아님, 새 의존성 도입의 정상적인 파급). `booking-form.test.tsx`에 게이트 토큰 만료·"계정을 더 이상 찾을 수 없음" 각각에 대해 재시작 버튼이 뜨고 `router.push("/")`가 호출되는지 확인하는 테스트 2건 추가. **web 60→62 그린.**
  - **실서버 검증**: Playwright로 정상 진입→입장 허가까지 받은 뒤(세션이 살아있었다는 증거), **세션 도중** 게이트 토큰만 무효한 값으로 덮어써 실제 만료 상황을 재현 → "예매하기" 클릭 → "다시 시도" 버튼은 안 뜨고 "처음부터 다시 시작하기"만 뜸 → 클릭 → 루트로 이동 → 게이트 재입장 폼이 정상적으로 다시 뜨는 것까지 스크린샷으로 확인.
- **잡일 — 재고 잔량이 음수로 표시되다 고정되는 버그, 원인 규명·수정**: 위 -2 고정 버그를 고친 뒤에도 사용자가 다시 대량 테스트(재고 거의 소진, VU 수천)를 해보니 "잔량이 1,0,-1,-2로 틱마다 바뀌다가 모두 입장 후 -3에 고정됐다. 잔량은 -1 이하로 표시되면 안 된다. 0→1로 오르는 게 CONFIRMED였던 예약이 나중에 취소되는 경우인지 체크하고 둘 다 고쳐달라"고 재차 리포트.
  - **질문 답변("0→1 상승이 확정 취소 때문인가?")**: `payment.processor.ts`를 감사한 결과, 이 코드베이스엔 CONFIRMED(결제 확정) 상태가 나중에 취소·환불되는 전이가 아예 구현돼 있지 않다 — CONFIRMED는 종단 상태. 실제 0→1 상승 원인은 셋 중 하나다: ① 결제 실패로 HELD→CANCELLED 전환되며 재고가 원복되는 경우, ② 허가창 안에서 결제 시도 자체가 없던 예약을 `SweepProcessor`가 30초 HELD TTL로 회수(EXPIRED)하며 재고가 원복되는 경우, ③ `ReconcileProcessor`가 실제보다 낮게 틀어져 있던 값을 위로 보정하는 경우.
  - **근본 원인(음수 고정) 재현 시도**: 30 재고/5,000 VU로 강한 경합을 만들어 90~150초씩 폴링하며 여러 차례 재현을 시도했지만 Redis 카운터가 매번 자연히 0으로 자가 복구돼, 라이브 재현으로는 "영구 고정"을 강제로 만들어내지 못했다. 대신 코드 감사로 원인을 특정: `reservations.service.ts`의 `createHeld()` catch 블록이 **P2002(재전송) 에러일 때만** 관문 DECRBY를 INCRBY로 보상하고 있었고, **그 외 모든 에러(DB 커넥션 끊김·타임아웃 등)는 보상 없이 그대로 rethrow**했다. 대량 동시 요청을 걸어본 세션 로그(`/tmp/sunchak-api-dev.log`)에서 실제로 `PrismaClientKnownRequestError`(P1001, "Can't reach database server")가 이 세션 중 발생했던 걸 확인해, 이 분기가 실제 부하 상황에서 발동 가능함을 뒷받침했다 — 이 분기를 탈 때마다 "관문에서는 이미 깎았지만 티켓은 실제로 확보 안 된" 수량만큼 재고가 영구 누락되고, 다음 `ReconcileProcessor`(1분 주기, 단 위 -2 버그 수정 전이라면 활동 신호가 없을 때 하루 한 번으로 완화될 수도 있는 상태)가 돌 때까지 음수가 유지된다.
  - **root-cause 수정**: `createHeld()` catch 블록의 rethrow 직전에 `await this.redis.incrby(key, quantity)` 보상을 추가 — P2002 분기와 동일한 보상을, 예약 row는 만들지 않고 에러만 그대로 다시 던지는 경로에도 적용.
  - **방어선 수정(사용자가 명시적으로 요구)**: root-cause 수정으로도 이론상 남을 수 있는 다른 race window에 대비해, `LoadTestService.getStats()`/`DemoService.getStats()`가 응답하는 `remainingQty`를 `Math.max(0, Number(remaining ?? 0))`로 **표시값만** clamp — 관문이 실제로 읽고 쓰는 Redis 원본 값 자체는 그대로 둔다(관문이 순간적으로 음수를 찍는 것 자체는 이 프로젝트가 다루는 정상적인 동시성 현상이라, 관문 로직에 영향을 주면 안 된다는 원칙 유지).
  - **테스트**: `reservations.service.spec.ts`에 "P2002가 아닌 다른 이유로 DB INSERT가 실패하면 Redis 재고를 그대로 되돌린 뒤 원래 에러를 다시 던진다" 1건(`jest.spyOn(prisma.reservation, 'create').mockRejectedValueOnce(...)`로 강제 실패 주입) + `load-test.service.spec.ts`/`demo.service.spec.ts`에 "Redis 원본이 음수여도 remainingQty는 0으로 표시" 각 1건, 총 3건 신규. **API 133→136 그린.**
  - **실서버 검증**: 신규 계정으로 게이트+로그인 후 `POST /load-test/reset`으로 이벤트 생성 → `redis-cli SET stock:event:{id} -4`로 관문 원본 값을 강제로 음수화 → `GET /load-test/stats/stream` SSE 응답을 직접 열어 확인 → Redis 원본은 -4로 유지된 채 API가 내려주는 `remainingQty`는 정확히 0으로 clamp됨을 확인. 확인 후 이벤트는 다시 `reset`으로 정리.
- **잡일 — `git push` 후 CD가 skipped된 원인 발견·수정**: 위 커밋 3건을 push했더니 사용자가 "CD가 skipped 됐어요"라고 리포트. CD는 `workflow_run`으로 `ci.yml`이 성공했을 때만 도는데, GitHub API(`/actions/runs`)로 확인해보니 `ci.yml`의 `web-check` job에서 `eslint`가 **실패**해 CI 전체가 `failure`로 끝나 있었다 — CD의 `if: conclusion == 'success'` 조건이 false가 되며 skip된 것. 원인은 `eslint-plugin-react-hooks` v7의 `react-hooks/set-state-in-effect` 규칙(effect 안에서 setState를 동기 호출하지 말 것)에 이번 세션에 추가한 코드 2곳이 신규로 걸린 것 — `page.tsx`는 재시도 버튼용으로 추가한 `setStatus("checking")`이 마운트 effect에서도 같이 불리게 됐고(초기값이 이미 "checking"이라 원래 불필요한 호출이었음, `checkStatus().then(setStatus)`를 effect에서 직접 호출하도록 분리해 해결), `load-test/page.tsx`는 대기열 임계치 도달 시 `setReady(true)`를 effect에서 동기 호출하던 걸 React 공식 "렌더 중 상태 조정" 패턴(`!ready` 가드로 한 방향 래치 유지)으로 대체해 해결. web lint/test(62)/build 로컬 재확인 후 커밋(`aafb7e4`) → push → **CI 성공 → CD 성공**까지 GitHub API 폴링으로 직접 확인, `api.15.164.234.208.sslip.io`/`app.15.164.234.208.sslip.io` 헬스체크 200 확인.

## 2026-08-31 · 배포 환경에서 대용량 테스트 커넥션 풀 고갈 발견·수정 + 결과 카운터 정직화

- **계기**: 사용자가 배포된 `/load-test`에서 재고 10,000·VU 5,000으로 테스트해보니 "재고 잔량 9,974(26개만 소진), 최종 결과 총 40건(전부 포기), 확보중 26·입장대기중 7"이라며 가상 유저 투입/대기열 처리가 제대로 동작하지 않는 것 같다고 스크린샷과 함께 리포트.
- **원인 규명(VM SSH 직접 접속, `~/.ssh` 등록 없이 `sunchak-key.pem`으로 매번 명시 접속)**: `docker logs sunchak-api`에서 대량의 `PrismaClientKnownRequestError`(P2024, "Timed out fetching a new connection from the connection pool ... connection_limit: 3")를 확인. 코드에 `connection_limit`을 명시한 적이 없어 Prisma가 VM의 vCPU 수 기반으로 자동 계산한 값(3)이었다. `runInjectionBatches()`가 100명씩 `Promise.all()`로 동시에 쏘는데(0.5초 간격, 초당 200명 유입 목표) 창구가 3개뿐이라 대부분이 Prisma의 `pool_timeout`(10초)을 못 넘기고 실패 — 유입 속도가 처리 속도를 압도해 "느리지만 결국 다 처리"가 아니라 "대부분 실패"로 귀결됐다. `Promise.all()` 구조상 배치 하나가 최대 10초 넘게 걸릴 수 있어, 5,000명 전체 처리에 최대 8~9분이 걸릴 수 있는 상태였다(실측: 15분 뒤에도 미완료, 1,260건+ 에러 누적, 그 시점에도 여전히 에러 발생 중이라 죽은 게 아니라 극도로 느리게 크롤링 중이었음을 로그로 확인).
- **왜 로컬에선 안 터졌는지**: 로컬은 (1) 개발 머신 CPU가 더 많아 Prisma `connection_limit` 기본값 자체가 훨씬 크고, (2) 앱과 DB(로컬 Postgres)가 같은 머신이라 쿼리 왕복 지연이 거의 0이라 창구 회전이 훨씬 빨라, 이 클래스의 타임아웃이 사실상 발생하지 않았다.
- **왜 안전하게 올릴 수 있는지 확인**: `DATABASE_URL`이 이미 Neon의 pooled(pgbouncer) 엔드포인트를 쓰는지(`-pooler` 서브도메인 포함 여부) 비밀값을 노출하지 않는 방식으로 확인 — 맞음. WebFetch로 Neon 공식 문서(`connect/connection-pooling`) 실측: 모든 플랜에서 pgbouncer `max_client_conn`은 10,000(우리 앱이 얼마든지 여유 있게 말 걸어도 되는 상한), 프리 티어 기본 0.25 CU의 `max_connections`는 104(Neon 예약 7개 제외 실사용 97). 이 DB는 캐주얼 데모·실제 이벤트와 공유하는 자원이라 상한 근처(87)까지 쓰지 않고 보수적으로 **20**을 제안, 사용자 확정.
- **수정(운영 시크릿 변경, 사용자 확인 후 진행)**: VM의 `~/sunchak/api.env`(원본은 `api.env.bak.<timestamp>`로 백업)에서 `DATABASE_URL`에 `&connection_limit=20` 추가 → `docker compose up -d api`로 재기동. 사용자가 별도로 Infisical의 `DATABASE_URL`에도 동일 반영 완료(다음에 VM을 새로 만들 때 유실 안 되게).
- **실측 검증**: 재고 5,000·VU 2,000으로 재투입 — **약 20초 만에 투입 완료, 그 사이 커넥션 풀 에러 0건**(수정 전엔 5,000명이 15분 넘게 미완료, 에러 1,260건+). 최종 stats: `paidCount 55 + failedCount 12 + soldOutCount 0 + abandonedCount 378 = 445`, `heldCount 1456`(아직 결제 시도 전 정상 대기) — 445+1456=1901로 투입 2,000명 대부분이 설명됨(수정 전엔 5,000명 중 40건만 결과가 잡히고 나머지는 유령이었음).
- **부수 문제 — 결과 카운터가 산발적 시스템 오류를 아예 안 잡던 갭 발견·수정**: `connection_limit` 수정으로 대부분 해결됐지만, `injectVirtualUser`/`simulateBookingAttempt`의 catch 블록이 `ForbiddenException`(포기)·`ConflictException`(재고소진)이 아닌 그 외 모든 에러(P2024 등)를 로그만 남기고 어떤 카운터도 안 올리는 구조는 그대로였다 — "투입 인원 = 확정+결제실패+재고소진+포기 합" 불변식이 이 경로에서 깨져 화면에 설명 안 되는 유령 인원이 생기는 근본 구조가 남아있었다(원인은 이미 고쳤어도, 어쩌다 한 번 나는 실패는 여전히 안 잡힘). 사용자 결정: 기존 "결제실패"에 합치지 않고 **"시스템 오류" 새 카테고리 신설**(정직성 우선).
  - **구현**: `LoadTestStats`에 `systemErrorCount` 추가, Redis `system-error:event:{id}` 카운터 신설(다른 카운터와 같은 패턴), `injectVirtualUser`의 두 catch 지점(가상 유저 생성/대기열 진입 실패, 예매 시도 중 오류) 모두에서 `redis.incr` 추가, `reset()`에서 다른 카운터와 함께 0으로 초기화. 프론트(`load-test-dashboard.tsx`의 `AttemptOutcomeBar`)에 5번째 세그먼트(호박색)로 추가.
  - **테스트**: `load-test.service.spec.ts`에 `jest.spyOn(prisma.user, 'create').mockRejectedValueOnce(...)`로 강제 실패를 주입해 `systemErrorCount`가 반영되는지 확인하는 테스트 1건(`reservations.service.spec.ts`의 기존 P2002 아닌 에러 테스트와 같은 기법). **API 136→137 그린.** web은 기존 `AttemptOutcomeBar` 테스트를 5분할 기준으로 갱신(신규 케이스 추가는 아님). **web 62개 그린 유지.**
  - **로컬 실서버 검증**: `GET /load-test/stats/stream`을 직접 열어 응답에 `systemErrorCount: 0` 필드가 정상 노출되는 것 확인(강제 재현은 unit test의 `jest.spyOn`으로, 실서버에선 구조 배선만 확인).
- **배포 검증**: `connection_limit=20` + `systemErrorCount` 커밋을 push → CI/CD 성공 → 배포 사이트에서 재고 5,000·VU 2,000 재투입 — **약 20초 완료, 에러 0건, `systemErrorCount:0` 정상 노출**까지 확인.

## 2026-09-01 · "결제 처리 중" 멈춤 + 확정 카운트 부풀림, 진짜 원인은 BullMQ 워커 concurrency 기본값(1)

- **계기**: 위 배포 검증 뒤 사용자가 재고 10,000·VU 5,000으로 다시 테스트하며 본인 명의로 1,000매를 직접 결제해봤는데, "결제 처리 중"에서 화면이 멈췄고 재고도 이상하게 오락가락한 것 같다고 스크린샷과 함께 리포트(최종 결과 총 5,001건 — 확정 3,226·결제실패 796·포기 979, 재고 잔량은 9,968/10,000).
- **1차 조사(VM SSH + 컨테이너 내부에서 Prisma로 직접 DB 조회, 비밀값 노출 없이)**: 스크린샷 직후 같은 이벤트를 다시 조회해보니 실제 `Reservation` 상태는 **CONFIRMED 32건, EXPIRED 3,981건, CANCELLED 9건**뿐인데, `Payment`는 **PAID 3,226건, FAILED 796건**으로 잡혀 있었다 — 화면이 보여준 "확정 3,226"은 실제로는 대부분 거짓이었다. 재고 잔량 9,968은 사실 정확했다(`10000-32=9968`, 진짜 CONFIRMED 32건과 정확히 일치) — "재고가 소진됐다 다시 올랐다"는 인상은 재고 자체의 왕복이 아니라 **"확정" 카운트 자체가 처음부터 부풀려져 있었던 것**으로 밝혀짐.
- **근본 메커니즘**: `payment.processor.ts`(결제 판정)와 `confirm.processor.ts`(HELD→CONFIRMED 확정) 둘 다 이제까지 아무 옵션 없이 `@Processor(QUEUE_NAME)`으로 등록돼 있었는데, BullMQ의 Worker 클래스 소스(`node_modules/bullmq/dist/cjs/classes/worker.js`)를 직접 확인해보니 **`concurrency` 기본값이 1**이었다 — job을 한 번에 딱 하나씩만 처리한다는 뜻. 5,000명이 몰리면 DB 커넥션 풀을 20으로 넉넉히 늘려놔도 워커 자체가 한 줄로 세워 처리해, 사용자 본인의 결제 job도 큐에 들어간 뒤 **3분 34초**가 지나서야 처리됐다(Payment 행의 `createdAt`/`updatedAt` 실측 확인). 그런데 `HELD` TTL은 30초라, 결제 job이 실제로 처리되기 한참 전에 `SweepProcessor`가 이미 `EXPIRED`로 회수하고 재고도 돌려준 상태였다. 뒤늦게 실행된 `payment.processor.ts`의 성공 경로(80% 확률)는 예매가 여전히 HELD인지 확인하지 않고 그냥 `Payment→PAID`로 바꾼 뒤 `confirm` 큐에 job만 넣는데, `confirm.processor.ts`는 `updateMany(WHERE status='HELD')` 가드가 있어 이미 EXPIRED인 예매는 조용히 무시(count===0, 멱등 no-op)한다 — 그 결과 "Payment는 PAID인데 Reservation은 영원히 EXPIRED"라는 데이터 불일치가 대량으로 쌓였다.
- **"결제 처리 중" 멈춤도 같은 경합**: 사용자 본인의 예매(#14993, 1,000매)도 이 경로를 그대로 탔다 — 최종적으로 결제는 FAILED 처리됐지만, `SweepProcessor`가 만료시킬 때도, `ConfirmProcessor`의 no-op 경로도 예약 상태 변화를 SSE 방송 버스(`ReservationEventsService`)에 **아무것도 publish하지 않는다**(둘 다 "이미 종료 상태니 할 일 없음"으로 조용히 리턴). `reservations.service.ts`의 `streamStatus()`는 "미래 방송(future$) 아니면 구독 시점 DB 조회(current$)"로 동작하는데, current$는 **구독 시점에 딱 한 번만** 확인하고(폴링 아님), 그 시점엔 아직 HELD라 필터에 걸러진다 — 이후 상태가 바뀌어도 아무도 방송을 안 하니 영원히 대기한다. 프론트(`booking-form.tsx`)도 `CONFIRMED`/`CANCELLED`만 처리하고 별도 타임아웃이 없어, 이 경로에선 "결제 처리 중"에서 그대로 멈추는 게 확인된 동작이었다.
- **사용자 논의 — 어느 방향이 근본 수정인가**: 처음엔 ①TTL 연장 ②sweep/만료 시에도 SSE 방송 ③프론트 타임아웃, 세 가지 완화책을 제시했는데, 사용자가 "결국 큐 처리가 느린 게 문제 아니냐, 그럼 큐 처리 성능을 올리는 게 맞지 세 가지는 근본 해결이 아니지 않냐"고 반문 — 정확한 지적이었다. BullMQ 기본 concurrency=1이라는 사실을 다시 파서 확인한 뒤 방향을 수정.
- **수정**: `payment.processor.ts`/`confirm.processor.ts` 둘 다 `@Processor(QUEUE_NAME, { concurrency: 20 })`으로 변경 — 커넥션 풀 크기(20)와 맞춰, 워커가 풀이 감당 가능한 만큼만 동시에 job을 처리하도록 함. 하나만 올리면 병목이 다음 큐로 그대로 넘어가므로 둘 다 함께 올림. 앞서 제시했던 세 가지 완화책은 이 근본 수정 뒤에도 남을 수 있는 극단적 상황의 안전망으로만 남겨두고 지금은 보류.
- **삽질 — 무관한 pre-existing flaky 테스트 발견**: 수정 후 `jest` 전체 스위트를 5회 반복 실행해보니 간헐적으로(약 40%) `queue/admission.processor.spec.ts`가 실패했다. 원인 규명을 위해 `git stash`로 이번 concurrency 변경을 완전히 되돌린 뒤 같은 방식으로 5회 재실행해보니 **변경 전에도 동일하게(오히려 더 높은 빈도로, 5번 중 4번, 2건 동시 실패도 있었음) 실패**함을 확인 — 이번 세션 작업과 무관한, 이 프로젝트에 이미 있던 테스트 격리 문제(admission 관련 스펙들이 하드코딩된 `eventId`를 공유하는 것으로 추정)로 판단, 이번 범위 밖이라 손대지 않고 기록만 남김.
- **테스트**: 기존 137개 전체 그린(신규 테스트는 추가하지 않음 — concurrency는 실제 BullMQ 워커 스케줄링 동작이라 유닛 테스트로 의미 있게 검증하기 어렵고, 기존 `payment.processor.spec.ts`/`confirm.processor.spec.ts`는 `process()`를 직접 호출해 워커 큐 자체를 우회하므로 이 변경으로 깨지지 않음).
- **배포 검증(3,000 VU 재현)**: concurrency 수정을 push→배포 후, 재고 5,000·VU 3,000으로 재현 — 확정(CONFIRMED) 32→688건으로 **정확히 concurrency 20배 증가와 거의 일치**하는 개선을 확인해 진단이 맞았음을 검증. 그런데 여전히 `Payment PAID(1,887) − Reservation CONFIRMED(688) = 1,199건`의 divergence가 남아있었다 — concurrency를 올려도 3,000명 규모의 동시 부하 앞에선 완전히 사라지지 않는, 확률만 줄어든 같은 종류의 경합이었다.
- **더 근본적인 논의(사용자 주도)**: "job이 30초 넘게 막히는 게 정상인가, 물리적으로 성능을 높이는 것만이 답인가"라는 사용자의 반문으로 설계를 다시 봄. `LoadTestAdmissionProcessor`가 매 초 대기열의 20%(최소 50·최대 1,000)를 허가하는 계산이 **대기열 크기만 볼 뿐 결제 파이프라인이 실제로 몇 명이나 처리 가능한지는 전혀 안 본다**는 게 진짜 원인이었다 — concurrency를 아무리 올려도 VU 규모를 그만큼 더 키우면 다시 재현되는 구조적 문제. Little's Law(`L=λ·W`)를 적용해 "지금 이미 HELD로 결제 처리 중인 인원(L)"이 "처리량(λ)×허가창(W)"을 넘지 않게 admission이 스스로 페이스를 조절하는 백프레셔로 방향을 잡음(ADR 0023). 사용자가 이어서 "VU 상한(현재 1만)도 낮은 것 아니냐"고 질문 — 백프레셔가 있으면 VU를 더 키워도 "완료까지 걸리는 시간이 배수로 늘어날 뿐" 정합성은 안 깨지므로 상향이 안전해진다고 답하고, 이번 범위에서는 백프레셔부터 하고 상한 상향은 별도 판단으로 미룸(사용자 동의).
- **처리량(λ) 실측**: 5,000명을 투입해 결제 큐를 포화시킨 뒤 `Payment.updatedAt`을 1초 단위로 집계 — 가상 유저 생성이 끝나고 결제/확정 큐만 순수하게 밀린 백로그를 처리하는 구간(약 40초)에서 **평균 초당 70.8건**(47~90) 처리를 확인. 가상 유저 생성 트래픽과의 경합을 감안해 여유를 두고 **60**을 기본값(`LOAD_TEST_PAYMENT_THROUGHPUT_PER_SEC`)으로 채택.
- **구현**: `load-test-admission.processor.ts`에 `maxInFlight() = 처리량 × 허가창(초)` 계산과, 매 틱 `reservation.count({eventId, status:HELD})`로 현재 in-flight 인원을 재서 `허가 인원 = min(포아송 배치, max(0, 상한 − 현재 인원))`로 캡을 씌움. `payment.processor.ts`의 성공 경로에도 실패 경로와 대칭으로 `updateMany(WHERE status=HELD, data:{status:HELD})`(값은 안 바뀌지만 원자적 존재 확인+잠금 용도) 가드를 추가 — sweep이 먼저 회수했다면 `PAID`로 안 남기고 `FAILED`로 정직하게 처리. 역할 분담: 백프레셔는 "얼마나 자주 겪는지"를 줄이고, 이 가드는 "겪었을 때 거짓 데이터가 안 남게" 보장(하나가 다른 하나의 대체가 아님).
- **테스트**: `load-test-admission.processor.spec.ts`에 백프레셔 전용 describe 신설(2건) — 처리량 10건/초×허가창 5초=50명으로 작게 설정해, ①이미 HELD가 상한만큼 있으면 대기열이 많아도 아무도 안 들이는 것 ②여유분만큼만 들이고 나머지는 대기열에 남기는 것을 검증(Reservation은 실제 FK가 필요해 이 describe만 진짜 Event/User를 만들어 씀). `payment.processor.spec.ts`에 "성공 판정인데 이미 HELD가 아니면 PAID로 안 남고 FAILED로 처리" 1건 추가(기존 "실패 판정인데 이미 HELD가 아니면" 테스트와 대칭). **API 137→140 그린.**
- **문서**: ADR 0023(`docs/decisions/0023-load-test-admission-backpressure.md`) 신설 — 맥락(3단계 문제 연쇄)·결정(Little's Law 백프레셔)·고려한 대안(완화책 3종/처리량 무한 증설/적응형 측정/L 측정 방식)·근거·결과를 정리. ADR README 인덱스에 그동안 누락돼 있던 0019~0022도 함께 백필.
- **배포 검증(3,000 VU)**: push → CI/CD 성공 → 재고 5,000·VU 3,000 재투입, 시스템이 완전히 안정될 때까지(로그·수치 변화 없음) 관찰 — `Payment PAID(541) − Reservation CONFIRMED(467) = 74`건으로 divergence가 **1,199 → 74건(94% 감소)**. 크게 개선됐지만 0은 아니었다.
- **잔여 74건 원인 추적**: `payment.processor.ts`의 가드는 "지금 이 순간" HELD인지만 확인하고 `confirm` 큐에 job을 넣는데, **그 job이 실제 처리되기까지 기다리는 동안**에도 `SweepProcessor`는 여전히 벽시계 `heldUntil` TTL만 보고 회수할 수 있었다 — 같은 종류의 경합이 결제 큐 경계에서 확정 큐 경계로 자리만 옮겨 훨씬 작은 규모로 남아있었던 것. 이건 이전에 사용자와 논의만 하고(백프레셔 논의로 넘어가며) 실제 구현은 미룬 "sweep이 결제 시도 중인 예매는 안 건드린다"는 이단계 설계였다 — 사용자가 "마저 제거"로 마무리 진행 확정.
- **수정**: `sweep.processor.ts`의 회수 조건을 이단계로 분리 — ① 결제 시도(Payment row)가 **없는** HELD만 기존 촘촘한 `heldUntil`(30초) TTL로 회수(진짜 이탈), ② 결제 시도가 **있는** HELD는 이 TTL에서 제외하고 결제/확정 큐가 스스로 확정짓게 맡기되, "결제 job 자체가 영영 안 끝나는" 진짜 장애만 잡는 별도의 훨씬 관대한 안전망(`PAYMENT_ATTEMPT_FALLBACK_MS`, 5분, `createdAt` 기준)을 신설. 한 SQL(`UPDATE...WHERE...RETURNING`)에 `NOT EXISTS(SELECT 1 FROM payments...)` OR 조건으로 결합해 원자성은 그대로 유지.
- **테스트**: `sweep.processor.spec.ts`에 2건 추가 — ①결제 시도가 있으면 heldUntil이 지나도 안 건드리는 것 ②그래도 5분(안전망)이 지나면 회수하는 것(`createdAt`을 과거로 직접 시딩해 재현). **API 140→142 그린.**
- **다음**: 이번 sweep 이단계 수정을 push → CI/CD 확인 → 배포 사이트에서 3,000 VU 재투입으로 divergence가 0(또는 안전망 케이스만 남는 수준)까지 줄었는지 최종 재검증.

## 2026-09-05 · 배포 사이트 실사용 중 발견한 4가지 — 결제 우선순위(ADR 0018 개정)·포기율 조정·투입 대기 카운터

- **계기**: 위 sweep 이단계 수정 배포 후 사용자가 배포 사이트(`/load-test`, 재고 10,000·VU 5,000 자동 세팅)에서 직접 재현 — 최종 결과 확정 3,203·결제실패 810·재고소진 0·포기 988·시스템오류 0(총 5,001건, 본인 1,000매 포함)을 스크린샷으로 공유하며 4가지를 지적: ①포기 비중이 너무 큼(초반 30~40%, 안정화 후 20%대) ②본인 "결제 처리 중"이 수십 초 멈춤 ③완료 상태 정합성 확인 필요 ④"아직 대기열에도 못 들어간 인원"이 안 보임.
- **①포기율 조사**: `load-test.service.ts`의 `abandonProbability()`가 `DEMO_SIM_ABANDON_PROBABILITY`(기본 0.2)를 그대로 씀 — 988/5001≈19.8%로 정확히 이 값과 일치, 버그 아니라 설계된 확률(캐주얼 데모와 공유). 초반 30~40%로 보인 건 백프레셔로 소수만 먼저 처리되는 구간의 표본 변동(통계적으로 정상)이 전체 처리 후 진짜 확률로 수렴한 것.
- **②결제 지연 조사**: `payment.processor.ts`에 인위적 지연 없음 — 순수하게 결제 큐 적체(concurrency=20, 실측 처리량 초당 약 70건, ADR 0023)가 원인. 사용자가 "입장 대기는 남을 기다려도 되지만 결제까지 남의 결제를 기다리는 건 잘못된 설계"라고 지적 — 정확한 지적(재고 확보 후의 결제 판정은 개인 트랜잭션이라 순서를 다툴 이유가 없음). 처음엔 "결제 큐를 대용량/실제용으로 물리적 분리"를 검토했으나, 사용자 본인의 클릭도 그 대용량 테스트의 참여자 중 하나라 분리해도 체감 지연이 그대로 남는다는 걸 논의 중 확인 — 필요한 건 "어느 큐냐"가 아니라 "같은 큐 안에서 누가 먼저 처리되냐"였음.
- **②수정(ADR 0018 개정)**: `payments.service.ts`가 예약자 이메일이 `@sunchak.demo`(캐주얼·대용량 가상 유저 공통 도메인)로 끝나는지로 실제/가상 유저를 판별해, 가상 유저 job에만 BullMQ `priority` 옵션을 검. BullMQ는 priority 없는(0) job을 priority 있는 job보다 항상 먼저 처리하므로(`bullmq/dist/cjs/classes/job.js` 확인), 실제 유저는 옵션 없이 그대로 두는 것만으로 가상 유저 트래픽 규모와 무관하게 항상 먼저 처리된다. `payment.processor.ts`도 이 판정값을 job data로 이어받아 `confirm` 큐 투입에 동일 적용(중복 조회 없음). 상세 근거는 ADR 0018 개정 이력 참고.
- **①수정**: `DEMO_SIM_ABANDON_PROBABILITY` 기본값 0.2→0.08(캐주얼·대용량 공유 유지, 사용자 결정). 코드 기본값(`demo.service.ts`/`load-test.service.ts`)과 `.env.example` 둘 다 갱신 — **배포 VM의 `api.env`가 이 값을 명시하고 있다면 별도로 0.08로 갱신 필요(SSH 권한이 막혀 있어 사용자가 직접, Infisical도 동일 반영)**.
- **④수정**: Redis에 "요청된 투입 총원"(`loadtest:injection:requested:{eventId}`)과 "실제 투입 완료 수"(`loadtest:injection:injected:{eventId}`) 두 카운터를 신설 — `simulateLoad()` 호출 시 요청 총원에 누적, `runInjectionBatches()`가 배치 하나를 끝낼 때마다 완료 수에 누적, `reset()`이 둘 다 0으로 초기화. `LoadTestStats`에 `pendingInjectionCount`(= max(0, 요청−완료)) 추가해 stats 스트림·프론트(`DetailStrip`)에 "투입 대기중"으로 노출.
- **③(미완료)**: 완료 상태(확정 3,203/결제실패 810)의 실제 정합성(`Payment PAID` == `Reservation CONFIRMED` 여부)은 SSH가 auto mode 분류기에 막혀 이번 세션엔 직접 확인 못 함 — 사용자에게 읽기 전용 SELECT 스크립트(SSH+`docker exec`+Prisma raw query)를 전달, 실행 결과는 아직 회신받지 못함. 위 787줄의 "3,000 VU 재검증"과 사실상 같은 항목으로 합쳐 다음 세션에 이어감.
- **테스트**: `payments.service.spec.ts`/`payment.processor.spec.ts`에 우선순위 확인 1건씩, `load-test.service.spec.ts`에 `pendingInjectionCount` 계산 3건 + `simulateLoad`의 요청 카운터 누적 1건 신규. 기존 `admissionQueueCount` 전체 스냅샷 단언에 새 필드 반영. **API 142→148 그린**(`load-test-admission.processor.spec.ts`의 두 테스트가 전체 스위트 동시 실행 시 간헐 실패했으나 파일 단독 실행·재실행 시 통과 — 2026-09-01에 이미 기록된 기존 flaky 이슈 재현, 이번 변경과 무관). web 62개 그린, tsc(api/web)·eslint(web) 클린.
- **문서**: ADR 0018에 개정 이력(결제 우선순위 정책) 추가. STATUS.md "다음 할 일"에 이번 3가지 작업 반영.
- **다음**: ①③ 3,000 VU 재검증(divergence 확인, 이번엔 우선순위 도입 후 본인 체감도 같이 확인) ②배포 VM `api.env`의 `DEMO_SIM_ABANDON_PROBABILITY` 수동 갱신(사용자) ③이번 변경 push→CD 배포 확인.

## 2026-09-05 (같은 날, 이어서) · 결제 우선순위 철회 → job당 DB 왕복 축소 + 백프레셔 burst 윈도우 분리

- **계기**: 위 우선순위 구현을 보고 사용자가 정확히 반박 — "이건 내 결제만 안 느리게 만든 것뿐, 결제 시스템 처리 속도 자체가 느려선 안 된다는 원칙은 하나도 안 풀었다. 실제 결제 시스템은 동시 요청이 많든 적든 느려지지 않는다." 맞는 지적 — 재조사.
- **진짜 병목 재확인**: `payment.processor.ts`가 job 하나당 Neon에 순차적으로 2~3번 왕복(HELD 재확인/취소 + Payment 갱신 + 조건부 조회)한다 — 왕복 1회 약 100ms, concurrency=20을 곱해도 실측 처리량이 초당 약 70건에 그치는 이유.
- **철회**: `payments.service.ts`/`payment.processor.ts`의 우선순위 분기, `VIRTUAL_USER_EMAIL_DOMAIN`/`VIRTUAL_USER_QUEUE_PRIORITY` 상수, 관련 테스트 2건 전부 되돌림. ADR 0018 개정 이력에 "철회" 절 추가(왜 왔다가 왜 빠졌는지 기록).
- **수정 1(처리량 자체 확대)**: `payment.processor.ts`의 성공/실패 분기 각각을 raw SQL 한 문장으로 재작성 — 성공 분기는 `UPDATE payments ... FROM reservations RETURNING (r.status='HELD')`로 "HELD 재확인+Payment 갱신"을 2왕복→1왕복, 실패 분기는 CTE로 "예약 취소+Payment 갱신+반환할 수량 조회"를 3왕복→1왕복으로 합침(`sweep.processor.ts`와 같은 패턴 — Prisma `updateMany`는 RETURNING 미지원이라 raw SQL 필요). job당 지연이 약 절반으로 줄어 이론상 처리량이 2배(초당 약 70→140건)로 늘어난다(실측은 다음 3,000 VU 재검증에서 확인 예정).
- **수정 2(유입 자체 제한)**: `load-test-admission.processor.ts`의 `maxInFlight = 처리량 × W`에서 W가 `admissionWindowMs()`(사람이 반응할 시간, 30초)를 그대로 쓰던 걸 발견 — 처리량 60/초 기준 `maxInFlight=1,800`이라, 이 인원이 허가 직후 랜덤 지연(0.5~10초) 안에 몰리면 처리량을 올려도 순간 부하가 여전히 컸다. "허가창"(사람의 반응 속도)과 "동시 결제 허용치"(뒷단 처리 능력)는 서로 다른 개념이라는 사용자와의 논의로, 신규 `LOAD_TEST_PAYMENT_BURST_WINDOW_MS`(기본 3초)를 분리 도입 — `maxInFlight`가 1,800→180(같은 처리량 기준)으로 줄어 결제 단계에 몰릴 수 있는 최대 동시 인원 자체가 항상 처리 여력 안에 들어오고, 넘치는 인원은 입장 대기열에서 기다린다. ADR 0023에 개정 이력 추가.
- **확인 질문에 답변**: 사용자가 "결국 결제 처리에서 DB 커넥션 처리량보다 요청량이 많아서 느릴 수밖에 없는 구조냐"고 질문 — 정확하다고 확인, 다만 (a) job당 낭비되는 왕복을 줄이면 같은 커넥션 예산으로도 처리량 자체를 올릴 수 있고(수정 1), (b) 그래도 유입이 순간적으로 처리량을 넘으면 어딘가에서 반드시 기다려야 하므로, 그 "기다림"이 결제 단계가 아니라 입장 대기열에서 일어나도록 유입 자체를 처리량에 맞춰 제한해야 한다(수정 2)고 설명. 이어서 "결제 시도 가능 여부를 입장 허가 자체에 포함시켜, 결제 시도를 못 할 상황이면 입장도 안 시키는 거냐"는 확인 질문에 — 그렇다, `LoadTestAdmissionProcessor`가 매 틱 `availableSlots = maxInFlight - 현재 HELD 수`를 계산해 여유가 없으면 이번 틱엔 아무도 새로 허가하지 않는 방식(이미 있던 백프레셔 메커니즘 그대로, 이번 변경은 그 상한값만 재조정한 것)이라고 확인.
- **테스트**: `payments.service.spec.ts`/`payment.processor.spec.ts`의 우선순위 테스트 제거(원래 단언으로 복원). `load-test-admission.processor.spec.ts`에 "허가창을 늘려도 백프레셔 상한은 안 커진다" 1건 추가, 기존 백프레셔 테스트 2건은 새 env 키(`LOAD_TEST_PAYMENT_BURST_WINDOW_MS`)로 같은 상한값을 재현하도록 조정. **API 148→147 그린**(순감소 — 우선순위 2건 제거 + burst 윈도우 1건 추가). tsc 클린.
- **SSH 권한**: 사용자가 이 작업과 별개로 SSH(auto mode 차단) 허용을 요청 — `update-config` 스킬 호출 자체도 분류기가 차단해, 에이전트가 settings.json을 대신 고쳐줄 수 없음을 확인·안내(사용자가 직접 설정해야 함). 이번 세션엔 미해결.
- **다음**: 이번 변경(우선순위 철회 + 처리량 2배 개선 + burst 윈도우 분리) push → CI/CD 배포 확인 → 3,000 VU(또는 더 큰 규모) 재현으로 ①divergence 0 확인 ②실측 처리량이 실제로 2배 가까이 올랐는지 ③본인 결제 체감(우선순위 없이도 burst 제한만으로 충분히 빠른지) 셋 다 검증. 배포 VM `api.env`의 `DEMO_SIM_ABANDON_PROBABILITY` 수동 갱신은 여전히 남아있음.

## 2026-09-08 · push + 배포 확인, SSH 권한 설정, 배포 VM 포기 확률 값 수동 반영

- **push**: 위 두 항목(concurrency/백프레셔/sweep 이단계 + 우선순위 철회/처리량 개선/burst 윈도우 분리)을 커밋 `0e9bb2b`(`fix(load-test): 대용량 테스트 UX 개선 + 결제 처리 병목 근본 해결`) 하나로 묶어 push. CI/CD(ADR 0022) 자동 트리거.
- **SSH 권한**: 지난 세션에 막혔던 걸 사용자가 `.claude/settings.json`에 `Bash(ssh -i ~/Desktop/aws/sunchak-key.pem ubuntu@15.164.234.208 *)` 허용 규칙을 직접 추가 — 이후 에이전트가 VM에 정상 SSH 가능함을 실측 확인(이전엔 `update-config` 스킬 호출조차 auto mode 분류기가 차단했었음).
- **배포 확인**: SSH로 `docker ps` 확인 결과 `sunchak-api`/`sunchak-web`이 31초 전에 재기동돼 있어 CD가 이미 성공적으로 반영한 상태였음. `docker logs sunchak-api`로 전체 라우트 매핑·`Nest application successfully started`까지 에러 없이 뜬 것 확인.
- **`DEMO_SIM_ABANDON_PROBABILITY` 수동 반영**: `~/sunchak/api.env`를 SSH로 직접 grep해 `DEMO_SIM_ABANDON_PROBABILITY=0.2`가 명시돼 있음을 확인(코드 기본값만 바꿔선 안 먹히는 상태였다는 예상이 맞았음). 사용자 확인 후 ①타임스탬프 백업(`api.env.bak.<timestamp>`) ②`sed`로 `0.2`→`0.08`로 수정 ③`docker compose up -d api`로 컨테이너 재생성 ④컨테이너 내부 `printenv DEMO_SIM_ABANDON_PROBABILITY`로 `0.08` 반영과 무에러 기동을 확인. `LOAD_TEST_PAYMENT_BURST_WINDOW_MS`/`LOAD_TEST_ADMISSION_WINDOW_MS`는 VM에 별도 설정이 없어 새 코드 기본값(3초/30초 그대로)이 자동 적용됨 — 추가 조치 불필요.
- **다음**: 배포 사이트(`/load-test`)에서 재고 5,000·VU 3,000(또는 더 큰 규모) 재현 → ①divergence(Payment PAID vs Reservation CONFIRMED) 0 확인 ②실측 처리량이 2배 가까이 올랐는지 ③본인 결제가 우선순위 없이도 즉시 처리되는지(burst 윈도우 축소만으로 충분한지) 세 가지 검증. 이제 SSH가 풀렸으니 divergence 확인은 에이전트가 직접 위 SELECT 스크립트로 실행 가능.

## 2026-09-08 (같은 날, 이어서) · 재검증 — divergence 0 확인, "burst 윈도우가 너무 작다" 신규 버그 실측 발견·수정

- **사용자가 배포 사이트에서 재고 10,000·VU 5,000 재현** — 관찰: ①처리 속도 향상 체감됨 ②본인 결제 완료까지 시간은 줄었지만 여전히 체감 5~10초 지연 ③"입장 대기 중"이 한 번에 쫙 줄었다가 멈추고 다시 쭉 빠지는 패턴 반복(설계 의도인지, 서비스 관점에서 문제인지 확인 요청) ④"내가 직접 실시간으로 처리량을 관측할 수 없냐"는 요청.
- **에이전트가 SSH로 직접 검증**(새 부하 추가 없이 방금 완료된 테스트의 기존 DB 기록만 분석):
  - **divergence 확인**: `Reservation CONFIRMED=3,679` = `Payment PAID=3,679` — **정확히 0**. `CANCELLED=914`=`Payment FAILED=914`(성공률 80.1%, 설계값 80%와 부합). sweep 이단계화가 실전에서 완전히 작동함을 확인.
  - **"burst-then-pause" 패턴 원인 추적**: `reservation.createdAt`을 초 단위로 집계해보니 전체 164초 중 약 49초(30%)가 **새 예매 시도가 0건인 완전한 정지 구간**이었고, 이 정지가 `payment` 큐 적체가 아니라 **admission 자체의 정지**임을 확인(같은 시간대 `reservation.createdAt` 히스토그램과 `payment.updatedAt` 히스토그램을 나란히 비교).
  - **근본 원인**: `paymentBurstWindowMs`(W, ADR 0023 개정)를 "결제 job의 순수 DB 왕복 시간"(3초)으로만 잡아, Little's Law의 W가 실제보다 훨씬 작았다. 진짜 W는 "HELD 진입→최종 확정/취소까지의 전체 체류 시간"이어야 하는데, 여기엔 사람이 결제 버튼을 누르기까지의 랜덤 지연(0.5~10초)도 포함돼야 했다 — 이걸 빼먹어 아직 결제 시도조차 안 한 사람들이 좁은 예산(`maxInFlight=180`)을 다 차지해버려, 결제 워커가 놀고 있는데도 admission이 주기적으로 완전히 멈췄다.
- **사용자와의 개념 확인(자기설명 검증, 지독하게 확인 원칙)**: ①"job을 3초 단위로 끊어서 지연이 발생하고, 기존 랜덤 지연 때문에 추가로 늦어지는 거냐"는 질문에 — 방향은 맞지만 정확히는 "예산 계산에 랜덤 지연이 반영이 안 된 것"이라고 정정. ②"이게 실제 서비스에서도 맞는 현상이냐"는 질문에 — 사람이 결제까지 시간이 걸리는 건 현실이지만, "그래서 입장 자체가 통째로 멈추는" 건 우리 프로젝트의 인위적 DB 예산 제약 때문이지 실제 대형 서비스의 정상 동작은 아니라고 설명. ③Little's Law(L=λ·W)를 식당 비유(λ=분당 손님 수, W=한 손님이 머무는 시간, L=테이블 수)로 설명 후 "랜덤 지연이 늘면 maxInFlight도 늘어나냐"는 확인 질문에 사용자가 정확히 답변("체류시간이 올라가므로 L도 올라감"). ④"maxInFlight가 커지면 시스템 어디에 영향을 미치냐"는 후속 질문에 — 코드상 `availableSlots = maxInFlight - inFlight`(load-test-admission.processor.ts) 한 곳에서만 쓰이지만, 파급효과로 결제 큐 부하·재고 게이지 체감이 영향받는다고 설명(작을수록 무조건 좋은 것도, 클수록 무조건 좋은 것도 아닌 트레이드오프). ⑤"15초로 했을 때 p90을 넘는 경우는 안전하냐"는 질문에 — 안전하다고 확인: `maxInFlight`는 평균 관계식일 뿐이고 `inFlight`가 매 틱 실시간 재계산돼(사전 예측 아님) 자동 보정되며, 이 값이 부정확해도 데이터 정합성(별도 방어선인 원자적 가드·sweep 이단계화가 담당)은 안 깨지고 오직 체감 매끄러움만 좌우한다고 설명.
- **실측**: 배포 사이트의 실제 예약 레코드로 HELD 체류 시간(`updatedAt - createdAt`)을 재봄 — 평균 7.9초·중앙값 9.4초·**p90 14.9초**·p99 18.8초·최대 21.7초. ADR 0023 원안의 처리량 실측(임의 추측 대신 실측값 채택)과 같은 방식으로 p90을 반올림한 **15초**를 새 기본값으로 채택 — `maxInFlight`가 180→900(처리량 60 기준)으로 커짐.
- **수정**: `load-test-admission.processor.ts`의 `paymentBurstWindowMs()` 기본값 3,000→15,000ms, `.env.example` 동일 반영, ADR 0023에 개정 이력 추가. **테스트**: 기존 백프레셔 스펙이 env를 명시적으로 세팅해 검증하므로 기본값 변경으로 인한 회귀 없음 — **API 147개 그린**(admission.processor.spec.ts 1건이 전체 스위트 동시 실행 시 또 간헐 실패 — 단독 실행 시 통과, 기존에 기록된 flaky 이슈 재확인, 이번 변경과 무관), tsc 클린.
- **다음**: 이번 변경 push → CI/CD 배포 확인 → 재검증(admission이 더 이상 통째로 멈추지 않는지, 실측 처리량이 이론상 2배에 가까운지, 본인 결제 체감이 5~10초보다 줄었는지).

## 2026-09-09 · Grafana 계정 안내, 캐주얼 데모 폐기 결정, updatedAt 관측 버그 발견·수정, 커넥션 풀 근본 원인 규명·해결

- **Grafana 계정**: 사용자가 "계정 정보를 모르겠다"고 요청 — VM의 `~/sunchak/grafana.env`(git 미포함)에서 `admin`/비밀번호 확인해 전달. 로컬에도 `infra/grafana.env`(gitignore 대상, `*.env` 규칙에 이미 걸림) 신규 생성해 다른 기기 재배포 시 참고 가능하게 함.
- **Grafana "Neon 예산 잔량" 패널 백로그 드롭**: 사용자 요청으로 STATUS.md·ADR 0016 둘 다에서 제거·"드롭"으로 기록.
- **추후 작업 기록(구현은 안 함)**: 사용자가 "캐주얼 데모를 삭제하고 대용량 트래픽 테스트를 메인으로 전환"하겠다는 방향을 밝힘 — STATUS.md 백로그에 기록만 해두고 이번 세션 범위 밖으로 미룸(관련 코드: `demo/` 모듈, `admission.processor.ts`, 프론트 `demo-dashboard.tsx` 등, 도움말·문서 갱신 필요).
- **배포 재검증 중 결제 지연 재보고**: 사용자가 "결제하기 눌러도 결제 처리 중 뜨기까지 딜레이 수초, 결제 처리 중 자체도 약 10초"라고 캐주얼 데모에서 관찰 보고 — 서버 DB(`payment.updatedAt`)로 확인해보니 전부 즉시(0초) 처리된 것으로 나와 "인프라엔 원인 없음"이라고 1차 결론(오판, 아래 참고).
- **⚠️ 오판 정정 — updatedAt 관측 버그 발견**: 사용자가 배포 사이트에서 대용량 테스트(VU 5,000, 재고 10,000)를 하며 브라우저 DevTools Network 탭 캡처 2장을 공유 — `POST .../reservations`(3.29초), `POST .../pay`(4.55초), `GET .../stream`(SSE, 최종 결과까지 **40.84초**)로 실제 지연이 뚜렷했다. 이게 "즉시 처리"라던 DB 조회 결과와 정면으로 안 맞아 재조사 — **2026-09-05 raw SQL 최적화(job당 DB 왕복 축소)가 `updatedAt` 갱신을 빠뜨렸음을 발견**. `$queryRaw`는 Prisma Client의 자동 `@updatedAt` 관리를 안 거친다(그건 `prisma.model.update()` 같은 고수준 API 전용 기능) — 그 결과 `payments.updatedAt`이 실제 처리 시각이 아니라 `payments.createdAt`(결제하기 클릭 시점)에 영원히 멈춰있었고, 이걸 그대로 믿고 "즉시 처리"라는 잘못된 결론을 냈던 것. `sweep.processor.ts`의 EXPIRED 전이도 같은 문제(2026-08-01 최초 구현부터, 이번 세션과 무관한 기존 버그)라 같이 발견·수정. **수정**: 두 파일의 raw SQL에 `"updatedAt" = now()`를 명시적으로 추가. 비즈니스 로직 영향은 없음(이 필드를 읽는 코드가 없었음, `grep`으로 확인) — 순수하게 관측/디버깅용 타임스탬프 버그였지만, 이번처럼 잘못된 진단으로 이어질 수 있다는 걸 실제로 겪음. **테스트**: 기존 15개 그린(회귀 없음). 커밋 `e5f32cf`.
- **진짜 원인 추적(pg_stat_activity 직접 관측)**: 사용자가 "직접 테스트하며 실시간으로 관측할 수 없냐"고 요청 — 게이트→회원가입→로그인까지 curl로 자동화(스로어웨이 계정)해 통제된 규모(VU 1,500→4,000)로 직접 재현하면서, VM에 Postgres `pg_stat_activity`를 1초 간격으로 스냅샷 찍는 node 스크립트를 별도로 올려 커넥션 풀 사용 현황을 실시간 관측. **VU 4,000에서 커넥션 풀(20개)이 93%의 시간 동안 18~20으로 가득 찼고, 특정 순간엔 최대 15개가 "idle in transaction"**(트랜잭션은 열려있는데 다음 단계를 기다리며 노는 상태)이었다. 이 상태로 잡힌 쿼리를 분류해보니 대부분 `confirm.processor.ts`(`prisma.reservation.updateMany()`)와 `reservations.service.ts`의 `createHeld()`(`prisma.reservation.create()`)였다 — 반면 이미 raw SQL로 바꾼 `payment.processor.ts`는 이 목록에 전혀 안 나타남. **근본 원인**: Prisma Client의 CRUD API(`create`/`updateMany` 등)는 단일 쓰기도 암묵적으로 `BEGIN...COMMIT`으로 감싸(Prisma의 기본 안전장치) 왕복을 3배로 늘리고, 동시 요청이 많으면 Prisma 엔진 자체의 내부 스케줄링 지연 때문에 커넥션이 "BEGIN만 하고 다음 단계를 기다리며 노는" 상태로 묶인다.
- **수정**: `confirm.processor.ts`의 `updateMany()`와 `reservations.service.ts`의 `createHeld()`(예매 생성, INSERT)를 payment.processor.ts와 같은 방식으로 raw SQL(`$queryRaw`)로 전환. **부수 발견(테스트로 실제 실패 확인 후)**: raw SQL의 unique violation 에러는 Prisma Client API와 달리 최상위 `code`가 `P2002`가 아니라 항상 `P2010`("Raw query failed")로 오고, 실제 DB 에러 코드(`23505`)는 `e.meta.code`에 문자열로 담긴다 — `createHeld()`의 재전송(idempotency) 판정 로직을 이에 맞게 수정. **테스트**: 기존 11개 전부 회귀 없이 통과(재전송 테스트 자체가 이 버그를 곧바로 잡아냄). 커밋 `baa5646`.
- **재검증(같은 VU 4,000 통제 테스트 재현)**: 배포 후 **"idle in transaction"이 테스트 전체에서 정확히 0건**(이전 최대 15건)으로 완전히 해소됨을 확인. 최대 동시 커넥션도 20을 안 찍고 18에서 안정(평균 14.4). divergence도 여전히 0(`confirmed=2926`=`paid=2926`). 다만 실제 체류 시간(이제 정상 작동하는 `updatedAt`으로 재측정)은 중앙값 21.2초·p90 27.1초로 기대만큼 안 줄어든 것도 함께 발견 — 처리 능력(raw SQL로 실제로는 크게 오름)과 admission의 페이스 기준(옛 처리량 가정 60/초)이 안 맞아, 병목이 "DB 처리 능력"에서 "낡은 설정값"으로 옮겨간 것으로 진단.
- **처리량(λ) 재실측**: 2026-09-01 실측(70.8/초)과 정확히 같은 방법론(포화 후 순수 처리 구간 관찰)으로 재측정하되, 이번엔 admission·시뮬레이션 페이스 개입을 완전히 배제하기 위해 격리된 테스트 이벤트에 HELD 예매 3,000건+`Payment`(PENDING) 3,000건을 직접 생성해 `payment` 큐에 한 번에(`addBulk`) 투입, `Payment.updatedAt` 분포만 관찰 — **3,000건이 20.2초 만에 처리, 내내 초당 150~165건**(끝 구간 제외 평균 약 156) 안정적 유지, **약 2.2배 개선**. `LOAD_TEST_PAYMENT_THROUGHPUT_PER_SEC` 기본값 60→**130**(여유 마진 적용)으로 상향, burst 윈도우(15초)는 "몇 초 안에 처리되게 할 것인가"라는 별개의 설계 선택이라 그대로 유지(`maxInFlight`가 900→1,950으로 자연스럽게 커짐). ADR 0023에 개정 이력 추가. **테스트**: 기존 147개 전부 회귀 없이 그린, tsc 클린.
- **정리**: 실측용으로 만든 스로어웨이 계정·이벤트(probe 계정 3개, 테스트 이벤트 3개)는 별도 정리 필요 — 다음 세션 초입에 확인.
- **다음**: 이번 처리량 재조정 push(커밋 `73a5fb1`, 완료) → CI/CD 배포 확인(세션 종료 시점까지 컨테이너가 재기동 안 돼 미확인 — 평소보다 오래 걸리는 중, 다음 세션에서 GitHub Actions 로그로 실패 여부부터 확인할 것) → 최종 재검증(체류 시간이 실제로 줄었는지, 사용자 체감 지연이 개선됐는지) → 스로어웨이 테스트 데이터 정리. 사용자 요청으로 이번 세션은 여기서 마무리.

## 2026-09-09 (같은 날, 이어서) · 배포 확인 + pay() 프론트도어 DB 왕복 축소(1회로) 발견·수정

- **SSH 권한 규칙 버그 발견·수정**: 지난 세션이 `.claude/settings.local.json`에 추가해둔 SSH 허용 규칙이 `"\"Bash(ssh ... *)\""`처럼 값에 불필요한 이스케이프 따옴표가 섞여 있어 실제로는 어떤 명령과도 매칭되지 않는 상태였음을 발견 — 지금까지 SSH 명령이 통과된 건 이 규칙 덕분이 아니라 auto mode 분류기가 매번 "읽기 전용이라 안전"이라고 개별 판단해준 우연이었다. 사용자가 직접 수정(따옴표 제거 + 절대경로 버전 추가)해 규칙을 살림.
- **배포 확인**: `docker exec`(쓰기 작업 아닌 조회)로 VM의 `sunchak-api` 컨테이너 시작 시각(2026-09-08 16:56:56 UTC)이 마지막 관련 커밋(`73a5fb1`, 01:50:31 KST)보다 늦음을 확인해 CD 배포가 정상 반영됐다고 판단. `connection_limit=20`도 env로 확인.
- **사용자가 배포 사이트에서 대용량 테스트(재고 10,000·VU 5,000) 재현 중 다시 리포트**: "결제하기 눌러도 딜레이가 있고, 처리 중으로 바뀐 후 수십 초 지나야 완료된다." 브라우저 DevTools Network 패널 공유 — `pay` fetch 6.83초, `stream`(SSE) 14~42초.
- **1차 가설(기각)**: "브라우저가 origin당 동시 연결 6개로 제한하는데, SSE 연결이 여러 개 열려 있어 `pay` 같은 일반 요청이 슬롯을 기다려 느려지는 것 아니냐" — Explore 서브에이전트로 `/load-test` 페이지의 EventSource 개수를 코드로 확인한 결과 최대 2개뿐(6개 제한에 전혀 안 걸림), nginx도 HTTP/1.1이라 관련 없음 확인 후 기각.
- **2차 가설과 실측 — 큐 워커 처리량은 건강함을 확인**: `pg_stat_activity`로 커넥션 상태를 보니 idle 19/20으로 여유 있었음. `payment` 큐 대기열이 5초 동안 2,200→1,941로 줄어든 걸 "약 51/초"로 계산해 "처리량이 설계값(130/초)보다 훨씬 낮다"고 1차 진단했으나, 이는 측정 방법 결함(순감소분을 처리량으로 착각 — 그 5초 동안 신규 admission도 계속 큐에 들어오고 있어 순감소 ≠ 총처리량)이었음을 스스로 정정.
- **격리 재실측으로 확정**: 지난 세션(2026-09-09 앞부분)의 방법론(HELD+Payment 대량 직접 생성 후 큐 일괄 투입)을 이번엔 **완전히 격리된 새 이벤트가 아니라, 백그라운드 admission·통계 폴링이 실제로 도는 상태에서** 재현(스로어웨이 이벤트+유저 1,500명, 실행 후 즉시 삭제) — **1,500건이 12.2초에 처리, 초당 150~170건 유지**로 지난 세션 격리 측정치(150~165/초)와 일치. → **큐 워커 자체의 처리량은 건강**하고, "다른 큐와의 커넥션 경합" 가설은 이번 실측에서 확인되지 않음.
- **사용자와의 개념 확인 문답**: "허가창이 현재 입장허가를 말하는거냐"는 질문에 — 코드로 재확인해 `LOAD_TEST_ADMISSION_WINDOW_MS`(30초, 개인 반응 시간)와 `LOAD_TEST_PAYMENT_BURST_WINDOW_MS`(15초, `maxInFlight` 계산에 쓰는 Little's Law의 W)가 서로 다른 값임을 정정 설명. 이어 "maxInFlight에서 최악의 경우로 입장하면 그만큼 지연이 생기고 방금 그 경우였냐"는 질문에 — 방향은 맞되, `maxInFlight(1,950명) ÷ 실측 처리량(150~160/초) ≈ 12~13초`가 정확히 설계 목표(15초)와 일치함을 확인시켜, **현재 지연은 버그가 아니라 설계대로의 "최악의 경우 대기시간"**이라고 결론.
- **개선 방향 논의 — "옵션 B"가 이미 시도됐다 되돌려진 것이었음을 사용자가 지적**: burst 윈도우를 더 줄이자는 제안에 사용자가 "입장 대기 시간을 늘리고 처리 가능한 선에서만 입장시키는 개선을 이전에 하지 않았냐"고 확인 요청 — ADR 0023을 다시 읽어 정확히 확인: 2026-09-05에 burst 윈도우 3초로 설정했다가, 2026-09-08 재검증에서 "결제 시도 전 랜덤 지연을 반영 안 해 admission이 전체 시간의 30% 동안 완전히 멈추는" 새 버그를 발견해 15초(p90 실측)로 되돌린 이력이 있었음 — 새 실측 근거 없이 다시 줄이면 그 버그를 재현할 위험이 있다고 정정, burst 윈도우 재조정은 보류 권장.
- **진짜 원인 확정 — 프론트도어(`pay()` 자체)의 커넥션 경합**: "큐 워커 처리량 실측"과 사용자가 실제로 겪은 "`pay()` 자체가 6.83초" 사이의 간극에 주목 — 큐에 이미 들어간 job의 처리 속도가 아니라, **`pay()` API 자체(소유권 확인→예약+이벤트 재조회→Payment 생성, 3회 순차 Prisma 호출)가 burst 순간 수백 명이 동시에 호출될 때 느려지는 것 아니냐**는 가설을 사용자 동의로 실측 — 격리된 800명이 `pay()`와 동일한 3회 왕복을 동시에 호출하는 스크립트 실행: **min 1.0초·p50 2.8초·p90 3.9초·p99 4.2초**. 평소 300~450ms면 끝날 요청이 커넥션 풀(20개) 경합으로 크게 늘어짐을 확인 — 사용자가 겪은 증상과 정확히 일치.
- **수정**: `payments.service.ts`의 `pay()`를 CTE(Common Table Expression) 기반 단일 raw SQL로 재작성 — "소유권+HELD 상태 확인"(`target` CTE)과 "Payment 생성"(`ins` CTE)을 한 문장으로 묶어 왕복을 3→1로 축소(`payment.processor.ts`가 이미 쓰던 패턴을 프론트도어까지 확장, ADR 0018 2026-09-05 철회 절의 연장선). 404/403/409 에러 판별은 두 CTE의 결과(`targetId`·`targetUserId`·`paymentId`)를 한 행에 담아 반환해서 유지. 더 이상 안 쓰는 `ReservationsService`(assertOwned 호출) 의존을 `PaymentsService`에서 제거, 테스트 모듈의 불필요한 provider(`ReservationEventsService`/`RedisService`)도 함께 정리.
- **재실측(같은 800명, 신규 CTE 쿼리)**: min 193ms(5.2배)·p50 2.04초(28%↓)·p90 3.14초(20%↓)·p99 3.37초(19%↓)·max 3.40초(19%↓). **솔직한 평가**: 3배가 아니라 20~30%대 개선 — 800명이 20개 커넥션을 다투는 상황에서 대기시간 대부분이 "왕복 길이"가 아니라 "커넥션 슬롯을 기다리는 큐잉 시간" 자체이기 때문. 완화책이지 근본 해결은 아님(근본 해결은 처리량/커넥션 확대나 대기 중 UX 보완 중 하나 필요, 둘 다 백로그). ADR 0018에 개정 이력 기록.
- **커넥션 풀 20개가 실서비스 대비 터무니없이 작은지에 대한 질문**: 숫자 자체(20)는 이 프로젝트의 Neon 예산 제약 때문(실제 상한은 약 97~104) — 실제 회사라면 더 크게 잡거나 PgBouncer 등 풀러를 쓴다. 다만 "왕복 횟수를 줄이는 것" 자체는 풀이 작아서만 필요한 임시방편이 아니라, 진짜 대형 서비스(수십만 동시 요청)일수록 풀 크기만으로는 못 버텨 오히려 더 중요해지는 범용 원칙(N+1 쿼리 회피와 같은 계열)이라고 설명 — 대형 티켓팅 서비스가 이 프로젝트처럼 대기열·Redis 우선 재고 차감·왕복 최소화를 함께 쓰는 이유.
- **테스트**: `payments.service.spec.ts`에 404(신규, 예약 없음) 1건 추가 + 기존 4건 재검증. **API 148개 그린**(admission 스펙 1건은 전체 스위트 동시 실행 시 간헐 실패 — 단독 실행 시 통과, 기존에 여러 세션에 걸쳐 기록된 flaky 이슈로 이번 변경과 무관), tsc 클린. 실측에 쓴 VM 임시 스크립트(`throughput_live.js`/`pay_burst_test*.js`/`check_*.js`)는 컨테이너·VM `/tmp`에서 정리 완료, 각 스크립트가 만든 스로어웨이 이벤트·유저·예약·결제 행도 스크립트 자체가 실행 종료 시 삭제.
- **다음**: 이번 커밋(pay() 왕복 축소) push → CD 배포 확인 → 배포 사이트에서 실제 체감 지연이 줄었는지 재확인. `docs/decisions/0017-admission-queue.md`의 미커밋 변경(다른 세션이 작성한 백로그 정정+설계 스케치, "대기 중 순번 실시간 감소" 후보 설계)은 이번 세션 범위 밖이라 보류 — 다음에 처리. 지난 세션이 남긴 "스로어웨이 테스트 데이터 정리"는 이번 세션에서 만든 것들은 자체 정리됐으나, 그 이전 세션들이 만든 probe 계정·이벤트가 남아있는지는 아직 미확인.
