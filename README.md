...
이 프로젝트의 맥락은 전부 파일로 남아 있으므로, 새 세션에서 대화 기억을 옮길 필요가 없다. 이 폴더가 연결된 상태에서 다음만 읽으면 전체 맥락이 복원된다.

1. `README.md` — 개요·스택·진행 상황
2. `docs/DEVLOG.md` — 시간순 개발 이력(결정·삽질·배운 점)
3. `docs/decisions/` — 모든 선택의 대안·근거(ADR)
4. `git log` — 커밋 단위 이력

즉, 새 세션에서는 "레포 읽고 이어가자"고만 하면 위 문서로 catch-up이 된다. 기기 독립성이 필요하면 GitHub 원격에 push해두고 어느 기기서든 clone한다.
